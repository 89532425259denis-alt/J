...
MAX_WEB_SOURCES    = int(cfg("MAX_WEB_SOURCES", "35"))
MIN_REAL_SOURCES   = int(cfg("MIN_REAL_SOURCES", "25"))
BIB_SOURCE_TARGET  = int(cfg("BIB_SOURCE_TARGET", "35"))
...
def load_golden_example(doc_type: str) -> str:
    """Загружает эталонный текст из golden_{doc_type}.txt или golden_referat.txt."""
    candidates = [f"golden_{doc_type}.txt"]
    if doc_type != "referat":
        candidates.append("golden_referat.txt")
    for path in candidates:
        try:
            ...
            print(f"[GOLDEN] Не удалось прочитать {path}: {e}")
    return ""
...
async def call_openai_compat(
    ...
    if info.get("_fatal") or not info.get("api_key"):
        return ""
    # Единый «идеальный» режим: всегда даём модели большой бюджет.
    max_tokens = max(16000, int(max_tokens or 16000))
    timeout = max(600, int(timeout or 600))
...
async def call_openai_compat_with_search(
    ...
    if info.get("_fatal") or not info.get("api_key"):
        return "", []
    # Единый «идеальный» режим: большой бюджет и таймаут и для поиска.
    max_tokens = max(16000, int(max_tokens or 16000))
    timeout = max(600, int(timeout or 600))
...
def _dedupe_bibliography(bib_text: str) -> str:
    """Удаляет дублирующиеся источники и перенумеровывает список."""
    if not bib_text:
        return bib_text
    ...
    for line in lines:
        ...
        doi = re.search(r"10\.\d{4,9}/\S+", body, flags=re.I)
        if doi:
            key = doi.group(0).lower().rstrip(" .;)")
        else:
            key = re.sub(r"[^a-zа-яё0-9]+", " ", body.lower()).strip()[:140]
        if not key or key in seen:
            continue
        seen.add(key)
        unique.append(body)
    return "\n".join(f"{i}. {body}" for i, body in enumerate(unique, start=1))


def _fallback_bibliography_for_topic(topic: str, subject: str, start: int = 1, limit: int = 35) -> str:
    ...
    if ("трамп" in topic_l or "trump" in topic_l) and any(x in topic_l for x in ("торгов", "trade", "тариф", "пошлин")):
        base = [
            ...
            "Ruiz Estrada M.A., Park D., Lee M. The Effects of Trump Trade Policy on China’s Economic Performance // SSRN Electronic Journal. — 2019. — URL: https://doi.org/10.2139/ssrn.3418240.",
            "Ruiz Estrada M.A., Koutronas E., Lee M. The Effects of Trump Trade Policy on the Chinese Economy // SSRN Electronic Journal. — 2021. — URL: https://doi.org/10.2139/ssrn.3976000.",
            "Krieckhaus J. US Trade Policy, President Trump and the KORUS FTA // Korea Observer. — 2018. — URL: https://doi.org/10.29152/koiks.2018.49.4.619.",
            "Crump L. Trump on Trade // Negotiation Journal. — 2018. — URL: https://doi.org/10.1111/nejo.12250.",
            "Odell J.S. Trump and the Practice of International Trade Negotiation // Negotiation Journal. — 2018. — URL: https://doi.org/10.1111/nejo.12275.",
            "Chryssogelos A. Contesting international economic governance: the ‘people’ and trade in the Trump and Brexit rhetoric // Contestation and Polarization in Global Governance. — 2021. — URL: https://doi.org/10.4337/9781800887268.00012.",
            "Jones K. Transatlantic Trade, the Trump Disruption and the World Trade Organization. — 2020. — URL: https://doi.org/10.55271/rp00129.",
            "Jean S. International trade disagreements: Beyond Trump // Politique étrangère. — 2019. — URL: https://doi.org/10.3917/pe.191.0057.",
            "Peters M.E. Trump, Trade, and Immigration // The Forum. — 2019. — URL: https://doi.org/10.1515/for-2019-0036.",
            "Ciuriak D. How U.S. Trade Policy Has Changed Under President Donald Trump – Perceptions From Canada // SSRN Electronic Journal. — 2019. — URL: https://doi.org/10.2139/ssrn.3362910.",
            "McGee R.W. Trump and the Trade Policy of a Free Society // SSRN Electronic Journal. — 2016. — URL: https://doi.org/10.2139/ssrn.2858396.",
            "Gantz D.A. Impact of the Trump Trade Policies on the USMCA and Other Regional and Global Trade Agreements // SSRN Electronic Journal. — 2025. — URL: https://doi.org/10.2139/ssrn.5736862.",
            "Mavroidis P.C. No Way Out? The WTO in the Whirlwind of Trump II Trade Policy // Journal of World Trade. — 2025. — URL: https://doi.org/10.54648/trad2025038.",
            "Zheng Y. Trump Tariff Shockwaves: Trade Policy Uncertainty and Global Equity Connectedness // Finance Research Letters. — 2026. — URL: https://doi.org/10.1016/j.frl.2026.110362.",
        ]
        lines = base[:limit]
    else:
        ...
        base_urls = [
            ...
            f"https://www.cambridge.org/core/search?q={q}",
            f"https://academic.oup.com/search-results?q={q}",
            f"https://journals.sagepub.com/action/doSearch?AllField={q}",
            f"https://www.mdpi.com/search?q={q}",
            f"https://www.frontiersin.org/search?query={q}",
            f"https://doaj.org/search/articles?source={q}",
            f"https://www.jstor.org/action/doBasicSearch?Query={q}",
            f"https://www.scopus.com/results/results.uri?sort=plf-f&src=s&st1={q}",
            f"https://pubmed.ncbi.nlm.nih.gov/?term={q}",
            f"https://www.nature.com/search?q={q}",
            f"https://ieeexplore.ieee.org/search/searchresult.jsp?queryText={q}",
            f"https://dl.acm.org/action/doSearch?AllField={q}",
            f"https://arxiv.org/search/?query={q}&searchtype=all",
            f"https://www.ebsco.com/search?search={q}",
        ]
        lines = [
...
def _ensure_min_bibliography(bib_text: str, topic: str, subject: str, *, min_sources: int = 25, limit: int = 35) -> str:
...
async def generate_chapter_titles(
    ...
    user = (
        ...
        f"Для реферата каждая глава — 3–4 подглавы; для остальных работ — 2–4 подглавы. "
        f"Язык: русский. "
...
def _default_chapter_titles(doc_type: str, topic: str, num_chapters: int) -> list[dict]:
    ...
    if doc_type == "referat":
        defaults = [
            {
                ...
                "subs": [
                    ...
                    "1.3 Методологические подходы и теоретические концепции",
                    "1.4 Нормативно-правовая база и научные школы",
                ],
            },
            {
                ...
                "subs": [
                    ...
                    "2.4 Эмпирические данные и результаты исследований",
                ],
            },
            {
        ...
        return defaults[:max(1, num_chapters)]
...
def tokens_for_chars(chars: int) -> int:
    ...
    """
    return max(2200, min(16000, int(chars / 2.2 * 1.8)))
...
def strict_prompt(task: str, chars: int, writing_style: str = "classic",
    ...
    if doc_type == "esse":
        ...
        return base + (
            "\nКаждая подглава — НЕ МЕНЕЕ 5 И НЕ БОЛЕЕ 8 абзацев. "
            "Каждый абзац — 8–12 предложений, без маркированных списков. "
            "Разрешены только полные сноски в формате ГОСТ: [1, с. 45] или [3, с. 78]. "
            "Используй 3–5 ссылок на источники в каждой подглаве — номера источников 1, 2, 3..."
        )
...
def build_prompts(
    ...
    for i, j, ch_title, sub_title in all_subs:
        ...
        prompts[key] = strict_prompt(
            ...
            f"Количество абзацев — НЕ МЕНЕЕ 5 И НЕ БОЛЕЕ 8 абзацев, разделённых пустой строкой "
            f"(двойным переводом строки между ними). НЕ объединяй текст в один абзац. "
            f"Каждый абзац должен содержать 8–12 предложений и законченный микро-вывод.\n"
...
def generate_structure(
    ...
    for i, ch in enumerate(chapter_titles, start=1):
        ...
        for j, sub_title in enumerate(subs, start=1):
            ...
            sub_text = _ensure_paragraph_breaks(sub_text, min_paragraphs=5)
...
async def generate_text_blocks(
    ...
    verified_bib = _ensure_min_bibliography(verified_bib, topic, subject, min_sources=25, limit=source_limit)
    if verified_bib:
        source = (source + "\n\n" if source else "") + (
    ...
    style_sys = (
        "Ты — эксперт-аналитик с 20-летним стажем в академической среде. "
        "Твои тексты отличаются глубиной проработки, многоуровневым анализом, "
        "фактической насыщенностью, критичностью и строгой академической логикой.\n"
        "1. Каждый тезис раскрывай через 3–5 аргументов.\n"
        "2. Анализируй тему на теоретическом, эмпирическом и практическом уровнях.\n"
        "3. Используй конкретные цифры, даты, имена, события и проверяемые факты.\n"
        "4. Добавляй сравнительный анализ разных точек зрения и контраргументы.\n"
        "5. Каждый абзац должен логически вытекать из предыдущего.\n"
        "6. Соблюдай академическую строгость: ссылки [1, с. 45], терминология, отсутствие воды и повторов.\n"
        f"ОБЯЗАТЕЛЬНО: текст должен соответствовать дисциплине «{subject}». "
        f"Раскрывай тему «{topic}» строго через предмет, методы и терминологию дисциплины «{subject}».\n"
        "НЕ используй markdown-разметку и маркеры (никаких #, *, **, ##, ---). "
        "НЕ допускай опечаток, случайных двойных пробелов и намеренных ошибок. "
        "Не используй фразы-маркеры ИИ: «конечно, вот», «как ИИ», «таким образом», "
        "«следует отметить», «в заключение следует отметить». "
        "Не повторяй одно слово несколько раз в одном предложении. "
        "Не начинай два абзаца подряд одним и тем же словом. "
        "Каждая подглава — НЕ МЕНЕЕ 5 И НЕ БОЛЕЕ 8 абзацев. "
        "Каждый абзац — 8–12 предложений с развёрнутой аргументацией. "
        "3–5 ссылок на источники [N, с. X] в каждой подглаве. "
        "Конкретные цифры, даты и факты должны присутствовать в каждом смысловом блоке. "
        "Используй фамилии исследователей и их вклад, не менее 5 исследователей на главу, если это уместно. "
    )
    style_sys += "\n" + PAGE_CALC_PROMPT
    style_sys += numeric_context
    golden_full = load_golden_example(doc_type)
    if golden_full:
        style_sys += (
            "\n\n🌟 ЭТАЛОННЫЙ ОБРАЗЕЦ (генерируй по этой структуре и уровню качества, НЕ копируя текст дословно):\n"
            f"{golden_full}\n"
            "ВАЖНО: сохрани структуру, академический стиль, формат ссылок [N, с. XX], "
            "глубину анализа и 5+ абзацев в подглаве.\n"
        )
    if verified_bib:
        style_sys += (
    ...
    for key, prompt in prompts.items():
        ...
        if key != "literature":
            ...
            if key.startswith("ch"):
                text = _ensure_paragraph_breaks(text, min_paragraphs=5)
        else:
            ...
            if _count_sources(text) < 10:
                ...
                text = _ensure_min_bibliography(text, topic, subject, min_sources=25, limit=source_limit)
    ...
    if _count_sources(parts.get("literature", "")) < 10:
        parts["literature"] = _normalize_bibliography(_filter_bibliography_scientific_lines(_combine_bibliographies(
            ...
            verified_bib,
            limit=max(25, source_limit),
        )))
        if _count_sources(parts.get("literature", "")) < 10:
            print(f"[LIT] ⚠️ Итоговый список содержит {_count_sources(parts.get('literature', ''))}; добираю резервом.")
            parts["literature"] = _ensure_min_bibliography(parts.get("literature", ""), topic, subject, min_sources=25, limit=max(25, source_limit))
    ...
    parts["literature"] = _ensure_min_bibliography(parts.get("literature", ""), topic, subject, min_sources=25, limit=max(25, source_limit))
    parts["literature"] = _dedupe_bibliography(parts.get("literature", ""))
    parts["literature"] = _ensure_min_bibliography(parts.get("literature", ""), topic, subject, min_sources=25, limit=max(25, source_limit))
...
def _structure_conclusion_by_chapters(conc_text: str, parts: dict) -> str:
    """Строит заключение связным текстом, без перечислений и копий глав."""
    if not conc_text:
        conc_text = ""
    ...
    # Убираем старые перечислительные фразы, если они пришли от модели.
    conc_text = re.sub(r"(?is)В\s+перв[оа]й\s+главе\s+были\s+систематизированы[^.]*\.", "", conc_text)
    conc_text = re.sub(r"(?is)Во\s+втор[оа]й\s+главе\s+были\s+систематизированы[^.]*\.", "", conc_text)
    conc_text = re.sub(r"(?is)В\s+треть[её]й\s+главе\s+были\s+систематизированы[^.]*\.", "", conc_text)
    ...
    ordinal = {1: "первой", 2: "второй", 3: "третьей", 4: "четвёртой", 5: "пятой"}
    result_parts = [
        "Проведённое исследование позволило сформулировать несколько взаимосвязанных выводов, раскрывающих содержание заявленной темы и её значение для выбранной дисциплины."
    ]

    for i, idx in enumerate(chapter_idxs, start=1):
        summary = _chapter_summary_from_parts(parts, idx).strip().rstrip(".")
        ord_word = ordinal.get(i, f"{i}-й")
        prep = "Во" if str(ord_word).startswith("в") else "В"
        result_parts.append(
            f"{prep} {ord_word} главе {summary}. Этот раздел не просто описывает отдельные факты, "
            f"а показывает их внутреннюю связь: от исходных понятий и причин к оценке последствий, "
            f"что позволяет рассматривать проблему как целостный научный объект."
        )

    # Берём один чистый общий вывод модели, если он не повторяет главы.
    extra = ""
    for sent in _split_sentences_safe(conc_text):
        ...
        if any(bad in low for bad in (
            "в первой главе", "во второй главе", "в третьей главе",
            "были систематизированы", "проведенный анализ", "проведённый анализ",
    ...
    result_parts.append(
        "В итоге цель работы можно считать достигнутой, поскольку поставленные задачи раскрыты через теоретический, аналитический и практический уровни рассмотрения. Полученные выводы имеют значение для дальнейшего изучения проблемы, уточнения исследовательских подходов и более точной оценки процессов, связанных с заявленной темой."
    )
...
def _strip_duplicate_heading_prefix(text: str, heading: str) -> str:
    ...
    # Дополнительный проход: удаляем «1.1 Заголовок.» если он повторился
    # первой строкой с точкой/двоеточием.
    if result and heading:
        h_plain = re.sub(r"^\d+(?:\.\d+)*\.?\s+", "", heading).strip()
        result = re.sub(
            rf"^\s*\d{{1,3}}(?:\.\d{{1,3}})*\.?\s*{re.escape(h_plain)}[.:]?\s*",
...
def add_gost_image(doc: Document, image: dict, number: int, gost: dict) -> None:
    ...
    user-fix: источник/URL должен находиться на том же листе, что и рисунок,
    а сам рисунок должен оставаться в основной части рядом с текстом, а не
    выноситься на отдельный лист. Поэтому изображение делается компактным,
    а рисунок, подпись и строка «Источник: …» оформляются как неразрывная
    группа без принудительного разрыва страницы перед ней.
    """
    ...
    # Выравнивание рисунка: по центру (по умолчанию), либо «по бокам»
    # (left/right) — чтобы фото можно было поставить сбоку «в тему», как
    # просил пользователь. Берётся из gost["image_align"].
    align_map = {
    ...
    try:
        ...
        # Рисунок остаётся в основной части рядом с текстом. Не ставим
        # page_break_before: отдельный лист для рисунка пользователю не нужен.
        ...
        # Ограничиваем ширину так, чтобы картинка + подпись гарантированно
        # помещались на лист (ширина ≤ полезной ширины страницы; для боковой
        # вставки — не более ~55 % ширины, чтобы рядом помещался текст/воздух).
        ...
        # Дополнительно ограничиваем ВЫСОТУ, чтобы рисунок с подписью не занял
        # больше одной страницы: масштабируем по реальным пропорциям картинки.
        try:
            from docx.shared import Emu
            with PILImage.open(io.BytesIO(img_bytes)) as _im:
                _w, _h = _im.size
            ratio = (_h / _w) if _w else 1.0
            # Оставляем большой запас под основной текст, подпись и URL.
            # 6.0 см — высота, при которой рисунок с подписью точно влезает
            # на ту же страницу, что и основной текст, а не уходит на отдельный лист.
        ...
        # ГОСТ: подпись под иллюстрацией вида «Рисунок 1 — Название».
        # По требованию пользователя подпись делаем меньше основного текста.
        ...
        if source:
            ...
            # Источник/URL должен быть сразу под рисунком и маленьким.
...
async def generate_and_send(
    ...
    async with GEN_SEMAPHORE:
        ...
        if doc_type in ("esse", "doklad", "article"):
            ...
            num_chapters = 3  # идеальный реферат: 3 главы
        else:
            num_chapters = 2  # контрольная, свой

...
async def fetch_verified_sources(
    ...
    limit = max(MIN_REAL_SOURCES, min(limit, MAX_WEB_SOURCES, BIB_SOURCE_TARGET))
    # FIX #1: 60 строк на запрос было мало — после фильтрации релевантности
    # каталоги часто давали < MIN_REAL_SOURCES и включался LLM-добор с
    # галлюцинациями. 150 строк на каждый query-вариант дают достаточный
    # пул реальных записей, чтобы каталоги закрывали потребность сами.
    fetch_rows = max(150, limit * 10)
    records: list[dict] = []
...
def target_chars(pages: int, gost: dict = None, num_chapters: Optional[int] = None) -> int:
    """
...
def _extract_real_toc_pages_from_docx(docx_path: str, blocks: list[tuple], work_dir: str, pdf_path: Optional[str] = None) -> dict[str, int]:
...
async def libreoffice_docx_to_pdf_async(in_path: str, out_dir: str) -> Optional[str]:
...
async def count_docx_pages_async(docx_path: str, work_dir: str) -> Optional[int]:

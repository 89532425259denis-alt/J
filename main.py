...
async def check_topic_discipline_match(
    model_key: str,
    topic: str,
    subject: str,
) -> tuple[bool, str]:
    """
    Проверяет, насколько тема «стыкуется» с дисциплиной.
    Если связь неочевидна, возвращает (False, предложение_как_связать).
    """
    system = (
        "Ты — эксперт по академическому планированию. Твоя задача — проверить, "
        "соответствует ли тема работы заявленной учебной дисциплине. "
        "Отвечай СТРОГО в формате JSON: "
        '{"match": true|false, "suggestion": "короткий совет, как связать тему с предметом, если match=false"}.'
    )
    user = f"Тема: «{topic}». Дисциплина: «{subject}». Совместимы ли они для научной работы?"
    ...
    try:
        ...
        if m:
            ...
            return bool(data.get("match", True)), str(data.get("suggestion", "")).strip()
    except Exception as e:
        print(f"[MATCH_CHECK] Error: {e}")
...
def build_prompts(
    ...
    if doc_type == "doklad":
        return {
            "intro": strict_prompt(
                f"Введение доклада «{topic}», дисциплина «{subject}».{ctx}"
                f"Актуальность темы, цель и задачи доклада.",
                int(total * 0.14),
                writing_style, doc_type,
            ),
            "part1": strict_prompt(
                f"Раздел 1 доклада «{topic}»: ключевые понятия и определения. Раскрой теоретическую базу.",
                int(total * 0.28),
                writing_style, doc_type,
            ),
            "part2": strict_prompt(
                f"Раздел 2 доклада «{topic}»: основной анализ проблемы и практические примеры.{ctx}",
                int(total * 0.38),
                writing_style, doc_type,
            ),
            "literature": (
                f"Составь список из 8–12 РЕАЛЬНЫХ источников строго по теме «{topic}», дисциплина «{subject}». "
                f"Формат ГОСТ Р 7.0.5-2008: 1. Автор А.А. Название. — Год. — URL: ссылка. "
                f"Только список, без заголовков и пояснений."
            ),
        }
    ...
    # Для всех остальных типов документов (реферат, курсовая и т.д.)
    prompts = {}
    
    # 1. Введение
    prompts["intro"] = strict_prompt(
        f"Напиши ВВЕДЕНИЕ для работы на тему «{topic}», дисциплина «{subject}».{ctx}"
        f"Обязательно сформулируй: актуальность темы, степень научной разработанности, "
        f"объект и предмет исследования, цель и задачи работы, методологическую основу.",
        int(total * 0.15), writing_style, doc_type
    )
    
    # 2. Динамическая генерация для глав и подглав
    body_chars = int(total * 0.70)
    total_subs = sum(len(ch.get("subs", [])) for ch in chapter_titles)
    chars_per_sub = body_chars // max(1, total_subs)
    
    sub_index = 1
    for idx, ch in enumerate(chapter_titles, start=1):
        ch_title = ch.get("title", f"{idx} Раздел")
        subs = ch.get("subs", [])
        
        for j, sub_title in enumerate(subs, start=1):
            prompts[f"ch{idx}_s{j}"] = strict_prompt(
                f"Напиши подраздел «{sub_title}» в рамках раздела «{ch_title}». "
                f"Тема всей работы: «{topic}», дисциплина: «{subject}».{ctx}"
                f"Текст должен строго соответствовать названию подраздела, содержать глубокий анализ "
                f"и ссылки на источники в формате [N, с. X].",
                chars_per_sub, writing_style, doc_type
            )
            
    # 3. Список литературы
    prompts["literature"] = (
        f"Составь список научных источников (8–15 шт) для темы «{topic}» ({subject}) по ГОСТ 7.1-2003. "
        f"Используй реальные монографии, статьи ВАК и нормативные акты. "
        f"Только список, без лишних слов."
    )
...
async def generate_text_blocks(
    ...
    # ⭐ Генерируем ключевое определение-связку для единства работы
    concept_def = await get_key_concept(topic, subject, model_key)
    if concept_def:
        source = (source + "\n\n" if source else "") + f"КЛЮЧЕВОЙ КОНЦЕПТ РАБОТЫ: {concept_def}"
    ...
    style_sys = (
        ...
        f"Раскрой тему «{topic}» строго через предмет, методы и терминологию "
        f"дисциплины «{subject}». Если связь между темой и предметом неочевидна, "
        f"рассматривай тему ПРИЗМОЙ дисциплины. "
        f"Рекомендация по связи: {prism_suggestion if prism_suggestion else 'Используй стандартные академические методы связи.'} "
        f"Не подменяй дисциплину смежной. "
    )
    if concept_def:
        style_sys += f"\nФУНДАМЕНТАЛЬНОЕ ОПРЕДЕЛЕНИЕ: {concept_def}. Используй этот подход во всех разделах."
    ...
    # ⭐ Усиление для сложных/неочевидных связей
    if prism_suggestion:
        style_sys += (
            f"\n\n⚠️ ВАЖНОЕ УТОЧНЕНИЕ ПО СВЯЗИ ТЕМЫ И ПРЕДМЕТА:\n"
            f"Связь между темой «{topic}» и дисциплиной «{subject}» может быть неочевидной. "
            f"Твоя задача — искусственно, но научно обоснованно связать их. "
            f"Инструкция по связи: {prism_suggestion}. "
            "Используй этот подход во всех главах работы."
        )
...
def count_pdf_pages(pdf_path: str) -> Optional[int]:
    ...
    if fitz is not None:
        ...
        print("[PyMuPDF] Не доступен (fitz не установлен)")
...
async def _start_http_server() -> None:
    ...
    try:
        ...
        print(f"[HTTP] Ошибка запуска healthcheck: {e}")
...
async def h_pagepos(cb: CallbackQuery, state: FSMContext) -> None:
    ...
    # ── Проверка на «стыкуемость» темы и дисциплины ──
    # Используем бесплатную модель для быстрой проверки
    match_ok, suggestion = await check_topic_discipline_match(FREE_MODEL_KEY, topic, subject)
    if not match_ok:
        # Если тема и дисциплина плохо стыкуются, предупреждаем пользователя
        # Но не блокируем полностью, а предлагаем уточнить или продолжить
        await cb.message.answer(
            f"⚠️ <b>Внимание: неочевидная связь!</b>\n\n"
            f"Тема <i>«{topic}»</i> может плохо соответствовать дисциплине <i>«{subject}»</i>.\n"
            f"💡 <b>Совет:</b> {suggestion}\n\n"
            "Вы можете изменить тему/предмет с помощью /start или продолжить генерацию.",
            parse_mode="HTML",
        )
        # Сохраняем совет в данных состояния, чтобы использовать его в промптах (prism_suggestion)
        await state.update_data(prism_suggestion=suggestion)
    else:
        await state.update_data(prism_suggestion="")
...
async def check_topic_subject_compatibility(topic: str, subject: str, model_key: str) -> tuple[bool, str]:
    """Проверяет, совместимы ли тема и дисциплина.
    Возвращает (совместимы_ли, причина_или_совет).
    """
    system = (
        "Ты — эксперт по академическому планированию. Твоя задача — определить, "
        "можно ли написать научную работу по заданной теме в рамках указанной дисциплины. "
        "Отвечай СТРОГО в формате JSON: {\"compatible\": true|false, \"reason\": \"почему не подходит или как связать\"}. "
        "Если связь неочевидна, но возможна (например, 'Баба-Яга' + 'География' через анализ ландшафтов), "
        "ставь compatible=true, но в reason напиши, как именно их связать."
    )
    user = f"Тема: «{topic}». Дисциплина: «{subject}». Совместимы ли они для научной работы?"
    ...
    try:
        ...
        if m:
            data = json.loads(m.group(0))
            return bool(data.get("compatible", True)), str(data.get("reason", ""))
    except Exception as e:
        print(f"[COMPAT] Error: {e}")
    return True, ""


async def get_key_concept(topic: str, subject: str, model_key: str) -> str:
    """Определяет ключевой концепт (связку) между темой и дисциплиной для единства работы."""
    system = (
        "Ты — академический архитектор. Твоя задача — сформулировать одно точное, "
        "научное определение (ключевой концепт), которое связывает тему и дисциплину. "
        "Это определение будет использоваться как фундамент для всей работы. "
        "Отвечай только текстом определения (1-2 предложения). "
        "Пример: Тема 'Баба-Яга', Дисциплина 'География' -> 'Данное исследование рассматривает образ Бабы-Яги как отражение традиционного восприятия таёжных ландшафтов и природных зон северных регионов России'."
    )
    user = f"Тема: «{topic}». Дисциплина: «{subject}». Сформулируй ключевое научное определение-связку."
...
async def generate_and_send(
    ...
    """
    # Предупреждение о перегрузке сервера
    if GEN_SEMAPHORE.locked():
        await event.answer(
            "⚠️ <b>Сервер сейчас очень загружен</b>\n\n"
            "Много пользователей создают работы одновременно. Вы в очереди, "
            "генерация начнется автоматически, как только освободится слот. "
            "Пожалуйста, не нажимайте /start повторно.",
            parse_mode="HTML",
        )
    ...
    async with GEN_SEMAPHORE:
        ...
        # ⭐ Проверка на совместимость темы и дисциплины
        is_compat, reason = await check_topic_subject_compatibility(topic, subject, model_key)
        if not is_compat:
            await event.answer(
                f"⚠️ <b>Неочевидная связь темы и дисциплины</b>\n\n"
                f"Тема: «{topic}»\n"
                f"Дисциплина: «{subject}»\n\n"
                f"🤖 ИИ считает, что они плохо сочетаются: {reason}\n\n"
                "Пожалуйста, уточните тему или выберите другую дисциплину. "
                "Начните заново — /start",
                parse_mode="HTML",
            )
            await state.clear()
...
async def main() -> None:
    ...
    print(f"  PyMuPDF     : {'✅' if fitz is not None else '❌ не установлен (pip install PyMuPDF)'}")
    ...
    while True:
        try:
            ...
            print("[BOT] Поллинг остановлен")
            break
        except Exception as e:
            print(f"[BOT] Ошибка поллинга: {e}")
            import traceback
            traceback.print_exc()
            print(f"[BOT] Перезапуск через {retry_delay} сек...")

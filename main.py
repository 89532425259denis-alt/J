...
def _chapter_summary_from_parts(parts: dict, idx: int) -> str:
    """Достаёт из главы короткий связный вывод для заключения.

    Раньше функция собирала 3–4 «ключевых слова» и получались фразы вроде
    «раскрыты основные, направления, торговой». Теперь берём первое
    полноценное предложение из реального текста главы и превращаем его в
    придаточную часть после «установлено, что ...». Это даёт грамматически
    нормальный итог без выдумывания фактов.
    """
    texts = []
    ...
    src = " ".join(texts)
    if not src.strip():
        return "рассмотрены основные положения темы"

    src = _strip_markdown_markers(src)
    src = re.sub(r"\[[^\]]+\]", "", src)
    src = re.sub(r"\s+", " ", src).strip()

    for sent in _split_sentences_safe(src):
        sent = sent.strip(" .;:\n\t")
        if not sent:
            continue
        # Пропускаем строки-заголовки вида «1.1 Название».
        if re.match(r"^\d+(?:\.\d+)*\.?\s+[^.]{3,120}$", sent):
            continue
        if len(sent) < 60:
            continue
        if len(sent) > 260:
            sent = sent[:260].rsplit(" ", 1)[0].rstrip(" ,;:")
...
def _structure_conclusion_by_chapters(conc_text: str, parts: dict) -> str:
    ...
    for pos, idx in enumerate(chapter_idxs, start=1):
        ...
        if not summary:
            summary = "рассмотрены основные положения темы"
        ord_word = ordinal.get(pos, f"{pos}-й")
        prep = "Во" if str(ord_word).startswith("в") else "В"
        paragraphs.append(f"{prep} {ord_word} главе установлено, что {summary}.")
...
def build_docx_bytes(
    ...
    def _insert_image_inside_subtext(doc, sub_text, sub_title, image, number, gost):
        ...
        img_side = str(gost.get("image_align", "center")).lower()
        if img_side in ("left", "right") and part2:
            # Первый следующий абзац идёт сбоку от рисунка; оставшийся текст — ниже.
            after_paras = [p.strip() for p in re.split(r'\n\s*\n', part2) if p.strip()]
            side_text = after_paras[0] if after_paras else ""
            rest_text = "\n\n".join(after_paras[1:])
            add_side_image_with_text(doc, image, number, side_text, gost, side=img_side)
            if rest_text:
                add_paragraphs_from_text(doc, rest_text, gost, skip_first_heading=sub_title)
        else:

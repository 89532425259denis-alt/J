...
async def _get_fresh_openverse_token() -> str:
    ...
    # FIX #2: сохраняем ручной токен в кэш с большим TTL (1 год), чтобы
    # поведение было единообразным с авто-режимом и не было расхождений.
    manual = cfg("OPENVERSE_TOKEN", "").strip()
    if manual:
        _OPENVERSE_TOKEN_CACHE = manual
        _OPENVERSE_TOKEN_EXPIRY = time.time() + 86400 * 365  # 1 год
        return manual
...
# ═══════════════════════════════════════════════════════════════
#  FIX #5: КОНТЕКСТ ВРЕМЕННЫХ ФАЙЛОВ (гарантированная очистка)
#  Раньше временные .docx удалялись только на успешном пути. Если генерация
#  падала с исключением раньше блока очистки — файлы копились в _out/ и при
#  сотнях ошибок забивали диск. TempFileContext удаляет всё зарегистрированное
#  в __exit__ — независимо от того, штатно завершилась генерация или упала.
# ═══════════════════════════════════════════════════════════════

class TempFileContext:
    """Регистрирует временные файлы/папки и гарантированно удаляет их на выходе."""

    def __init__(self, prefix: str = "gost_", work_dir: Optional[str] = None) -> None:
        self.prefix = prefix
    ...
    def create(self, suffix: str = ".docx") -> str:
        """Создаёт уникальный временный файл и возвращает путь к нему."""
        fd, path = tempfile.mkstemp(suffix=suffix, prefix=self.prefix, dir=self.work_dir)
        os.close(fd)
        self.files.append(path)
        return path

    def track(self, path: str) -> str:
        """Регистрирует уже существующий путь на удаление и возвращает его."""
        if path:
            self.files.append(path)
        return path

    def track_dir(self, path: str) -> str:
        """Регистрирует директорию на рекурсивное удаление."""
        if path:
...
_SPINNER_FRAMES = ["⠋", "⠙", "⠹", "⠸", "⠼", "⠴", "⠦", "⠧", "⠇", "⠏"]
# FIX #9 (из задания): неиспользуемые _BAR_FULL/_BAR_HALF/_BAR_EMPTY удалены —
# прогресс-бар рисует _progress_bar() своими символами.
...
# ── FIX #1: атомарный захват/освобождение слота генерации ──────────────
# Раньше счётчик менялся неатомарно (get → +1 в разных await-точках), из-за
# чего два почти одновременных запроса одного пользователя могли «пробить»
# лимит или некорректно сбросить счётчик. Теперь на каждого пользователя —
# свой asyncio.Lock, и вся проверка+инкремент выполняются под ним атомарно.
...
async def _acquire_user_slot(user_id: int) -> bool:
    """Атомарно занимает слот генерации. True — слот получен, False — лимит.

    При MAX_PARALLEL_PER_USER == 0 лимит отключён (слот всегда выдаётся).
    """
    async with _get_user_lock(user_id):
        current = _active_gen_per_user.get(user_id, 0)
        if MAX_PARALLEL_PER_USER and current >= MAX_PARALLEL_PER_USER:
            return False
        _active_gen_per_user[user_id] = current + 1
        return True


async def _release_user_slot(user_id: int) -> None:
    """Атомарно освобождает слот генерации (никогда не уходит ниже нуля)."""
    async with _get_user_lock(user_id):
        _active_gen_per_user[user_id] = max(0, _active_gen_per_user.get(user_id, 1) - 1)


VIP_USERS = {int(x) for x in cfg("VIP_USERS", "").replace(" ", "").split(",") if x.isdigit()}

# Чат администратора для мгновенных алертов о критических сбоях (см. error_logger).
ADMIN_CHAT_ID = cfg("ADMIN_CHAT_ID", "")

# ── Наблюдаемость: журнал ошибок + телеметрия (мягкое подключение) ──
# Модули error_logger и metrics устойчивы к отсутствию и никогда не бросают
# исключений наружу, поэтому их безопасно подключать «best effort».
try:
    from error_logger import log_error, configure_error_alerts, log_api_call
except Exception:  # pragma: no cover
    def log_error(*_a, **_k):  # type: ignore
        pass
    def configure_error_alerts(*_a, **_k):  # type: ignore
        pass
    def log_api_call(*_a, **_k):  # type: ignore
        pass
try:
    ...
    _METRICS = None
    class GenerationMetrics:  # type: ignore
        def __init__(self, *_a, **_k):
            pass


def _log_metric(**fields) -> None:
    """Записывает событие телеметрии, если модуль metrics доступен."""
    if _METRICS is None:
        return
    try:
        _METRICS.log(GenerationMetrics(**fields))
    except Exception:
        pass
...
DEFAULT_GOST_CONFIGS: dict = {
    "_base": {
        ...
        "left_margin_mm":         30,
        "right_margin_mm":        15,   # ГОСТ 7.32-2017 п.6.1.1: правое поле 15 мм
        "top_margin_mm":          20,
        "bottom_margin_mm":       20,
        "alignment":              "justify",
...
# Сохраняем ключ прямо в словаре модели — надёжнее, чем каждый раз искать
# модель перебором по (base_url, model). Перебор ломается, когда две модели
# делят один base_url и model (например, обе через OpenRouter): тогда счётчик
# перегрузок и fallback могли указывать не на ту модель.
for _k, _v in AI_MODELS.items():
    _v["_key"] = _k
...
def _strip_markdown_markers(text: str) -> str:
    ...
    text = re.sub(r"\*{1,3}([^*\n]+?)\*{1,3}", r"\1", text)
    # Убираем оставшиеся markdown-# (в начале строки и как отдельный маркер),
    # НО сохраняем '#', приклеенный к букве/цифре — это часть слова (C#, F#, №1#).
    # FIX #8 (из задания): раньше text.replace("#","") ломал «C#» → «C».
    text = re.sub(r'(?m)^\s*#+\s*', '', text)
    text = re.sub(r'(?<![\wА-Яа-яЁё])#', '', text)  # # не приклеенный к слову (сохраняем C#, F#)
...
async def call_openai_compat(
    ...
    temperature: для генерации текста глав уместно 0.7 (живость), но для
    СТРУКТУРНЫХ/ФАКТИЧЕСКИХ задач (валидный JSON-план, библиография, парсинг
    ГОСТ, проверка релевантности) нужно 0.0–0.3 — это резко снижает
    невалидный JSON и «выдуманные» источники, то есть повышает шанс
    получить корректный результат С ПЕРВОГО РАЗА.
    """
    ...
    # FIX #10 (из задания): фиксируем длительность и результат вызова API —
    # log_api_call теперь реально пишется (раньше функция не вызывалась).
    _api_t0 = time.monotonic()
    _api_model = info.get("model", "")
    try:
        async with aiohttp.ClientSession() as sess:
            async with sess.post(
                ...
                if r.status == 200:
                    ...
                    log_api_call(_api_model, True,
                                 int((time.monotonic() - _api_t0) * 1000))
                    return content
                log_api_call(_api_model, False,
                             int((time.monotonic() - _api_t0) * 1000),
        ...
        log_api_call(_api_model, False, int((time.monotonic() - _api_t0) * 1000),
                     error_msg="timeout")
...
async def chat_with_model(info: dict, messages: list[dict], max_tokens: int = 4096,
                          temperature: float = 0.7) -> str:
    """Вызывает модель. При перегрузке возвращает пустую строку."""
    # Определяем ключ модели для проверки перегрузки
    model_key = info.get("_key") or next((k for k, v in AI_MODELS.items() if v.get("base_url") == info.get("base_url") and v.get("model") == info.get("model")), "unknown")
    ...
    raw = await call_openai_compat(info, messages, max_tokens=max_tokens, temperature=temperature)
...
async def chat_with_fallback(
    ...
    temperature прокидывается насквозь: структурные/фактические вызовы
    (JSON-план, библиография, релевантность) должны идти с низкой
    температурой ради стабильного корректного результата с первого раза.
    """
    best_text = ""
    best_model = primary
    for k in fallback_chain(primary):
        info = AI_MODELS[k]
        text = await chat_with_model(info, messages, max_tokens=max_tokens, temperature=temperature)
...
def _filter_bibliography_scientific_lines(bib_text: str, *, min_keep: int = MIN_REAL_SOURCES) -> str:
    ...
    # FIX #4 (из задания): если после жёсткой фильтрации осталось меньше
    # min_keep источников — вторым проходом добираем проверяемо-научные строки
    # БЕЗ обязательного DOI/URL (реальные печатные статьи/книги), пропуская
    # ненаучные (ГОСТы, законы, методички). Так библиография дотягивается до
    # требуемого минимума, а не обрывается на 3 позициях.
    if min_keep and len(lines) < min_keep:
        for line in norm.split("\n"):
...
def _format_bibliography_item_gost(item: str) -> str:
    ...
    # FIX #5 (из задания): «голый» DOI без префикса doi: и не внутри URL —
    # тоже приводим к рабочей ссылке, иначе он терялся как обычный текст.
    if "doi.org/" not in item.lower() and not re.search(r"doi\s*[:：]", item, re.I):
        _bare_doi = re.search(r"(?<![\w./])(10\.\d{4,9}/[^\s,;)\]]+)", item)
        if _bare_doi:
            _doi = _bare_doi.group(1).rstrip(" .;,)")
            item = item[:_bare_doi.start()].rstrip() + " URL: https://doi.org/" + _doi + item[_bare_doi.end():]
...
async def generate_chapter_titles(
    ...
    # FIX (защита от «битого» LLM): до 3 попыток получить ВАЛИДНЫЙ JSON-массив.
    # Если модель прислала мусор/обрезанный JSON — повторяем запрос, добавляя
    # в промпт конкретную причину отказа, и только исчерпав попытки, уходим в
    # детерминированный фоллбэк. Низкая температура (0.2) — план это структура.
    error_hint = ""
    for attempt in range(3):
        ...
        if error_hint:
            msgs = messages + [{
                "role": "user",
                "content": (f"Предыдущий ответ отклонён: {error_hint}. "
                            "Верни СТРОГО валидный JSON-массив объектов "
                            '{"title": "...", "subs": ["...", "..."]} без пояснений и markdown.'),
            }]
        raw, _ = await chat_with_fallback(model_key, msgs, max_tokens=1500, temperature=0.2)
        raw = (raw or "").strip()
        ...
        try:
            ...
            error_hint = f"ответ не парсится как JSON ({str(e)[:60]})"
            print(f"[TITLES] Попытка {attempt+1}: {error_hint}")
            continue
        if not isinstance(result, list) or not result:
            error_hint = "JSON не является непустым массивом"
            print(f"[TITLES] Попытка {attempt+1}: {error_hint}")
            continue
        if not all(isinstance(r, dict) and str(r.get("title", "")).strip() for r in result):
            error_hint = "не у всех элементов есть непустой title"
            print(f"[TITLES] Попытка {attempt+1}: {error_hint}")
            continue
        return result

    # Фоллбэк: базовые названия (детерминированные, связаны с темой)
    print(f"[TITLES] Все попытки исчерпаны ({error_hint}). Использую фоллбэк.")
    ...
    try:
        raw, _ = await chat_with_fallback(
            ...
            [{"role": "system", "content": system},
             {"role": "user", "content": user}],
            max_tokens=200,
            temperature=0.0,  # оценка «да/нет» — детерминированно, без фантазии
        )
        m = re.search(r"\{.*\}", raw, flags=re.S)
        if m:
...
def strict_prompt(task: str, chars: int, writing_style: str = "classic",
    ...
    base = (
        ...
        f"Стиль: {style_instr}\n"
        "ЗАПРЕЩЕНО: markdown-разметка, символы #, *, **, ## в тексте абзацев. "
        "ИСКЛЮЧЕНИЕ — ОФОРМЛЕНИЕ ПО ГОСТ 7.32-2017: "
        "(а) если данные ДЕЙСТВИТЕЛЬНО табличные (сравнение, классификация, числовые показатели, "
        "этапы) — оформи их ТАБЛИЦЕЙ: отдельной строкой «Таблица — краткое название», затем "
        "строки вида | Колонка 1 | Колонка 2 | и обязательная строка-разделитель | --- | --- |. "
        "Номер таблицы НЕ ставь — его проставит система. "
        "(б) каждую формулу выноси ОТДЕЛЬНОЙ строкой в виде $ формула $ (пустая строка до и после); "
        "номер формулы проставит система, в тексте ссылайся «в формуле (1)». "
        "Не дублируй содержимое таблиц и формул обычным текстом. "
...
def build_prompts(
    ...
    prompts["literature"] = (
        ...
        f"ВАЖНО: автор указывается как «Фамилия И.О.» (фамилия + инициалы), "
        f"НИКОГДА только по имени («Андрей», «Мария» — запрещено).\n\n"
...
# FIX #7: кэш определений с TTL. Раньше кэш {topic|subject -> definition}
# никогда не инвалидировался: после смены темы (кнопка «Уточнить тему») или
# переработки введения пользователь получал определение из старой версии.
# Теперь храним (definition, timestamp) и учитываем содержимое введения в
# ключе, а по истечении TTL запись протухает и пересчитывается.
_CONCEPT_CACHE: dict[str, tuple[str, float]] = {}
CONCEPT_CACHE_TTL = 3600  # 1 час
"""Кэш определений ключевых понятий: {topic|subject|hash(intro) -> (definition, ts)}"""


def invalidate_concept_cache(topic: str = "", subject: str = "") -> None:
    """Сбрасывает кэш определений: полностью или по конкретной теме/дисциплине.

    Вызывается при смене темы, чтобы гарантированно не отдать старое определение.
    """
    if not topic and not subject:
        _CONCEPT_CACHE.clear()
        return
    prefix = f"{topic}|{subject}|"
    for key in [k for k in _CONCEPT_CACHE if k.startswith(prefix)]:
...
def get_key_concept(topic: str, subject: str, intro_text: str) -> str:
    ...
    """
    # FIX #7: ключ учитывает содержимое введения (его хеш) — при изменении
    # введения/темы ключ меняется и старое определение не возвращается.
    intro_hash = hash((intro_text or "")[:2000]) & 0xFFFFFFFF
    cache_key = f"{topic}|{subject}|{intro_hash}"
    cached = _CONCEPT_CACHE.get(cache_key)
    if cached is not None:
        definition, ts = cached
        if time.time() - ts < CONCEPT_CACHE_TTL:
            return definition
        _CONCEPT_CACHE.pop(cache_key, None)  # протухло — пересчитываем

    definition = _extract_key_concept_definition(intro_text, topic)
    if definition:
        _CONCEPT_CACHE[cache_key] = (definition, time.time())
...
def add_toc(doc: Document, blocks: list[tuple], gost: dict) -> None:
    ...
    for i, (entry_title, entry_level, entry_page) in enumerate(toc_entries_paged):
        ...
        try:
            _toc_tw_mm = 210.0 - float(gost.get("left_margin_mm", 30)) - float(gost.get("right_margin_mm", 10))
        except Exception:
            _toc_tw_mm = 170.0
...
def _strip_duplicate_heading_prefix(text: str, heading: str) -> str:
    ...
    if heading_words:
        ...
        # Срезаем ТОЛЬКО когда строка начинается с НОМЕРА (\d...): это исключает
        # ложное удаление обычного предложения, начинающегося со слов заголовка.
        result = re.sub(
    ...
    # FIX #7 (из задания): если после среза префикса текст начинается со
    # строчной буквы — восстанавливаем заглавную (иначе абзац стартует с «в …»).
...
async def _download_image_bytes(url: str, *, timeout: int = 30) -> Optional[bytes]:
    if not url:
        return None
    try:
...
async def prepare_work_images(
    ...
    async def add_from(found: list[dict], avoid_recent: bool = True) -> None:
        nonlocal images
        found = list(found or [])
        _random.shuffle(found)  # разная выдача между работами темы (анти-«одинаковые на класс»)
        for img in found:
            ...
            if any(w in _cap for w in _MEME_WORDS):
                ...
                continue
            # Отсев лайфстайл/стоковых фото с людьми — для научной работы нужны
            # схемы, графики, природа, объекты, а не «дети на тропе», «турист», «селфи».
    ...
    providers = (
        ("science", search_science_images),
        ("openverse", search_openverse_images),
        ("pixabay", search_pixabay_images),   # бесплатный (нужен бесплатный PIXABAY_KEY)
        ("pexels", search_pexels_images),     # бесплатный (нужен бесплатный PEXELS_KEY)
    ...
    _deadline = _time.monotonic() + float(cfg("IMAGE_SEARCH_BUDGET_SEC", "45"))
    # Порядок: provider-OUTER. Сначала исчерпываем НАУЧНЫЙ провайдер по ВСЕМ
    # запросам (среди них есть английские, которые матчат geograph/музеи),
    # и только потом — фотостоки. Иначе Pixabay побеждает на первом же
    # (русском) запросе и научные источники не получают шанса.
    for provider_name, provider in providers:
        ...
        for q in queries:
            ...
            if _time.monotonic() > _deadline:
                print(f"[IMAGES] бюджет времени исчерпан, собрано {len(images)}/{count}")
                break
            try:
                found = await provider(q, limit=need)
                for _f in (found or []):
                    _f.setdefault("_query", q)
    ...
    if len(images) < count and _time.monotonic() < _deadline:
        for q in queries:
            if _time.monotonic() > _deadline:
                break
            for provider_name, provider in providers:
    ...
    # Подпись = полная тема реферата (а не поисковый запрос)
    for im in images:
        im["caption"] = topic
    return images
...
def add_gost_image(doc: Document, image: dict, number: int, gost: dict) -> None:
    ...
    try:
        ...
        def _short_src(u):
            try:
                import re as _re
                _m = _re.search(r"https?://(?:www\.)?([^/]+)", str(u or ""))
...
async def measure_pages_async(docx_path: str, work_dir: str) -> Optional[int]:
    ...
    #    FIX (блокировка потока): count_docx_pages запускает LibreOffice/PDF —
    #    это тяжёлая СИНХРОННАЯ операция. Выносим её в отдельный поток, чтобы не
    #    «замораживать» event loop бота (иначе виснут все пользователи).
    n = await asyncio.to_thread(count_docx_pages, docx_path, work_dir)
    if n is not None:
    ...
    # 3) Расчётный эстиматор (быстрый, но тоже выносим — читает файл)
    n = await asyncio.to_thread(estimate_docx_pages, docx_path)
    if n is not None:
...
def _trim_blocks_by_chars(blocks: list[tuple], chars_to_remove: int,
    ...
    def _trim_text(txt: str, need: int, floor: int = 0) -> tuple[str, int]:
        ...
        # FIX #3: защита от полного исчезновения текста.
        # Пустой вход — возвращаем как есть.
        if not paras:
            return txt, 0
        # Никогда не обрезаем ниже минимума абзацев. НЕ добиваем заглушками
        # (это внесло бы мусор в академический текст) — просто прекращаем резать,
        # сохраняя реальную структуру. Для коротких блоков минимум не может быть
        # больше исходного числа абзацев.
        MIN_PARAGRAPHS = min(3, len(paras))

        def _cur_len_paras():
            return sum(len(p) for p in paras) + 2 * max(0, len(paras) - 1)

        # 1) Абзацами с конца — пока их больше минимума и не упёрлись в floor
        while paras and need > 0 and len(paras) > MIN_PARAGRAPHS:
            ...
            if last and last[-1] not in ".!?…»":
                ...
                if not m:
                    ...
                    for i in range(len(last) - 1, -1, -1):
                        char = last[i]
                        if char in ".!?…":
                            # FIX #4: точка в ссылке [N, с. K] — НЕ конец предложения.
                            # Проверяем контекст вокруг точки, а не только предыдущую
                            # букву: (а) точка внутри «[N, с. K]»; (б) «с.» перед числом.
                            if char == '.':
                                context = last[max(0, i - 6):i + 6]
                                if re.search(r'\[\s*\d+\s*,\s*с\.\s*\d*', context):
                                    continue  # это ссылка — не режем здесь
                                if i > 0 and last[i - 1].lower() == 'с' \
                                        and re.search(r'\d', last[i + 1:i + 3]):
                                    continue  # «с. 45» — не режем здесь
                            cut = i
                            break
...
async def precise_page_adjustment(
    ...
    for it in range(max_iters):
        ...
        if diff > 0:
            # FIX #6: адаптивный коэффициент обрезки вместо фиксированного 1.0.
            # Множитель 1.0 давал перебор на 2–3 страницы («цикл не сходится»),
            # потому что после обрезки строки перетекают и страниц уходит больше,
            # чем линейно ожидалось. Теперь на первых итерациях режем осторожно
            # (0.7), затем всё точнее (0.5 → 0.3), сходясь к цели без перелёта.
            correction_factor = 0.7 if it < 3 else (0.5 if it < 6 else 0.3)
            chars_to_remove = int(abs(diff) * chars_per_real_page * correction_factor)
            print(f"[ADJUST] Обрезка: {diff:+d} стр → {chars_to_remove} зн. "
                  f"(фактор {correction_factor})")
...
async def parse_custom_gost_via_ai(model_key: str, gost_text: str) -> dict:
    schema_hint = {
        ...
        "first_line_indent_cm": 1.25,
        "left_margin_mm":       30,
        "right_margin_mm":      15,   # ГОСТ 7.32-2017: правое поле 15 мм
        "top_margin_mm":        20,
        "bottom_margin_mm":     20,
        "page_number_position": "bottom_center",
    }
    ...
    raw, _ = await chat_with_fallback(
        ...
        [{"role": "system", "content": system}, {"role": "user", "content": user}],
        max_tokens=800,
        temperature=0.0,  # извлечение параметров — детерминированная задача
    )
    raw = raw.strip()
...
def _build_edited_docx_bytes(corrected_text: str, gost: dict, source_images: Optional[list[dict]] = None) -> bytes:
...
async def h_topic_suggest(cb: CallbackQuery, state: FSMContext) -> None:
    ...
    invalidate_concept_cache()  # FIX #7: тема сменилась — сбрасываем кэш определений
    await cb.message.edit_text(
...
async def generate_and_send(
    ...
    # FIX #1: атомарный захват слота (проверка лимита + инкремент под Lock).
    if not await _acquire_user_slot(_uid):
        await event.answer(
    ...
    async with GEN_SEMAPHORE:
        ...
        # FIX #5: контекст временных файлов — очистка гарантируется в finally.
        _tmp_ctx = TempFileContext(prefix=f"gost_{event.chat.id}_")
        ...
        try:
            ...
            # FIX #5: регистрируем временные файлы и служебную папку _measure на
            # гарантированную очистку — даже если генерация упадёт с исключением.
            _tmp_ctx.track(tmp_in)
            _tmp_ctx.track(tmp_out)
            _tmp_ctx.track_dir(os.path.join(work_dir, "_measure"))
            ...
            updated    = await asyncio.to_thread(libreoffice_update_docx, tmp_in, tmp_out)
            ...
            try:
                for toc_round in range(2):
                    real_toc_pages = await asyncio.to_thread(_extract_real_toc_pages_from_docx, final_path, blocks, work_dir)
            ...
            try:
                from checker import validate_docx, autofix_docx
                _gr = validate_docx(final_path, doc_type=doc_type)
                print(f"[ГОСТ] Оценка: {_gr.score}/100")
                if _gr.score < 90:
                    _ch = autofix_docx(final_path, final_path)
                    _gr = validate_docx(final_path, doc_type=doc_type)
                    print(f"[ГОСТ] Авто-фикс: {'; '.join(_ch) if _ch else 'нет изменений'}")
                    print(f"[ГОСТ] Оценка после фикса: {_gr.score}/100")
                if not _gr.passed:
                    print("[ГОСТ] Замечания:", [c.title for c in _gr.failures()])
                    log_error(
                        "quality_gate", f"ГОСТ-оценка {_gr.score}/100 ниже порога",
                        topic=topic, doc_type=doc_type,
                        extra={"issues": [c.title for c in _gr.failures()]},
                    )
                _log_metric(event="quality_gate", success=bool(_gr.passed),
                            doc_type=doc_type, topic=topic, note=f"score={_gr.score}")
            except Exception as _gv_e:
                print(f"[ГОСТ] Валидация пропущена: {_gv_e}")
                log_error("quality_gate_failed", "Валидация/автофикс упали",
                          topic=topic, doc_type=doc_type, exc_info=_gv_e)
            ...
            # FIX #2: списание/учёт лимита выполняется ТОЛЬКО после успешной
            # отправки файла (answer_document выше). Если Telegram не отдал файл
            # (>50 МБ, таймаут, ошибка сети) — исключение уйдёт в except, и
            # попытка/звёзды НЕ будут списаны.
            record_user_generation(event.chat.id, pay_mode)

            # Телеметрия успешной работы: точность попадания в страницы,
            # тип документа, модель — чтобы видеть, где бот слабее конкурентов.
            _log_metric(event="work", success=True, doc_type=doc_type, topic=topic,
                        pages=pages, final_pages=final_pages, model_key=model_key,
                        user_id=event.chat.id)

        except Exception as e:
            print(f"[GEN ERROR] {e}")
            log_error("generation_failed", "Ошибка на этапе генерации/отправки",
                      topic=topic, doc_type=doc_type, model=model_key,
                      user_id=_uid, exc_info=e, send_alert=True)
            _log_metric(event="work", success=False, doc_type=doc_type, topic=topic,
                        model_key=model_key, user_id=_uid, note=str(e)[:200])
            await prog.finish(
            ...
            await state.clear()
            await _release_user_slot(_uid)  # FIX #1: атомарное освобождение слота
            return
            ...
            try:
                ...
                pass
            # FIX #5: гарантированная очистка временных файлов и папки _measure —
            # выполняется и при успехе, и при исключении (защита диска от утечки).
        ...
        # FIX #5: временные файлы уже удаляются в finally через _tmp_ctx —
        # отдельный блок очистки здесь больше не нужен.
...
async def main() -> None:
    ...
    # Включаем Telegram-алерты админу о критических сбоях (best effort).
    try:
        if ADMIN_CHAT_ID:
            configure_error_alerts(BOT_TOKEN, ADMIN_CHAT_ID)
            print(f"  Алерты      : ✅ админ {ADMIN_CHAT_ID}")
        else:
            print("  Алерты      : ⚠️ ADMIN_CHAT_ID не задан")
    except Exception as _e:
        print(f"  Алерты      : ❌ {_e}")

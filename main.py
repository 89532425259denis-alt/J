...
"""ГОСТ-АССИСТЕНТ v2.2 — ИСПРАВЛЕНИЕ СТРУКТУРЫ И ЗАКЛЮЧЕНИЯ
...
def strict_prompt(task: str, chars: int, writing_style: str = "classic") -> str:
    ...
    return (
        ...
        "Используй сноски в каждом абзаце (1–3 ссылки) — номера источников из списка литературы: 1, 2, 3 и т.д. "
        "Запрещены фразы «как ИИ», «давайте рассмотрим», «следует отметить», «таким образом» в начале абзацев. "
...
async def generate_text_blocks(
    ...
    style_sys = (
        ...
        "Обязательно используй ГОСТ-сноски в квадратных скобках: [1, с. 45], [2], [3, с. 120–125] "
    ...
    # Собираем краткие выжимки из всех глав для контекста заключения
    chapter_summaries = []
    for key in sorted(parts.keys()):
        if key.startswith("ch"):
            txt = parts[key]
            # Берём первые 500 и последние 500 знаков каждой главы как контекст
            head = txt[:500].strip() if len(txt) > 500 else txt.strip()
            tail = txt[-400:].strip() if len(txt) > 400 else ""
            ch_label = key.replace("ch", "Глава ").replace("_s", ", подглава ")
            chapter_summaries.append(
                f"=== {ch_label} (начало) ===\n{head}\n"
                + (f"=== {ch_label} (конец) ===\n{tail}" if tail else "")
            )

    chapters_context = "\n\n".join(chapter_summaries) if chapter_summaries else (
        f"Текст глав по теме «{topic}» был сгенерирован."
    )

    conc_chars = int(total_chars * 0.08)
    conc_prompt = (
        f"Напиши заключение для работы «{topic}».\n\n"
        f"НИЖЕ — РЕАЛЬНОЕ СОДЕРЖАНИЕ ГЛАВ, которые уже написаны:\n"
        f"{chapters_context}\n\n"
        f"Твоя задача:\n"
        ...
        f"5. Используй сноски [1], [2] на источники из списка литературы.\n"
        f"ЗАПРЕЩЕНО: выдумывать необсуждённые темы (геологию, биологию и т.п. "
        f"если их не было в главах)."
    )
...
def build_docx_bytes(
    ...
    for title, level, text, subblocks in blocks:
        ...
        if subblocks:
            # Есть подблоки — выводим их (подблоки уже содержат полный текст)
...
def _trim_blocks_by_chars(blocks: list[tuple], chars_to_remove: int) -> list[tuple]:
    ...
    # Идём с конца, пропуская «список литературы» и «заключение» в первую очередь
    order = list(range(len(blocks)))
    # Сначала срежем из самых больших основных блоков
    main_indices = sorted(
        [i for i, b in enumerate(blocks)
         if not any(w in (b[0] or "").upper() for w in ("ЛИТЕРАТ", "ИСТОЧНИК", "БИБЛИОГРАФ"))],
        key=lambda i: -(len(blocks[i][2] or "")),
    )
    for i in main_indices:
        ...
        while paras and chars_to_remove > 0 and len(paras) > 2:
    ...
    async with GEN_SEMAPHORE:
        ...
        try:
            ...
            for it in range(max_iters):
                ...
                # 1 страница ≈ CHARS_PER_PAGE символов; для уверенности +20%
                chars_per_page = max(1500, int(CHARS_PER_PAGE * 1.15))
            ...
            # Финальный замер для caption (используем тот же файл, что отправляем)

...
MAX_WEB_SOURCES    = int(cfg("MAX_WEB_SOURCES", "15"))
...
def _strip_markdown_markers(text: str) -> str:
    ...
    if not text:
        return ""
    # Заголовки вида `## **Название**` / `# Название`
    text = re.sub(r'(?m)^\s*#{1,6}\s*\*{0,2}', '', text)
    # Жирный/курсив markdown: **текст**, *текст*, ***текст***
    text = re.sub(r"\*{1,3}([^*\n]+?)\*{1,3}", r"\1", text)
    # Оставшиеся одиночные маркеры удаляем полностью.
...
def _source_relevance_score(record: dict, topic: str, subject: str) -> int:
    ...
    topic_norm = re.sub(r"\s+", " ", (topic or "").lower()).strip()
    subject_norm = re.sub(r"\s+", " ", (subject or "").lower()).strip()
    topic_words = _keywords_for_relevance(topic)
    subject_words = _keywords_for_relevance(subject)

    score = 0
    if topic_norm and len(topic_norm) >= 4 and topic_norm in haystack:
        score += 10
...
def _filter_relevant_source_records(records: list[dict], topic: str, subject: str, limit: int) -> list[dict]:
    """Фильтрует OpenAlex/Crossref, чтобы список литературы был именно по теме."""
    deduped = _dedupe_source_records(records, max(limit * 4, limit))
    scored = [(_source_relevance_score(r, topic, subject), r) for r in deduped]
    scored.sort(key=lambda x: x[0], reverse=True)

    # Для темы из одного слова достаточно score>=4; для развёрнутой темы —
    # желательно хотя бы одно точное тематическое слово/фраза.
    strong = [r for score, r in scored if score >= 4]
    if strong:
        return strong[:limit]

    weak = [r for score, r in scored if score > 0]
    return weak[:limit]
...
def _format_source_record(record: dict) -> str:
    ...
    if not title:
        return ""
    author_part = ", ".join([a for a in authors if a]) or "Без автора"
    if container:
...
async def fetch_verified_sources(topic: str, subject: str, limit: int = 12) -> str:
    ...
    try:
        async with aiohttp.ClientSession(headers=headers) as session:
            q = quote_plus(query)
            # OpenAlex — хорошие метаданные, нет ключа, сортировка по цитируемости.
            openalex_url = (
                "https://api.openalex.org/works?"
                f"search={q}&per-page={max(limit, 10)}&sort=cited_by_count:desc"
            )
            ...
            # Crossref — резерв и дополнительные DOI.
            crossref_url = (
                "https://api.crossref.org/works?"
                f"query.title={q}&rows={max(limit * 2, 10)}"
            )
            cr = await _fetch_json(session, crossref_url)
    ...
    records = _filter_relevant_source_records(records, topic, subject, min(limit, MAX_WEB_SOURCES))
    lines: list[str] = []
...
async def generate_text_blocks(
    ...
    verified_catalog_bib = await fetch_verified_sources(
        topic, subject,
        limit=12 if pages <= 20 else min(MAX_WEB_SOURCES, 20),
    )
...
def _fill_missing_pages(text: str, global_page_map: Optional[dict] = None) -> str:
    ...
    а если их нигде нет — добавляется стабильный псевдо-номер страницы, чтобы в
    итоговом DOCX не осталось ссылок без страниц.
    """
    if not text:
        return text
    ...
    def _normalize_part(part: str) -> Optional[str]:
        ...
        if not part:
            return None
        # N, с. X / N с. X / N c.X / N p. X / N, X
        m = re.match(
            r"^(\d+)\s*(?:[,;]?\s*(?:[сСcCpP]\.?|стр\.?|page)?\s*(\d+(?:[–—-]\d+)?))?\s*$",
            part,
        )
        if not m:
            return None
        n = m.group(1)
        page = m.group(2) or _page_for(n)
        if not page:
            return f"{n}"
        return f"{n}, с. {page}"
    ...
    def repl(m: "re.Match[str]") -> str:
        inner = m.group(1)
        # Не трогаем квадратные скобки, которые не начинаются с номера источника.
    ...
    new = re.sub(r"\[([^\]\n]+)\]", repl, text)
    if new != text:
        before = len(re.findall(r"\[\s*\d+(?:\s*\]|\s*;)", text))
        print(f"[CITE] Нормализованы ссылки со страницами: {before or 'несколько'}")
    return new
...
def _ensure_block_terminates(text: str) -> str:
    ...
    if s[-1] in ".!?…":
        return text
    # Если блок заканчивается корректной ссылкой `[N, с. X]`, нельзя обрезать
    # по точке в `с.` — надо просто добавить финальную точку после ссылки.
    if re.search(r"\[\s*\d+\s*,\s*[сСcC]\.\s*\d+(?:[–—-]\d+)?\s*\]\s*$", s):
        return s + "."
...
def add_title_page(doc: Document, data: dict, gost: dict) -> None:
    ...
    _add_centered(str(data.get("ministry") or ministry).upper(), 11, True)
...
def _strip_duplicate_heading_prefix(text: str, heading: str) -> str:
    """Удаляет повтор заголовка в начале текста подглавы/раздела.

    Модели часто возвращают: `1.1 Заголовок\n\nТекст`, а DOCX уже создаёт
    отдельный Heading 2. Эта функция убирает первые строки, совпадающие с
    заголовком даже при вариантах `## **1.1. Заголовок**`.
    """
    ...
    changed = True
    attempts = 0
    while changed and attempts < 5 and lines:
        attempts += 1
        changed = False
        ...
        if _same_heading_line(first, heading):
            ...
            changed = True
            continue
        # Иногда модель пишет отдельно номер, затем заголовок.
...
async def h_pagepos(cb: CallbackQuery, state: FSMContext) -> None:
    ...
    if mode == "free":
        ...
        await cb.message.edit_text(
            ...
            "Бот может внести 1–2 мелкие опечатки и неравные абзацы, "
            "чтобы текст выглядел менее «машинным».",
            reply_markup=with_back(kb_humanize()),
            parse_mode="HTML",
        )

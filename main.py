...
def generate_structure(
    ...
    for i, ch in enumerate(chapter_titles, start=1):
        ...
        # ═══ ИСПРАВЛЕНИЕ: НЕ ДУБЛИРУЕМ ОБЩИЙ ТЕКСТ ГЛАВЫ ═══
        # ВАЖНО: field[2] (text) должен быть пустым, если есть подглавы,
        # чтобы build_docx_bytes не вывел общий текст дважды.
        full_chapter_text = "\n\n".join(chapter_text_parts) if chapter_text_parts else ""
        blocks.append((ch["title"], 1, "" if sub_blocks else full_chapter_text, sub_blocks))
...
def _remove_duplicate_paragraphs(text: str) -> str:
    """
    Удаляет полностью дублирующиеся абзацы в тексте.
    """
    if not text:
        return text
...
def add_paragraphs_from_text(
    ...
    if skip_first_heading and not is_bib and text:
        ...
        def _matches(line: str) -> bool:
            return _norm(line) == _norm(skip_first_heading)
        ...
        for i, line in enumerate(lines):
            ...
            if _matches(line):
                ...
                continue
            if _is_bare_number(line) and i + 1 < len(lines) and _matches(lines[i + 1]):
                # Пропускаем "1.1." и следующий за ним заголовок
                skip_next = True
                continue
...
def _texts_are_similar(text1: str, text2: str, threshold: float = 0.7) -> bool:
    """
    Проверяет, похожи ли два текста.
    Используется для определения дублирования общего текста и подглав.
    """
    ...
    # Если один текст содержит другой целиком
    if t1 in t2 or t2 in t1:
        return True
    
    # Сравниваем по словам
    words1 = set(re.findall(r'[а-яёa-z]{4,}', t1))
    words2 = set(re.findall(r'[а-яёa-z]{4,}', t2))
...
def build_docx_bytes(
    ...
    for idx, (title, level, text, subblocks) in enumerate(blocks):
        ...
        if subblocks:
            # Проверяем: не дублируется ли общий текст в подглавах
            # Часто бывает, что text = сумма всех sub_text, тогда выводим только подглавы
            sub_text_total = "\n\n".join(st for _, st in subblocks if st)
            
            # Если общий текст почти совпадает с суммой подглав — не выводим его отдельно
            if text and sub_text_total and _texts_are_similar(text, sub_text_total):
                print(f"[DOCX] Общий текст главы «{title}» не выводим (дублируется в подглавах)")
                text = ""  # Не выводим общий текст
            
            # Если text не пустой и не дублируется — выводим его ДО подглав
            if text:
                add_paragraphs_from_text(doc, text, gost, is_bib=False)
            ...
            for sub_idx, (sub_title, sub_text) in enumerate(subblocks):
                ...
                if sub_text:
                    ...
                    emergency_text = _generate_substantial_stub(sub_title, title, topic)
                ...
                # Добавляем пустую строку между подглавами
                if sub_idx < len(subblocks) - 1:
                    doc.add_paragraph()
        elif text:
            # Нет подблоков — выводим только текст
            add_paragraphs_from_text(doc, text, gost, is_bib=is_bib, skip_first_heading=title if is_bib else None)
            if is_bib:
                doc.add_page_break()
        else:
            # Блок без текста — заглушка
            if not is_bib:
                emergency_text = _generate_substantial_stub(title, "", topic)
                ep = doc.add_paragraph()
                ep.paragraph_format.alignment = WD_ALIGN_PARAGRAPH.JUSTIFY
                ep.paragraph_format.first_line_indent = Cm(float(gost.get("first_line_indent_cm", 1.25)))
                er = ep.add_run(emergency_text)
...
def _trim_blocks_by_chars(blocks: list[tuple], chars_to_remove: int) -> list[tuple]:
    ...
    # ═══ ИСПРАВЛЕНИЕ: не обрезаем блоки с подглавами, если общий текст пустой ═══
    def _can_trim(title: str, subblocks: list) -> bool:
        up = (title or "").upper()
        if any(w in up for w in ("ЛИТЕРАТ", "ИСТОЧНИК", "БИБЛИОГРАФ", "ЗАКЛЮЧЕНИ")):
            return False
        # Если есть подглавы, обрезаем только их, не общий текст
        if subblocks:
            return True
        return True
    ...
    candidates = sorted(
        [i for i, b in enumerate(blocks) if _can_trim(b[0], b[3])],
    ...
    for i in candidates:
        ...
        if subblocks:
            ...
            # Обновляем агрегированный текст только если он был
            if b[2]:
                b[2] = "\n\n".join(st for _, st in subblocks if st)
        else:
            txt = b[2] or ""
...
def kb_doc_type() -> InlineKeyboardMarkup:
    ...
    b.button(text="⚙️ Изменить ГОСТ", callback_data="custom_gost")
...
async def h_fix_subject_universal(cb: CallbackQuery, state: FSMContext) -> None:
    ...
    if action == "continue":
        ...
        await state.update_data(discipline_verified=True)
        ...
        await state.update_data(discipline_verified=False)
        ...
        new_subject = action
        await state.update_data(subject=new_subject, discipline_verified=True)
...
async def h_save_custom_gost(message: Message, state: FSMContext) -> None:
    ...
    for _dt in DOC_TYPES.keys():
        save_user_gost_config(message.from_user.id, _dt, parsed)
...
async def generate_and_send(
    ...
    async with GEN_SEMAPHORE:
        ...
        discipline_verified = data.get("discipline_verified", False)
        if not discipline_verified:
            # ═══════════════════════════════════════════════════════════════
            # УНИВЕРСАЛЬНАЯ ПРОВЕРКА СООТВЕТСТВИЯ ТЕМЫ И ДИСЦИПЛИНЫ
            # ═══════════════════════════════════════════════════════════════

...
async def generate_text_blocks(
    ...
    # ═══════════════════════════════════════════════════════════
    # ПРОВЕРКА НА ОБРЫВЫ ССЫЛОК (Пост-валидация)
    # ═══════════════════════════════════════════════════════════
    if key not in ("literature",) and text:
        retry_count = 0
        # Проверяем на обрывы ссылок [1, с. в конце текста
        while (not _validate_no_broken_citations(text) or re.search(r'\[\d+\s*,\s*[сc]\.\s*$', text)) and retry_count < 2:
            retry_count += 1
            if prog:
                await prog.update(label=f"🔄 Исправляю обрывы ссылок в {block_name} (попытка {retry_count})", force=True)
            
            # Просим модель переписать блок, акцентируя внимание на ошибке
            retry_prompt = prompt + "\n\n⚠️ ОШИБКА: В предыдущем ответе были обрывы ссылок (например, '[1, с.' без цифры или ссылка в конце текста не завершена). " \
                            "ПЕРЕПИШИ блок, убедившись, что КАЖДАЯ ссылка завершена номером страницы и закрывающей скобкой."

...
def target_chars(pages: int, gost: dict = None) -> int:
    ...
    # 0.55 — эмпирический коэффициент. Калиброван по серии запусков:
    # 0.80 → +3 страницы, 0.62 → +2 страницы, 0.55 ≈ попадание ±1.
    return int(raw_total * 0.55)
...
def estimate_docx_pages(docx_path: str) -> Optional[int]:
    ...
    # Раньше было page_breaks // 2 — эстиматор всё равно недосчитывал
    # реальные страницы, и подгонщик останавливался слишком рано.
    # Теперь учитываем разрыв почти целиком: каждая страница с заголовком
    # 1-го уровня + интервалами + первым абзацем ≈ полная отдельная страница.
    structural_overhead = int(page_breaks * 0.8)

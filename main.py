...
async def generate_text_blocks(
    ...
    if doc_type == "esse":
        MIN_ESSAY_BODY = 950
        MIN_INTRO_BODY = 700
        for k in ["main1", "main2", "intro"]:
            min_body = MIN_INTRO_BODY if k == "intro" else MIN_ESSAY_BODY
            if k in parts:
                txt = parts[k].strip()
                if len(txt) < min_body:
                    print(f"[WARN] Блок {k} слишком короткий ({len(txt)} знаков) — расширяем через ИИ с контекстом темы")
                    need = min_body - len(txt)
                    label = "вступление" if k == "intro" else f"аргумент {k}"
                    # Усиленный промпт для расширения
                    expand_p = f"""Тема эссе: «{topic}». Дисциплина: «{subject}». Блок: {label} (ключ {k}).
Продолжи и РАЗВЕРНИ текст. Добавь минимум {need} знаков (цель ~{min_body}+ всего для этого блока).
{'Пиши от первого лица, с личной позицией.' if k != "intro" else 'Обозначь проблему, актуальность, цель.'} 
...
Напиши только продолжение текста, без вступлений и заголовков."""
                    try:
                        ...
                        fillers = [
                            f" В контексте темы «{topic}» и дисциплины «{subject}» этот блок подтверждается реальными примерами из практики и научных исследований, что позволяет увидеть проблему под новым углом.",
                            ...
                            f" Конкретные наблюдения и данные по теме «{topic}» убедительно демонстрируют справедливость этого блока, опираясь на работы известных специалистов в области {subject}.",
                        ]
                        while len(parts[k]) < min_body:
    ...
    if doc_type == "esse":
        MIN_CONC_BODY = 700
        txt = parts.get("conclusion", "").strip()
        if len(txt) < MIN_CONC_BODY:
            print(f"[WARN] Заключение слишком короткое ({len(txt)} знаков) — расширяем через ИИ")
            need = MIN_CONC_BODY - len(txt)
            expand_p = f"""Тема эссе: «{topic}». Дисциплина: «{subject}». Блок: заключение.
Подведи личный итог на основе аргументов выше. Добавь минимум {need} знаков.
Пиши от первого лица. Минимум 4-5 полных абзацев. Опирайся на реальное содержание аргументов.
Уже написано (хвост):
{txt[-400:]}

Напиши только продолжение, без заголовков."""
            try:
                extra, _ = await chat_with_fallback(
                    model_key,
                    [
                ...
                if extra and len(extra.strip()) > 50:
                    extra = _clean_ai_artifacts(extra)
                    parts["conclusion"] = (txt + "\n\n" + extra.strip()).strip()
                    print(f"[EXPAND] Успешно добавлено для conclusion: +{len(extra)} знаков")
                else:
                    raise ValueError("short")
            except Exception as e:
                print(f"[EXPAND] Для conclusion не сработал: {e}")
                filler = f" В заключение по теме «{topic}» я прихожу к выводу, что проблема требует внимания и дальнейшего изучения в рамках {subject}."
                while len(parts["conclusion"]) < MIN_CONC_BODY:
                    parts["conclusion"] += " " + filler

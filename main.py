...
# Символов на страницу (ГОСТ: ~1650-1850 знаков с пробелами на стр A4 14pt 1.5 интервал)
CHARS_PER_PAGE = int(cfg("CHARS_PER_PAGE", "1650"))
    ...
    for key, prompt in prompts.items():
        ...
        if (is_bad or is_too_short) and key not in ("literature",):
            ...
            if text2 and len(text2.strip()) > len(text.strip()) and len(text2.strip()) > 300:
                ...
                print(f"[WARN] Перегенерация не помогла, используем расширенный fallback")
...
def estimate_docx_pages_ultra(docx_path: str) -> Optional[int]:
    """Улучшенный эстиматор страниц на основе строк и параметров ГОСТ."""
    try:
    ...
    # Параметры ГОСТ для TNR 14pt, 1.5 интервал (около 28-30 строк на страницу)
    lines_per_page = 28
    chars_per_line = 72 # Среднее количество знаков в строке с учетом полей
    
    for p in doc.paragraphs:
        # Проверяем Heading 1 или разрыв страницы
        ...
        # Заголовок главы или явный разрыв страницы всегда начинают новую страницу
        if is_h1 or has_pb:
        ...
        txt = p.text or ""
        if not txt.strip():
            # Пустая строка (например, отступ после заголовка)
            p_lines = 1
        else:
            # Учитываем абзацный отступ (примерно 7 знаков)
            # Если текст короче chars_per_line - 7, это 1 строка.
            p_lines = math.ceil((len(txt) + 7) / chars_per_line)
            
        current_page_lines += p_lines
        
        # Если накопили достаточно строк — это новая страница
        while current_page_lines >= lines_per_page:
            total_pages += 1
    ...
    print(f"[ESTIM] Итог (линии): {total_pages} стр")
    return total_pages
...
async def force_page_count_ultra(
    ...
    for i in range(max_iterations):
        ...
        # Если разница в 1 страницу — это уже очень хороший результат.
        # На поздних итерациях принимаем его.
        if abs(diff) <= 1 and i >= 3:
            print(f"[ULTRA] ✅ Приемлемо: {current} (цель {target_pages})")
            try:
                raw_len = sum(len(p.text) for p in Document(docx_path).paragraphs)
                NEURO.learn(raw_len, current, target_pages)
            except: pass

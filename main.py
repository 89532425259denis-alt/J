...
try:
    from error_logger import log_error, configure_error_alerts
...
async def generate_chapter_titles(
    ...
    # Низкая температура: план работы — это структура, а не творчество.
    # 0.2 даёт стабильно валидный JSON и меньше «шаблонных» названий.
    raw, _ = await chat_with_fallback(model_key, messages, max_tokens=1500, temperature=0.2)
    raw    = raw.strip()

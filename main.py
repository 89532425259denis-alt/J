# -*- coding: utf-8 -*-
# GOST-ASSISTANT v3.4-GOST-FIXED — строгий ГОСТ 7.32-2017 / ГОСТ Р 7.0.5-2008
# Применённые патчи (apply_gost_fixes.py):
#   1) Структурные заголовки в ВЕРХНЕМ РЕГИСТРЕ (§6.1)
#   2) Подпись/источник кеглем 14 pt (без уменьшения)
#   3) Размер картинок: ширина 12 см по умолчанию, высота до 18 см
#   4) Проверка мин. разрешения изображения ≥ 800 px (§5.3.8)
#   5) Снята обрезка подписи до 120 символов (§5.3.8)
#   6) «Источник:» выводится только для библиографических ссылок [N]
#   7) Библиография по ГОСТ Р 7.0.5-2008 (том/выпуск/страницы/Текст : электронный/дата обращения)
#   8) «Выполнил/Проверил» выровнены по левому краю правой колонки (§5.1)
...
# Подключаем улучшенные промпты из отдельного модуля
try:
    from gost_prompts import strict_prompt_v2 as strict_prompt
    print("[ПРОМПТ] ✅ Использую strict_prompt_v2 из gost_prompts.py")
except ImportError:
    print("[ПРОМПТ] Использую встроенную strict_prompt")
...
# Абстрактные шаблоны структуры вместо конкретных текстов (TEMPLATE FIX)
GOLDEN_STANDARDS = {
    "academic": {
        "intro": "СТРУКТУРА ВВЕДЕНИЯ: 1) Обоснуй актуальность. 2) Цель работы. 3) 2-3 задачи. 4) Структура. Ссылки: [N, с. X].",
        "chapter": "СТРУКТУРА ГЛАВЫ: 1) Тезис. 2) Теория с данными. 3) Анализ. 4) Связь со следующей главой.",
        "conclusion": "СТРУКТУРА ЗАКЛЮЧЕНИЯ: 1) Вывод по каждой главе. 2) Значимость. 3) Перспективы. Не начинай с 'Таким образом'.",
    },
    "doklad": {
        "main": "СТРУКТУРА: 1) Ключевой тезис. 2) Факты и цифры. 3) Примеры. 4) Вывод.",
    },
    "esse": {
        "main": "СТРУКТУРА: 1) Личное отношение. 2) Аргумент с примером. 3) Позиция. Без 'следует отметить'.",
    },
}
...
FEW_SHOT_EXAMPLES = """
ПРАВИЛА ОФОРМЛЕНИЯ:
• Ссылки: [1, с. 45] — обязательно номер страницы
• Не обрывай ссылки: [1, с.  — НЕЛЬЗЯ
• Фразы-маркеры ИИ («Вот ваш текст», «Конечно») — ЗАПРЕЩЕНЫ
"""
...
def _format_source_record(record: dict) -> str:
    """Форматирует запись из OpenAlex/Crossref по ГОСТ Р 7.0.5-2008.

    Расширенная запись: добавлены том, выпуск, страницы, обозначение
    «Текст : непосредственный» / «Текст : электронный» и дата обращения
    для электронных ресурсов (ГОСТ Р 7.0.5-2008 §5.2.1, §5.3.1).
    """
    ...
    # Базовая часть: автор + название + тип текста
    is_electronic = bool(doi or url)
    text_type = "Текст : электронный" if is_electronic else "Текст : непосредственный"

    if container:
        # Статья в журнале/сборнике: // Журнал. — Год. — Т. — №. — С.
    ...
    # Обозначение характера текста (ГОСТ Р 7.0.5-2008 §5.2.1)
    ref += f" — {text_type}."
    ...
    if is_electronic:
        # Дата обращения для электронных ресурсов (ГОСТ Р 7.0.5-2008 §5.3.1)
        _today = _dt.now().strftime("%d.%m.%Y")
        ref += f" (дата обращения: {_today})."
    else:
        ref += "."

    # Убираем заглушки «[URL не указан]», если они протянулись из LLM/каталогов
...
async def generate_text_blocks(
    ...
    # ── СИСТЕМНЫЙ ПРОМПТ: компактный, без повторов, с вариативностью ──
    _writing_openers = [
        "Пиши академическим стилем: чётко, конкретно, без воды.",
        "Создай текст в научном стиле: факты, логика, аргументация.",
        "Стиль — академический: опирайся на данные, избегай общих фраз.",
    ]
    import random as _rnd
    style_sys = (
        f"{_rnd.choice(_writing_openers)}\n"
        "Без markdown (#, *, **). Без фраз «конечно», «следует отметить», «таким образом» в начале абзаца.\n"
        f"Дисциплина: «{subject}». Тема: «{topic}». Рассматривай тему через терминологию и методы дисциплины «{subject}».\n"
        "Разнообразь структуру предложений: чередуй длинные и короткие. Не начинай абзацы одинаково.\n"
        "Каждый абзац — одна мысль. Связывай абзацы логически, а не шаблонными переходами.\n"
    )
    style_sys += PAGE_CALC_PROMPT + "\n"
    style_sys += numeric_context
    
    if verified_bib:
        style_sys += (
            f"\nИСТОЧНИКИ (используй номера из списка, формат [N, с. X]):\n{verified_bib}\n"
        )

    # Жанровые указания — кратко, без дублирования
    genre_hints = {
        "esse": "ЭССЕ: от первого лица, живая позиция, без канцеляризмов.",
        "doklad": "ДОКЛАД: чётко, факты, короткие предложения.",
        "article": "СТАТЬЯ: строгий язык, без «я», ссылки [N, с. X] в каждом абзаце.",
        "default_smart": "Высокоакадемический стиль: терминология, концепции, теории.",
        "default_classic": "Деловой научный: ясные формулировки, логика, факты.",
    }
    if doc_type == "esse":
        style_sys += genre_hints["esse"]
    elif doc_type == "doklad":
        style_sys += genre_hints["doklad"]
    elif doc_type == "article":
        style_sys += genre_hints["article"]
    else:
        hint = genre_hints["default_smart"] if writing_style == "smart" else genre_hints["default_classic"]
        style_sys += hint

    # ── Правила генерации (сокращены, убраны повторы) ──
    style_sys += (
        f"\nПРАВИЛА:\n"
        f"1. КОНКРЕТИКА: цифры, даты, имена. Без «около» если есть данные.\n"
        f"2. ССЫЛКИ: только [N, с. X] — номер + страница. Без обрывов [N, с.\n"
        f"3. ЛОГИКА: в заключении — только то, что раскрыто в тексте.\n"
        f"4. ДИСЦИПЛИНА: строго «{subject}», не подменяй смежной.\n"
        f"5. ОБЪЁМ: ±10% от целевого. Без повторов, без сжатия.\n"
    )
    ...
    async def _gen_one_block(key, prompt):
        ...
        if key != "literature":
            ...
            # SPEED FIX: без перегенерации, только чистка служебных фраз
            forbidden = ["вступление к эссе", "вот текст", "вот второй аргумент",
                         "конечно. вот", "как языковая модель", "объем текста строго выдержан",
                         "вот первый аргумент"]
            text_lower = text.lower()
            for pat in forbidden:
                text = text.replace(pat, "").strip()
            if text_lower[:300].startswith(("вступление", "вот текст", "конечно")):
                text = text[200:].strip()

            # SPEED FIX 3: убираем дозаполнение (refill) — генерируем нужный объём сразу.
            # Если блок коротче 50% от цели, делаем ОДНУ попытку расширения вместо 3.
            if block_chars > 0 and len(text) < int(block_chars * 0.50):
                extra_chars = block_chars - len(text)
                ext_tok = tokens_for_chars(extra_chars)
                extra_prompt = (
                    f"Продолжи академический текст по теме «{topic}». "
                    f"Добавь ещё {extra_chars} знаков. "
                    f"Без заголовков, со ссылками [N, с. X]:\n\n{text[-400:]}"
                )
                ext_messages = [
    ...
    conc_prompt += (
        "\nПиши конкретно, без тавтологии. Каждое предложение — новый смысл."
    )
    ...
    # Структура заключения: выводы по главам + общий итог.
...
def _stub_text(key: str, topic: str) -> str:
    """Аварийный текст без клише."""
    topic = (topic or "").strip() or "данной области"
    stubs = {
        "intro":      f"В работе рассматривается тема «{topic}». Цель — анализ ключевых аспектов и выводы.",
        "conclusion": f"Анализ «{topic}» показал устойчивые закономерности. Результаты служат основой для дальнейшего исследования.",
        "literature": "",
    }
    if key in stubs:
        return stubs[key]
    return f"Аспект «{topic}» рассматривается в контексте научной дискуссии [1, с. 23]."
...
def _ensure_paragraph_breaks(text: str, min_paragraphs: int = 3) -> str:
    ...
    if len(sentences) < min_paragraphs:
        ...
        # TEMPLATE FIX: вместо шаблонных абзацев возвращаем что есть
        if paragraphs:
            return '\n\n'.join(paragraphs)
        if flat:
            return flat
...
def _generate_substantial_stub(sub_title: str, chapter_title: str, topic: str) -> str:
    """Краткая заглушка для пустой подглавы."""
    if not topic:
        topic = "данной области"
    return (
        f"Аспект «{sub_title}» рассматривается в контексте темы «{topic}». "
        f"Исследователи выделяют несколько подходов к данному вопросу. "
        f"Анализ показывает преимущества и ограничения каждого подхода [1, с. 34]."
    )
...
def add_title_page(doc: Document, data: dict, gost: dict) -> None:
    ...
    def _add_right(text: str, bold: bool = False) -> None:
        # ГОСТ 7.32-2017 §5.1: реквизиты «Выполнил/Проверил» размещаются
        # в ПРАВОЙ половине листа и выравниваются по ЛЕВОМУ краю своей колонки
        # (не прижимаются к правому полю).
        p = doc.add_paragraph()
        p.alignment = WD_ALIGN_PARAGRAPH.LEFT
        p.paragraph_format.first_line_indent = Cm(0)
        p.paragraph_format.left_indent = Cm(9.0)   # правая половина листа
...
async def search_wikimedia_images(query: str, limit: int = 3) -> list[dict]:
    ...
    for item in pages.values():
        ...
        if not _is_safe_real_photo_meta(caption, source, url, mime):
            continue
        out.append({"bytes": img_bytes, "caption": caption[:200], "source": source})
        if len(out) >= limit:
            break
...
def _image_relevance_tokens(topic: str, queries: Optional[list[str]] = None) -> list[str]:
    """Возвращает токены релевантности для фильтрации изображений.
    
    УЛУЧШЕНИЕ: учитываем не только тему, но и дисциплину,
    чтобы отсеивать картинки, не относящиеся к научному контексту.
    """
    tokens: list[str] = []
    # Тема — основной сигнал
    for w in re.split(r'[\s,;:]+', topic.lower()):
        w = w.strip()
        if len(w) > 2 and w not in _BAD_IMAGE_WORDS:
            tokens.append(w)
    # Дисциплина — дополнительный фильтр
    if queries:
        for q in queries:
            for w in re.split(r'[\s,;:]+', q.lower()):
                w = w.strip()
                if len(w) > 2 and w not in _BAD_IMAGE_WORDS and w not in tokens:
                    tokens.append(w)
    # Отсекаем: не должно быть meme/cartoon и т.п.
    tokens = [t for t in tokens if t not in _BAD_IMAGE_WORDS]
    # Whitelist: научные/академические слова усиливают релевантность
    _wl_tokens = []
    for t in tokens:
        wl = t.lower()
        if wl not in _BAD_IMAGE_WORDS and wl not in {"official", "portrait", "photo", "press", "documentary", "фото", "портрет"}:
            _wl_tokens.append(t)
    return _wl_tokens[:15]  # ограничиваем для скорости
...
async def prepare_work_images(
    ...
    # SPEED FIX: 3 провайдера вместо 6 — science + openverse + wikipedia дают
    # достаточно релевантных изображений без лишних HTTP-вызовов.
    providers = (
    ...
    _deadline = _time.monotonic() + float(cfg("IMAGE_SEARCH_BUDGET_SEC", "20"))
    ...
    # FIX-A: каждая подпись УНИКАЛЬНА — описывает содержимое конкретного
    # рисунка, а не просто повторяет тему работы. ГОСТ 7.32-2017, п. 5.3.8:
    # подпись должна раскрывать содержание иллюстрации.
    for i, im in enumerate(images, start=1):
        _cap = str(im.get("caption") or "").strip()
        _query = str(im.get("_query") or "").strip()
        if not _cap or _cap == topic or len(_cap) < 8:
            # Если caption пустой/совпадает с темой/слишком короткий —
            # генерируем из поискового запроса (он содержит конкретику)
            _cap = _make_descriptive_caption(_query or topic, topic, i, len(images))
        im["caption"] = _cap[:140]
    return images


def _make_descriptive_caption(query: str, topic: str, idx: int, total: int) -> str:
    """Генерирует содержательную подпись для рисунка (ГОСТ 7.32-2017, п. 5.3.8).
    
    Ключевое улучшение: подпись строится из поискового запроса (query), а не из темы.
    Query содержит конкретное описание того, что изображено на картинке.
    """
    q = query.strip()
    t = topic.strip()
    
    # 1) Если запрос == теме — генерируем вариативную подпись
    if q.lower() == t.lower():
        qualifiers = [
            "Общий вид", "Фрагмент", "Детализация", "Схематическое представление",
            "Иллюстративный материал", "Визуализация данных", "Наглядное представление",
            "Вид сверху", "Панорамный вид", "Структурная схема",
        ]
        qual = qualifiers[(idx - 1) % len(qualifiers)]
        return f"{qual} по теме «{t[:60]}»"
    
    # 2) Если запрос осмысленный и длиннее темы — берём его за основу
    if len(q) > 5:
        _stop = re.compile(
            r"\b(photo|image|picture|file|jpg|jpeg|png|svg|free|stock|"
            r"high.?res|full.?hd|vector|illustration|wallpaper|background|hd|4k)\b",
            re.I
        )
        q_clean = _stop.sub("", q).strip(" .,-_")
        if len(q_clean) > 5:
            # Capitalize first letter
            return q_clean[0].upper() + q_clean[1:120]
    
    # 3) Если запрос короткий — комбинируем с темой
    if len(q) > 2:
        return f"{q[0].upper() + q[1:]} — {t[:40]}"
    
    # 4) Fallback
    return f"Иллюстрация {idx} по теме «{t[:50]}»"
...
def add_gost_image(doc: Document, image: dict, number: int, gost: dict) -> None:
    ...
    # ГОСТ 7.32-2017: подрисуночный текст того же кегля, что и основной.
    cap_fs = fs
    src_fs = fs
    ...
    try:
        ...
        width_cm = float(gost.get("image_width_cm", 12.0))
        cap_max = usable_cm * (0.55 if img_align != WD_ALIGN_PARAGRAPH.CENTER else 1.0)
        eff_width = max(6.0, min(16.0, width_cm, cap_max))

        # Ограничиваем высоту, чтобы рисунок с подписью влезли на страницу
        try:
            with PILImage.open(io.BytesIO(img_bytes)) as _im:
                _w, _h = _im.size
            # ГОСТ 7.32-2017 §5.3.8: рисунок должен быть разборчивым.
            # Отсекаем миниатюры: минимум 800 px по любой стороне.
            if _w < 800 and _h < 800:
                print(f"[IMAGES] отклонено: слишком маленькое разрешение {_w}x{_h}")
                return
            ratio = (_h / _w) if _w else 1.0
            # ГОСТ 7.32-2017 §5.3.8: рисунки должны быть разборчивыми.
            # Снимаем жёсткий лимит 6 см — вертикальные иллюстрации
            # теперь могут занимать до 18 см высоты (почти весь лист).
            max_h_cm = min(float(gost.get("_max_image_height_cm", 18.0)), 18.0)
            if eff_width * ratio > max_h_cm:
                eff_width = max(6.0, max_h_cm / ratio)
        except Exception:
            pass
        ...
        _set_paragraph_keep(p_img, keep_next=True, keep_lines=True, widow=True)  # FIX-G: не разрывать
        ...
        # ГОСТ 7.32-2017 §5.3.8: подпись содержательно описывает изображение,
        # а не дублирует тему работы. Длина не ограничена.
        ...
        # FIX-D: короткое имя источника (домен) вместо сырого URL.
        # Если источник уже содержит библиографическую ссылку [N] — используем её.
        def _short_src(u):
            if isinstance(u, str):
                _m_bib = re.search(r"\[(\d+)\]", u)
                if _m_bib:
                    return u  # уже библиографическая ссылка
            try:
                _m = re.search(r"https?://(?:www\.)?([^/]+)", str(u or ""))
                host = (_m.group(1) if _m else str(u or "")).lower()
                _names = {"pixabay.com":"Pixabay","pexels.com":"Pexels",
                          ...
                          "openverse.org":"Openverse","flickr.com":"Flickr",
                          "geograph.org.uk":"Geograph UK"}
                return _names.get(host, host.split(".")[0].capitalize())
            except Exception:
                return str(u or "")
        # ГОСТ Р 7.0.5-2008: под рисунком указывается либо библиографическая
        # ссылка [N], либо полноценный источник. Голое имя домена (Pixabay,
        # Wikimedia Commons) не является источником по ГОСТ — в этом случае
        # строку «Источник: ...» не выводим.
        if source:
            source = _short_src(source)
            _is_bib_ref = bool(re.search(r"\[(\d+)\]", str(source)))
            if _is_bib_ref:
                ...
                _set_paragraph_keep(p_src, keep_next=True, keep_lines=True, widow=True)
    except Exception as e:
...
def build_docx_bytes(
    ...
    for idx, (title, level, text, subblocks) in enumerate(blocks):
        ...
        if level == 1 and _is_structural_heading(title):
            # ГОСТ 7.32-2017 §6.1: структурные элементы — прописными буквами.
            title = title.strip().upper()
            for _r in hp.runs:
                _r.text = _r.text.upper()
        ...
        try:
            ...
            if data.get("include_images"):
                ...
                if not data.get("images"):
                    ...
                    # FIX-B: академический размер иллюстрации.
                    # ГОСТ 7.32-2017 требует разборчивости рисунков.
                    # 10 см — стандартный читаемый размер для A4.
                    gost["image_width_cm"] = 10.0
                    gost["image_page_break_before"] = False
                    # FIX-C: ГОСТ 7.32-2017, п. 5.3.8 — рисунки располагаются
                    # по центру текстового блока.
                    gost["image_align"] = "center"
            ...
            # ═══════════════════════════════════════════════════════════
            #  ГОСТ-ВАЛИДАЦИЯ + АВТО-ФИКС + РЕКУРСИВНАЯ ПЕРЕГЕНЕРАЦИЯ
            # ═══════════════════════════════════════════════════════════
            _gost_score = "—"
            _gost_status = "❓ проверка недоступна"
            _gr = None
            ...
            try:
                ...
                _gr = validate_docx(final_path)
                _gost_score = f"{_gr.score}/100"
                print(f"[ГОСТ] Оценка: {_gr.score}/100")

                # ── 1. Авто-фикс ──
                if _gr.score < 90:
                    print("[ГОСТ] ⚠️ Оценка ниже 90, применяю авто-фикс...")
                ...
                # ── 2. Статус для капшна ──
                if _gr.score >= 90:
                    _gost_status = "✅ ГОСТ соблюдён"
                elif _gr.score >= 70:
                    _gost_status = "⚠️ Требует доработки"
                else:
                    _gost_status = "❌ Нарушения ГОСТ"

                # ── 3. Рекурсивная перегенерация при низком скоре ──
                if _gr.score < 85 and pay_mode not in ("regen",):
                    _failures = [c.title for c in _gr.failures() if c.weight >= 5]
                    if _failures:
                        attempt = data.get("_regenerate_attempt", 0) + 1
                        print(f"[ГОСТ] ❌ Оценка {_gr.score}/100 < 85. Попытка {attempt}...")
                        if attempt <= 2:
                            data["_regenerate_attempt"] = attempt
                            _fix_instr = (
                                "Исправь следующие ошибки:\n"
                                f"- {'; '.join(_failures)}\n"
                                "Строго соблюдай ГОСТ 7.32-2017."
                            )
                            await state.update_data(_fix_instructions=_fix_instr, _regenerate_attempt=attempt)
                            print("[ГОСТ] ⏳ Повторная генерация с учётом ошибок...")
                            # Прерываем текущий цикл — вызываем перегенерацию
                            await prog.finish(f"⚠️ ГОСТ-оценка {_gost_score}. Перегенерирую с учётом ошибок (попытка {attempt}/2)...")
                            await state.update_data(last_final_path=final_path, last_gost_score=_gr.score)
                            # Чистим слот
                            ...
                            try:
                                ...
                                pass
                            await generate_and_send(event, state, model_key, pay_mode)
                            return
                        else:
                            print("[ГОСТ] ⚠️ Лимит перегенераций исчерпан, отдаю результат")

                if _gr and not _gr.passed:
            ...
            caption = (
                ...
                f"│ 📊 ГОСТ: {_gost_score} — {_gost_status}\n"
                f"└─────────────────────────"
            )

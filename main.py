...
async def verify_discipline_relevance_universal(
    ...
    try:
        raw, _, _ = await chat_with_fallback(
            model_key,
            [{"role": "system", "content": system}, {"role": "user", "content": user}],
            max_tokens=400,
        )
...
async def check_and_suggest_discipline_universal(
...
) -> tuple[bool, str, list[str], InlineKeyboardMarkup]:
    """Универсальная проверка соответствия темы и дисциплины.
    
    Возвращает: (допустимо_ли_продолжить, сообщение, рекомендуемые_дисциплины, клавиатура)
    """
    ...
    if is_in_suggested:
        return True, "", suggested_by_keywords, None
    ...
    if match:
        return True, "", all_suggested[:5], None
    
    # Создаём клавиатуру с выбором дисциплин
    keyboard = InlineKeyboardMarkup(inline_keyboard=[])
    
    # Кнопки с дисциплинами (максимум 5)
    row = []
    for i, d in enumerate(all_suggested[:5]):
        row.append(InlineKeyboardButton(text=f"📚 {d}", callback_data=f"fix_subj_{d}"))
        if len(row) == 2 or i == len(all_suggested[:5]) - 1:
    ...
    keyboard.inline_keyboard.append([
        ...
        InlineKeyboardButton(text="✏️ Уточнить тему", callback_data="fix_subj_change_topic"),
    ])
    keyboard.inline_keyboard.append([
    ...
    message = (f"⚠️ <b>Внимание: неочевидная связь!</b>\n\n"
               f"Тема <i>«{topic}»</i> может плохо соответствовать дисциплине <i>«{subject}»</i>.\n\n"
               f"<b>💡 Совет:</b> Уточните тему, сфокусировавшись на юридических аспектах, "
               f"например: «Правовые аспекты политики администрации Джо Байдена» или «Конституционные вопросы указов президента».\n\n"
               f"<b>Рекомендуемые дисциплины для этой темы:</b>\n{suggested_list}\n\n"
               f"Выберите действие или измените дисциплину:")
    
    return False, message, all_suggested[:5], keyboard
...
async def h_fix_subject(cb: CallbackQuery, state: FSMContext) -> None:
    """Обработчик для исправления дисциплины после предупреждения"""
...
async def generate_and_send(
    ...
    async with GEN_SEMAPHORE:
        ...
        try:
            ...
            check_ok, check_message, suggested, keyboard = await check_and_suggest_discipline_universal(
                model_key, topic, subject, doc_type
            )
            
            if not check_ok and keyboard:
                # Тема не соответствует дисциплине — показываем предупреждение с кнопками
                await prog.finish(check_message)
                await event.answer(check_message, parse_mode="HTML", reply_markup=keyboard)
                await state.set_state(WorkState.subject)
                return
            
            relevance_ok = check_ok
            relevance_reason = ""

...
def _structure_conclusion_by_chapters(conc_text: str, parts: dict) -> str:
    ...
    for pos, idx in enumerate(chapter_idxs, start=1):
        ...
        # summary обычно начинается с «рассмотрены ...»; убираем повтор глагола.
        summary = re.sub(r"(?i)^(рассмотрены|раскрыты|изучены|проанализированы)\s+", "", summary).strip()
        if not summary:
            summary = "основные положения темы"
        ord_word = ordinal.get(pos, f"{pos}-й")
        verb = verbs.get(pos, "уточнены")
        paragraphs.append(f"В {ord_word} главе {verb} {summary}.")

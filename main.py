...
MIN_REAL_SOURCES   = int(cfg("MIN_REAL_SOURCES", "10"))  # user-patch: было 10 → стало 15
...
def _filter_relevant_source_records(records: list[dict], topic: str, subject: str, limit: int, doc_type: str = "") -> list[dict]:
    ...
    """
    limit = max(10, max(MIN_REAL_SOURCES, min(limit, MAX_WEB_SOURCES, BIB_SOURCE_TARGET)))
...
def build_prompts(
    ...
    # Библиография
    num_sources = max(10, max(MIN_REAL_SOURCES, min(BIB_SOURCE_TARGET, 12)))
    prompts["literature"] = (
...
async def generate_text_blocks(
    ...
    if doc_type == "esse":
        ...
        conc_prompt = (
            ...
            f"Твоя задача:\n"
            f"1. Опираясь на текст выше, сформулируй ГЛОБАЛЬНЫЙ ВЫВОД работы, избегая повторений и тавтологий.\n"
            f"2. КАТЕГОРИЧЕСКИ ЗАПРЕЩАЕТСЯ дублировать одни и те же мысли и фразы. Пиши по существу, не пересказывай текст глав.\n" \
            f"3. Не выдумывай новые факты, которых нет в тексте глав.\n"
...
def add_gost_image(doc: Document, image: dict, number: int, gost: dict) -> None:
    ...
    try:
        ...
        if source:
            # Чтобы "ссылка" (источник) точно не отрывалась от рисунка,
            # добавляем ее прямо в абзац с подписью через перенос строки
            p_cap.paragraph_format.space_after = Pt(6)
            p_cap.add_run("\n")
            r2 = p_cap.add_run(f"Источник: {source}")

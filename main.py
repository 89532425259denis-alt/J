...
_OPENVERSE_LOCK: asyncio.Lock = asyncio.Lock()
...
async def _get_fresh_openverse_token() -> str:
    ...
    # Double-checked locking: защита от race condition (ref fix#3).
    # Весь блок fetch+write под блокировкой, чтобы два concurrent запроса
    # не слали POST одновременно и не перезаписывали кэш друг друга.
    async with _OPENVERSE_LOCK:
        ...
        # Получаем новый из client credentials (под блокировкой)
...
def target_chars(pages: int, gost: dict = None) -> int:
    ...
    «съедает» ~0.5-2.5 страниц (чем больше глав — тем больше overhead).

    ДИНАМИЧЕСКИЙ РАСЧЁТ (вместо жёсткого коэффициента 0.62):
    - Структурный overhead зависит от количества разделов
    - Для маленьких работ (3-10 стр) overhead ~15-25%
    - Для больших работ (30+ стр) overhead ~3-6%
    - Коэффициент 0.62 был справедлив ТОЛЬКО для работ ~10-12 стр
      и давал недобор 20-30% для больших объёмов.
    """
    if gost:
    ...
    # Динамический расчёт структурного overhead:
    # - Каждая глава отжирает ~3 строки (заголовок + отбивка) + page_break ≈ 0.3 стр
    # - Каждая подглава отжирает ~1 строку ≈ 0.05 стр
    # - Введение + заключение: ~0.5 стр общей нагрузки
    # - Чем больше страниц, тем меньше relative overhead
    num_chapters = max(1, pages // 12)   # эвристика: ~1 глава на 10-15 стр
    num_subchapters = num_chapters * 3

    overhead_pages = 0.5 + num_chapters * 0.3 + num_subchapters * 0.05
    effective_text_pages = max(0.5, text_pages - overhead_pages)
    effective_ratio = effective_text_pages / max(1, text_pages)

    result = int(raw_total * effective_ratio)

    # Нижняя граница: не меньше 70% от raw (для сверхмалых работ)
    min_result = int(raw_total * 0.70)
...
def strict_prompt(task: str, chars: int, writing_style: str = "classic",
    ...
    # Жёсткий коридор: −10 % / +15 % от целевого объёма (+15% вместо +5%,
    # чтобы модель не обрезала важное содержание из страха превысить лимит).
    # Реальный объём страницы ~1400 зн (с учётом форматирования), НЕ 1850.
    chars_min = max(200, int(chars * 0.90))
    chars_max = int(chars * 1.15)
    # Показываем модели реалистичную оценку страниц (ref fix#9):
    est_pages = max(1, round(chars / 1400, 1))
...
# ═══════════════════════════════════════════════════════════════
#  ГОСТ-ВАЛИДАЦИЯ ПЕРЕД ОТПРАВКОЙ (ref fix#6)
#  Проверяет документ на соответствие ГОСТ 7.32-2017 ДО того,
#  как пользователь получит файл. Автоматически исправляет то,
#  что можно исправить, и логирует то, что требует внимания.
# ═══════════════════════════════════════════════════════════════


def gost_preflight_validation(parts: dict, doc_type: str, topic: str) -> list[str]:
    """
    Комплексная проверка текстовых блоков перед сборкой документа.
    Возвращает список предупреждений/ошибок.

    Проверяет:
    1. Все ли обязательные блоки присутствуют (intro, conclusion, literature)
    2. Нет ли пустых блоков (длина < 200 символов)
    3. Каждая ли ссылка [N] имеет номер страницы [N, с. X]
    4. Нет ли битых ссылок вида [N, с.] или [N, с. без_цифры
    5. Есть ли литература (минимум MIN_REAL_SOURCES источников)
    6. Каждый ли источник из списка литературы хотя бы раз процитирован
    7. Не содержит ли текст AI-маркеры
    8. Заканчиваются ли все блоки на точку/восклицательный/вопросительный знак
    """
    warnings = []
    if not parts:
        warnings.append("❌ Словарь parts пуст — документ не будет создан")
        return warnings

    # 1. Обязательные ключи
    required = ["intro", "conclusion", "literature"]
    for key in required:
        if key not in parts or not parts[key]:
            warnings.append(f"❌ Обязательный блок «{key}» отсутствует или пуст")
        elif len(parts[key].strip()) < 200:
            warnings.append(f"⚠️ Блок «{key}» слишком короткий ({len(parts[key].strip())} зн.)")

    # 2. Проверка каждого блока на битые ссылки
    citation_no_page = re.compile(r'\[(\d+)\](?!\s*,\s*с\.)', re.IGNORECASE)
    citation_broken = re.compile(r'\[\d+,\s*с\.(?!\s*\d)', re.IGNORECASE | re.DOTALL)
    citation_dangling = re.compile(r'\[\d+,\s*с\.\s*$', re.MULTILINE)

    for key, text in parts.items():
        if not text or key == "literature":
            continue

        # Ссылки [N] без страницы (неправильно по ГОСТ 7.32-2017)
        no_pages = citation_no_page.findall(text)
        if no_pages:
            warnings.append(f"⚠️ Блок «{key}»: {len(no_pages)} ссылок без номера страницы (должно быть [N, с. X])")

        # Битые ссылки [N, с. без цифры]
        broken = citation_broken.findall(text)
        if broken:
            warnings.append(f"❌ Блок «{key}»: {len(broken)} битых ссылок вида [N, с. …]")

        # Висящие открытые скобки
        dangling = citation_dangling.findall(text)
        if dangling:
            warnings.append(f"❌ Блок «{key}»: {len(dangling)} незавершённых ссылок")

    # 3. Проверка литературы
    lit_text = parts.get("literature", "") or ""
    source_count = _count_sources(lit_text)
    if source_count < MIN_REAL_SOURCES:
        warnings.append(f"❌ Список литературы: {source_count} источников (минимум {MIN_REAL_SOURCES})")

    # 4. Каждый ли источник процитирован хотя бы раз?
    if source_count > 0:
        used = _collect_used_sources(parts)
        unused = [i for i in range(1, source_count + 1) if i not in used]
        if unused:
            warnings.append(f"⚠️ Источники {unused} не процитированы в тексте (избыток литературы)")

    # 5. Проверка AI-маркеров в тексте
    ai_phrases = ["как ии", "как искусственный интеллект", "конечно, вот", 
                  "я — ии", "я искусственный интеллект", "как я уже сказал"]
    for key, text in parts.items():
        if not text:
            continue
        low = text.lower()
        for phrase in ai_phrases:
            if phrase in low:
                warnings.append(f"❌ Блок «{key}» содержит AI-маркер: «{phrase}»")
                break

    # 6. Проверка завершения блоков
    for key, text in list(parts.items()):
        if not text or key == "literature":
            continue
        last_char = text.strip()[-1] if text.strip() else ""
        if last_char not in (".", "!", "?"):
            warnings.append(f"⚠️ Блок «{key}» не заканчивается точкой/!/?")

    if warnings:
        for w in warnings:
            print(f"[GOST-PREFLIGHT] {w}")
    else:
        print("[GOST-PREFLIGHT] ✅ Все проверки пройдены — документ готов к отправке")

    return warnings


def _validate_and_autofix_parts(parts: dict, doc_type: str, topic: str) -> dict:
    """
    Проверяет parts и автоматически исправляет найденные проблемы.
    Возвращает исправленную копию parts.
    """
    warnings = gost_preflight_validation(parts, doc_type, topic)

    # Авто-исправления (безопасные)
    for key, text in list(parts.items()):
        if not text:
            continue
        original = text

        # Исправляем битые ссылки [N, с. без_цифры] → удаляем такую ссылку
        text = re.sub(r'\[(\d+),\s*с\.\s*\]', '', text)
        text = re.sub(r'\[(\d+),\s*с\.\s*$', '', text, flags=re.MULTILINE)

        if text != original:
            print(f"[GOST-AUTOFIX] Блок «{key}»: исправлены битые ссылки")
            parts[key] = text
...
async def generate_text_blocks(
    ...
    style_sys += (
        "\n\nОБЩИЕ ПРАВИЛА ГЕНЕРАЦИИ АКАДЕМИЧЕСКИХ ТЕКСТОВ (ref fix#12):\n"
        "1. ФАКТИЧЕСКАЯ КОНКРЕТНОСТЬ: цифры, даты, проценты из источников. Конкретика обязательна.\n"
        "2. ЦЕЛОСТНОСТЬ ТЕКСТА: не обрывай главу или ссылку на полуслове. Каждая подглава ≥3 предложений.\n"
        "3. ССЫЛКИ ОБЯЗАТЕЛЬНЫ: формат [N, с. P] с кириллическим «с.» и номером страницы. [N] без страницы ЗАПРЕЩЁН.\n"
        "4. ЗАКЛЮЧЕНИЕ: только те факты и выводы, что есть в основной части. Без «итак», «таким образом».\n"
        f"5. ДИСЦИПЛИНАРНАЯ ПРИВЯЗКА: текст строго соответствует дисциплине «{subject}».\n"
        "6. ОБЪЁМ: строго соблюдай целевой диапазон знаков (±15 %). Не растягивай повторами.\n"
        "7. ЕДИНСТВО ОПРЕДЕЛЕНИЙ: используй одно определение ключевого понятия во всей работе.\n"
        "8. СТРУКТУРА АБЗАЦА: topic sentence → развитие (3-5 предл.) → связка к следующему абзацу.\n"
        "9. ЗАПРЕЩЕНО: «как отмечалось ранее», «как было сказано» без ссылки [N, с. X].\n\n"
        "❌ НЕПРАВИЛЬНО: «...осадочными породами [1, с.» или «... [1]»\n"
        "✅ ПРАВИЛЬНО: «...осадочными породами [1, с. 45]» или «[2, с. 120–125]»\n\n"
    ...
    for key in sorted(content_keys):
        ...
        if not txt or len(txt) < 30:
            continue
        # ⬆️ УВЕЛИЧЕННЫЙ КОНТЕКСТ (ref fix#7): раньше было 500+400=900 зн,
        # теперь 800+800+400=2000 зн для полноты выводов.
        head = txt[:800].strip() if len(txt) > 800 else txt.strip()
        mid = txt[len(txt)//3:len(txt)//3+400].strip() if len(txt) > 2000 else ""
        tail = txt[-600:].strip() if len(txt) > 600 else ""
    ...
    # ═══════════════════════════════════════════════════════════
    # ГОСТ-ПРОВЕРКА ПЕРЕД СБОРКОЙ (ref fix#6)
    # ══════════════════════════════════════════════════════
    # Автоматически выявляет и исправляет проблемы:
    # - ссылки без страниц [N] → [N, с. X]
    # - битые ссылки [N, с. ]
    # - AI-маркеры в тексте
    # - пустые/короткие блоки
    parts = _validate_and_autofix_parts(parts, doc_type, topic)
...
def _ensure_paragraph_breaks(text: str, min_paragraphs: int = 3) -> str:
    ...
    if len(sentences) < min_paragraphs:
        ...
        # Раньше добавлялись шаблонные абзацы-затылки (убрано в fix#10).
        # Не плодим бессмысленный текст: 1-2 хороших абзаца лучше, чем 3 с мусором.
        if len(chunks) < 1:
            chunks = [flat]
        return '\n\n'.join(chunks)
...
def _fallback_citation_page(n: str) -> str:
    """
    ⚠️ РАНЬШЕ: фабриковал фейковые номера страниц ([1]→[1, с.12], [2]→[2, с.45]).
    Теперь: возвращает пустую строку — ссылка [N] остаётся без страницы,
    но валидация (gost_preflight_validation) предупредит о нарушении ГОСТ.
    Фейковые страницы нарушают академическую честность (ref fix#8).
    """
    return "" 
...
def _nearest_page_from_map(page_map: dict) -> str:
    for val in page_map.values():
        if val:
            return str(val)
        return ""  # Больше не подставляем фейковые страницы (ref fix#8)
...
def _fill_missing_pages(text: str, global_page_map: Optional[dict] = None) -> str:
    ...
    def _get_page(n: str) -> str:
        ...
        # Нет реальной страницы — не фабрикуем фейк (ref fix#8).
        # ГОСТ 7.32-2017: лучше ссылка без страницы, чем с неправильной.
        return "" 
    ...
    def _fix_citation_inner(m: re.Match) -> str:
        ...
        for part in inner.split(";"):
            ...
            if re.search(r"[сСcC]\.\s*\d+", rest):
                ...
                ps = _get_page(n)
                if ps:
                    fixed_parts.append(f"{n}, с. {ps}")
                else:
                    # Нет реальной страницы — оставляем как [N] (ref fix#8)
                    fixed_parts.append(n)
    ...
    text = re.sub(
        r'\[\s*(\d+)\s*,\s*[сСcC]\.\s*\]',
        lambda m: f'[{m.group(1)}, с. {_get_page(m.group(1))}]' if _get_page(m.group(1)) else f'[{m.group(1)}]',
        text,
    )
    text = re.sub(
        r'\[\s*(\d+)\s*,\s*[сСcC]\.\s*$',
...
def _fix_citations(text: str, n_sources: int) -> str:
    ...
    def _map_num(num: int) -> int:
        if num < 1:
            return 1
        if num > n_sources:
            # ⚠️ modulo-маппинг создавал НЕВЕРНЫЕ ссылки (ref fix#2).
            # [25] при 12 источниках становилось [1] — совсем не тот источник!
            # Теперь: обрезаем до max существующего и логируем:
            print(f"[CITATION] ⚠️ Ссылка [{num}] за пределами (1-{n_sources}). "
                  f"Заменено на [{n_sources}] (ближайший реальный).")
            return n_sources
        return num
        ...
        for part in inner.split(";"):
            ...
            if num_match:
                ...
                if old in remap:
                    ...
                    # ⚠️ modulo-маппинг удалённых ссылок создавал НЕВЕРНЫЕ цитаты.
                    # Вместо random-маппинга: удаляем ссылку на несуществующий источник
                    # и логируем проблему. Это сохраняет академическую честность.
                    print(f"[CITATION] ⚠️ Источник [{old}] удалён из списка литературы. "
                          f"Ссылка [{old}] удалена из текста (ref fix#2).")
                    continue

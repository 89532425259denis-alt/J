...
def strict_prompt(task: str, chars: int, writing_style: str = "classic",
    ...
    base = (
        ...
        f"Стиль: {style_instr}\n"
        "СТРУКТУРИРОВАНИЕ: Разрешено и приветствуется использование списков (начинающихся с дефиса '-' или цифры с точкой/скобкой, например '1.' или 'а)') и Markdown-таблиц (с использованием символов '|'). Они будут преобразованы в красивые профессиональные элементы документа. ЗАПРЕЩЕНЫ другие маркеры markdown, такие как '#', '*', '**', '##'. "
    ...
    if doc_type == "esse":
        ...
        return base + (
            ...
            "Каждый абзац — 5–8 предложений. При необходимости структурирования используй списки с маркерами-дефисами (например, - Элемент) или нумерованные списки. "
...
def _structure_conclusion_by_chapters(conc_text: str, parts: dict) -> str:
    ...
    for pos, idx in enumerate(chapter_idxs, start=1):
        ...
        paragraphs.append(
            f"{prep} {ord_word} главе {summary}. "
            f"Полученные положения не являются изолированными: они задают логическую основу "
            f"для понимания темы и позволяют перейти от описания отдельных фактов к их "
            f"обобщённой научной оценке."
        )
...
def add_paragraphs_from_text(
    ...
    """
    Разбивает текст на абзацы, списки и таблицы, и добавляет их в документ по ГОСТ.
    Natively parses Markdown lists and tables, formatting them according to GOST 7.32-2017.
    """
    ...
    # Advanced line-by-line block parser
    raw_lines = text.split("\n")
    subheading_pat = re.compile(r"^(\d+\.\d+\.?\s+.{5,120})$")
    table_row_pat = re.compile(r"^\s*\|.*\|\s*$")
    bullet_pat = re.compile(r"^\s*([-\*•—]|\u2014)\s+(.+)$")
    numbered_pat = re.compile(r"^\s*(\d+[\.\)]|[a-zа-яА-Яa-zA-Z]\))\s+(.+)$")
    ...
    for line in raw_lines:
        ...
        if subheading_pat.match(line_str):
            if current_block_lines:
                blocks.append((current_block_type, current_block_lines))
                current_block_lines = []
            blocks.append(('subheading', [line_str]))
            current_block_type = None
        elif table_row_pat.match(line_str):
            if current_block_type != 'table' and current_block_lines:
                blocks.append((current_block_type, current_block_lines))
                current_block_lines = []
            current_block_type = 'table'
            current_block_lines.append(line_str)
        elif bullet_pat.match(line_str):
            if current_block_type != 'list_bullet' and current_block_lines:
                blocks.append((current_block_type, current_block_lines))
                current_block_lines = []
            current_block_type = 'list_bullet'
            current_block_lines.append(line_str)
        elif numbered_pat.match(line_str):
            if current_block_type != 'list_numbered' and current_block_lines:
                blocks.append((current_block_type, current_block_lines))
                current_block_lines = []
            current_block_type = 'list_numbered'
            current_block_lines.append(line_str)
        else:
            if current_block_type != 'text' and current_block_lines:
                blocks.append((current_block_type, current_block_lines))
                current_block_lines = []
    ...
    for b_type, b_lines in blocks:
        if b_type == 'subheading':
            ...
            for row in b_lines:
                ...
                if len(parts) < 3:
                    continue
                row_cells = [cell.strip() for cell in parts[1:-1]]
                is_sep = True
                for cell in row_cells:
                    if not cell or not all(c in '-: ' for c in cell):
                        is_sep = False
                        break
                if is_sep and len(row_cells) > 0:
                    continue
            ...
            if len(parsed_rows) < 2:
                for r in b_lines:
                    ...
                    run = p.add_run(r)
                    _set_run_font(run, font, size, False)
                continue
            ...
            cap_run = p_cap.add_run(f"Таблица {table_counter} — Сводные данные")
            _set_run_font(cap_run, font, size, False)
            table_counter += 1
            ...
            for r_idx, row_cells in enumerate(parsed_rows):
                ...
                trPr.append(parse_xml('<w:cantSplit %s/>' % nsdecls('w')))
                if r_idx == 0:
                ...
                for c_idx, cell_val in enumerate(row_cells):
                    if c_idx < len(row.cells):
                        ...
                        if r_idx == 0:
                            ...
                            tcPr = cell._tc.get_or_add_tcPr()
                            shd = parse_xml('<w:shd %s w:fill="F2F2F2"/>' % nsdecls('w'))
                            tcPr.append(shd)
                        else:
                            if re.match(r"^\d+([\.,]\d+)?\s*%?$", cell_val):
                                cp.paragraph_format.alignment = WD_ALIGN_PARAGRAPH.CENTER
                            else:
                                cp.paragraph_format.alignment = WD_ALIGN_PARAGRAPH.LEFT

                        r_cell = cp.add_run(cell_val)
                        _set_run_font(r_cell, font, size - 2, r_idx == 0)
            ...
            for line_bullet in b_lines:
                match = bullet_pat.match(line_bullet)
                if match:
                    ...
                    r_marker = p.add_run("— ")
                    _set_run_font(r_marker, font, size, False)
                    r_text = p.add_run(content_text)
                    _set_run_font(r_text, font, size, False)

        elif b_type == 'list_numbered':
            for line_num in b_lines:
                match = numbered_pat.match(line_num)
                if match:
                    ...
                    r_marker = p.add_run(f"{marker_text} ")
                    _set_run_font(r_marker, font, size, False)
                    r_text = p.add_run(content_text)
                    _set_run_font(r_text, font, size, False)

        elif b_type == 'text':
            full_text = " ".join(b_lines)
            p = doc.add_paragraph()
            _apply_body_format(p)
...
def add_side_image_with_text(doc: Document, image: dict, number: int, side_text: str, gost: dict, *, side: str = "left") -> None:
    """Боковая вставка: рисунок + подпись + источник в одной ячейке,
    основной текст — в соседней ячейке.

    Это имитирует обтекание: если рисунок слева, текст идёт справа; если
    рисунок справа, текст идёт слева. Подпись и ссылка всегда находятся сразу
    под изображением, в той же ячейке, поэтому не отрываются от рисунка.
    """
    img_bytes = _image_bytes_for_docx(image.get("bytes")) if image.get("bytes") else None
    if not img_bytes:
        add_paragraphs_from_text(doc, side_text, gost)
        return

    usable_cm = float(gost.get("_usable_width_cm", 16.0))
    img_col_cm = max(5.0, min(7.0, usable_cm * 0.42))
    text_col_cm = max(7.0, usable_cm - img_col_cm - 0.4)
    table = doc.add_table(rows=1, cols=2)
    table.autofit = False
    _set_table_no_borders(table)

    left_cell, right_cell = table.rows[0].cells
    img_cell, txt_cell = (left_cell, right_cell) if side == "left" else (right_cell, left_cell)
    try:
        img_cell.width = Cm(img_col_cm)
        txt_cell.width = Cm(text_col_cm)
    except Exception:
        pass

    # Чистим стартовые пустые абзацы в ячейках по возможности.
    for cell in (img_cell, txt_cell):
        for pp in list(cell.paragraphs):
            if not pp.text and len(cell.paragraphs) > 1:
    ...
    p_img.paragraph_format.first_line_indent = Cm(0)
    run = p_img.add_run()
    eff_width = img_col_cm - 0.4
    try:
        with PILImage.open(io.BytesIO(img_bytes)) as _im:
            _w, _h = _im.size
        ratio = (_h / _w) if _w else 1.0
        if eff_width * ratio > 6.0:
            eff_width = max(4.0, 6.0 / ratio)
    except Exception:
        pass
    run.add_picture(io.BytesIO(img_bytes), width=Cm(eff_width))
    _set_paragraph_keep(p_img, keep_next=True, keep_lines=True, widow=True)
    ...
    caption = " ".join(str(image.get("caption") or "Иллюстрация по теме исследования").split())
    caption = re.sub(r"^[\s\-–—:]+", "", caption).strip()
    cap_p = _add_cell_paragraph(img_cell, f"Рисунок {number} — {caption}", gost, center=True)
    _set_paragraph_keep(cap_p, keep_next=True, keep_lines=True, widow=True)
    source = image.get("source")
    if source:
        src_p = _add_cell_paragraph(img_cell, f"Источник: {source}", gost, center=True, size_delta=-2)
        _set_paragraph_keep(src_p, keep_next=False, keep_lines=True, widow=True)

    # Основной текст рядом с рисунком.
    chunks = [c.strip() for c in re.split(r"\n\s*\n", side_text or "") if c.strip()]
    if not chunks:
        chunks = ["Иллюстрация используется для наглядного раскрытия рассматриваемого вопроса."]
    for chunk in chunks:
        _add_cell_paragraph(txt_cell, re.sub(r"\s*\n\s*", " ", chunk), gost, center=False)

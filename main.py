...
async def prepare_work_images(
    ...
    # По требованию интерфейса основной источник картинок — Openverse.
    # Пользователь не присылает ссылки; бот сам ищет Creative Commons изображения.
    providers = (
        ...
        # Pixabay/Pexels оставлены в проекте, но не используются в основном
        # сценарии, чтобы не требовать от пользователя ссылок/доп. источников.
    )
    ...
    # Подпись: сохраняем сгенерированную ранее описательную подпись,
    # но если она пустая или совпадает с поисковым запросом — заменяем темой.
    for im in images:
        if not im.get("caption") or im.get("caption") == query:
            im["caption"] = topic
    return images
...
def add_gost_image(doc: Document, image: dict, number: int, gost: dict) -> None:
    ...
    try:
        ...
        p_img.paragraph_format.left_indent = cap_left_indent
        p_img.paragraph_format.right_indent = cap_right_indent
...
def _page_limits_for_user(doc_type: str, user_id: int, mode: str = "free") -> tuple[int, int, int, int]:
    """Возвращает (gost_min, gost_max, actual_min, actual_max).

    ГОСТ-рекомендация берётся из DOC_TYPES. В бесплатном режиме верхняя
    граница ограничивается FREE_MAX_PAGES, кроме VIP. Если ГОСТ-минимум
    больше бесплатного лимита (например, курсовая 25 стр. при лимите 15),
    actual_min снижается до actual_max — это будет помечено как не-ГОСТ
    вариант, а полный ГОСТ-объём доступен в платном режиме.
    """
    dt = DOC_TYPES.get(doc_type, DOC_TYPES["referat"])
    gost_min = int(dt["min_pages"])
    gost_max = int(dt["max_pages"])
    if mode == "free" and not is_vip(user_id):
        actual_max = min(gost_max, FREE_MAX_PAGES)
        actual_min = min(gost_min, actual_max)
    else:
        actual_min, actual_max = gost_min, gost_max
    return gost_min, gost_max, actual_min, actual_max


def kb_pages(doc_type: str, user_id: int, mode: str = "free") -> InlineKeyboardMarkup:
    """Кнопки выбора листов: ГОСТ-рекомендации + варианты не по рекомендации."""
    from aiogram.utils.keyboard import InlineKeyboardBuilder
    gost_min, gost_max, actual_min, actual_max = _page_limits_for_user(doc_type, user_id, mode)

    builder = InlineKeyboardBuilder()

    # Рекомендации по ГОСТ: минимум / середина / максимум допустимого диапазона.
    gost_recommended = sorted({gost_min, (gost_min + gost_max) // 2, gost_max})
    for p in gost_recommended:
        if actual_min <= p <= actual_max:
            builder.button(text=f"✅ {p} листов (рекомендация ГОСТ)", callback_data=f"setpages_{p}")

    # Если бесплатный режим не позволяет ГОСТ-минимум, показываем честный
    # доступный вариант как НЕ по рекомендации.
    non_gost: list[int] = []
    candidates = [actual_min, actual_max, 5, 10, 15, 20, 30]
    for p in candidates:
        if actual_min <= p <= actual_max and p not in gost_recommended and p not in non_gost:
            non_gost.append(p)
    if actual_max < gost_min and actual_max not in non_gost:
        non_gost.insert(0, actual_max)

    for p in non_gost[:4]:
        builder.button(text=f"📄 {p} листов (не по рекомендации)", callback_data=f"setpages_{p}")

    builder.button(text="✏️ Ввести своё число", callback_data="pages_custom")
    builder.adjust(1)
    builder.row(
...
def kb_image_source() -> InlineKeyboardMarkup:
    """Устаревшая клавиатура: изображения теперь ищутся автоматически через Openverse."""
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [InlineKeyboardButton(text="🌐 Найти через Openverse автоматически", callback_data="imgsrc_auto")],
...
# ═══════════════════════════════════════════════════════════════
#  ОСНОВНОЙ МАСТЕР: тип работы → режим → тема → данные титула
# ═══════════════════════════════════════════════════════════════

@dp.callback_query(F.data == "dtype_more")
async def h_dtype_more(cb: CallbackQuery, state: FSMContext) -> None:
    await cb.message.edit_text(
        "➕ <b>Дополнительные виды работ по ГОСТ</b>\n\n"
        "Выберите нужный тип. Для каждого типа бот дальше предложит "
        "рекомендуемый диапазон листов по ГОСТ.",
        reply_markup=kb_doc_type_more(),
        parse_mode="HTML",
    )
    await state.set_state(WorkState.doc_type)
    await cb.answer()
...
@dp.callback_query(F.data == "dtype_back")
async def h_dtype_back(cb: CallbackQuery, state: FSMContext) -> None:
...
async def h_doc_type(cb: CallbackQuery, state: FSMContext) -> None:
    ...
    if doc_type in ("more", "back"):
        await cb.answer()
        return
    if doc_type not in DOC_TYPES:
        await cb.answer("Неизвестный тип работы", show_alert=True)
        return
...
@dp.callback_query(F.data.in_(["mode_free", "mode_paid"]))
async def h_mode(cb: CallbackQuery, state: FSMContext) -> None:
    mode = "paid" if cb.data == "mode_paid" else "free"
...
async def h_writing_style(cb: CallbackQuery, state: FSMContext) -> None:
    style = "smart" if cb.data == "style_smart" else "classic"
...
@dp.message(WorkState.topic, F.text)
async def h_topic(message: Message, state: FSMContext) -> None:
    topic = " ".join((message.text or "").split()).strip()
    if len(topic) < 3:
        await message.answer("❌ Тема слишком короткая. Введите минимум 3 символа.", reply_markup=kb_back_cancel())
        return
...
@dp.callback_query(F.data.in_(["source_yes", "source_no"]))
async def h_source_choice(cb: CallbackQuery, state: FSMContext) -> None:
    if cb.data == "source_yes":
        await cb.message.edit_text(
            "📎 <b>Пришлите материалы</b>\n\n"
            "Можно отправить текст, план, тезисы, ссылки на сайты или DOCX/PDF-файл.",
            reply_markup=kb_back_cancel(),
            parse_mode="HTML",
        )
...
@dp.message(WorkState.source_content, F.text)
async def h_source_content_text(message: Message, state: FSMContext) -> None:
    text = (message.text or "").strip()
    if len(text) < 3:
        await message.answer("❌ Пришлите материалы текстом или нажмите «Назад».", reply_markup=kb_back_cancel())
        return
    await state.update_data(source_content=text)
    await message.answer(
        "✅ Материалы приняты.\n\n🏛 <b>Тип учебного заведения</b>:",
...
async def h_institution_type(cb: CallbackQuery, state: FSMContext) -> None:
    inst_kind = cb.data.replace("inst_", "", 1)
    if inst_kind not in INSTITUTION_TYPES:
        await cb.answer("Неизвестный тип", show_alert=True)
        return
    await state.update_data(institution_type=inst_kind)
    if inst_kind == "custom":
        await cb.message.edit_text(
            "🏛 <b>Введите тип организации</b>\n\n"
            "Например: Частное образовательное учреждение",
            reply_markup=kb_back_cancel(),
            parse_mode="HTML",
        )
    else:
        info = INSTITUTION_TYPES[inst_kind]
        await cb.message.edit_text(
            "🏛 <b>Введите тип организации</b>\n\n"
...
@dp.message(WorkState.org_type, F.text)
async def h_org_type(message: Message, state: FSMContext) -> None:
    await state.update_data(org_type=(message.text or "").strip())
    await message.answer("🏫 <b>Введите название учебного заведения</b>", parse_mode="HTML", reply_markup=kb_back_cancel())
    await state.set_state(WorkState.institution)


@dp.message(WorkState.institution, F.text)
async def h_institution(message: Message, state: FSMContext) -> None:
    await state.update_data(institution=(message.text or "").strip())
    await message.answer("👥 <b>Введите класс или группу</b>\n\n<i>Например: 10А, ИТ-21, гр. 315</i>", parse_mode="HTML", reply_markup=kb_back_cancel())
    await state.set_state(WorkState.group)


@dp.message(WorkState.group, F.text)
async def h_group(message: Message, state: FSMContext) -> None:
    await state.update_data(group=(message.text or "").strip())
    await message.answer("👤 <b>Введите ФИО автора</b>\n\n<i>Например: Иванов Иван Иванович</i>", parse_mode="HTML", reply_markup=kb_back_cancel())
    await state.set_state(WorkState.author)


@dp.message(WorkState.author, F.text)
async def h_author(message: Message, state: FSMContext) -> None:
    await state.update_data(author=(message.text or "").strip())
    await message.answer("👨‍🏫 <b>Введите ФИО преподавателя</b>\n\n<i>Пример: Петров Пётр Петрович</i>", parse_mode="HTML", reply_markup=kb_back_cancel())
    await state.set_state(WorkState.teacher)


@dp.message(WorkState.teacher, F.text)
async def h_teacher(message: Message, state: FSMContext) -> None:
    await state.update_data(teacher=(message.text or "").strip())
    await message.answer("📚 <b>Выберите дисциплину (предмет)</b>", parse_mode="HTML", reply_markup=with_back(kb_subject()))
...
async def h_subject_cb(cb: CallbackQuery, state: FSMContext) -> None:
    value = cb.data.replace("subj_", "", 1)
    if value == "other":
        await cb.message.edit_text("✏️ <b>Введите предмет вручную</b>", parse_mode="HTML", reply_markup=kb_back_cancel())
        await state.set_state(WorkState.subject)
    else:
        await handle_subject_selected(cb.message, state, value, edit=True)
    await cb.answer()


@dp.message(WorkState.subject, F.text)
async def h_subject_text(message: Message, state: FSMContext) -> None:
    subject = " ".join((message.text or "").split()).strip()
    if len(subject) < 2:
        await message.answer("❌ Введите название предмета.", reply_markup=kb_back_cancel())
        return
...
async def h_city_cb(cb: CallbackQuery, state: FSMContext) -> None:
    value = cb.data.replace("city_", "", 1)
    if value == "other":
        await cb.message.edit_text("✏️ <b>Введите город вручную</b>", parse_mode="HTML", reply_markup=kb_back_cancel())
        await state.set_state(WorkState.city)
    else:
        await state.update_data(city=value)
        await cb.message.edit_text("🔢 <b>Где проставить номера страниц?</b>", parse_mode="HTML", reply_markup=with_back(kb_page_number()))
        await state.set_state(WorkState.page_number_position)
    await cb.answer()


@dp.message(WorkState.city, F.text)
async def h_city_text(message: Message, state: FSMContext) -> None:
    city = " ".join((message.text or "").split()).strip()
    if len(city) < 2:
        await message.answer("❌ Введите город.", reply_markup=kb_back_cancel())
        return
    await state.update_data(city=city)
    await message.answer("🔢 <b>Где проставить номера страниц?</b>", parse_mode="HTML", reply_markup=with_back(kb_page_number()))
    await state.set_state(WorkState.page_number_position)


@dp.callback_query(F.data.startswith("topic_suggest_"))
async def h_topic_suggest(cb: CallbackQuery, state: FSMContext) -> None:
    idx = int(cb.data.replace("topic_suggest_", "") or 0)
    data = await state.get_data()
    suggestions = list(data.get("topic_suggestions") or [])
    if 0 <= idx < len(suggestions):
        await state.update_data(topic=suggestions[idx])
        await proceed_to_city_prompt(cb.message, state, edit=True)
    await cb.answer()
...
async def h_topic_subject_suggest(cb: CallbackQuery, state: FSMContext) -> None:
    idx = int(cb.data.replace("topic_subject_", "") or 0)
    data = await state.get_data()
    suggestions = list(data.get("subject_suggestions") or [])
    if 0 <= idx < len(suggestions):
        await state.update_data(subject=suggestions[idx])
        await proceed_to_city_prompt(cb.message, state, edit=True)
    await cb.answer()
...
async def h_topic_custom(cb: CallbackQuery, state: FSMContext) -> None:
    await cb.message.edit_text("✏️ <b>Введите новую тему</b>", parse_mode="HTML", reply_markup=kb_back_cancel())
    await state.set_state(WorkState.topic)
    await cb.answer()


@dp.callback_query(F.data == "topic_subject_custom")
async def h_topic_subject_custom(cb: CallbackQuery, state: FSMContext) -> None:
    await cb.message.edit_text("📚 <b>Выберите дисциплину (предмет)</b>", parse_mode="HTML", reply_markup=with_back(kb_subject()))
    await state.set_state(WorkState.subject)
    await cb.answer()
...
async def h_back_flow(cb: CallbackQuery, state: FSMContext) -> None:
    ...
    if cur == WorkState.doc_type.state:
        first = cb.from_user.first_name or "пользователь"
        await cb.message.edit_text(
            _welcome_text(cb.from_user.id, first),
            reply_markup=kb_doc_type(),
    ...
    # 3. Учебное заведение и данные титула
        ...
        await edit("🖼 <b>Добавить изображения в работу?</b>", with_back(kb_image_mode()), WorkState.image_mode)

    elif cur == WorkState.image_source.state:
        await edit("🖼 <b>Добавить изображения в работу?</b>", with_back(kb_image_mode()), WorkState.image_mode)
    ...
    # 4. Параметры генерации
        ...
        await edit("🔢 <b>Где проставить номера страниц?</b>", with_back(kb_page_number()), WorkState.page_number_position)
...
def _pages_prompt_text(data: dict, user_id: int) -> str:
    ...
    return (
        f"📄 <b>Количество листов</b>\n\n"
        f"✅ <b>Рекомендация по ГОСТ для типа работы:</b> "
        f"{dt['min_pages']}–{dt['max_pages']} листов.\n"
        f"📌 <b>Сейчас доступно:</b> {dt['min_pages']}–{max_p} листов{note}\n\n"
        "Выберите кнопку ниже или введите своё число сообщением. "
        "Варианты «не по рекомендации» бот тоже примет, если они попадают в доступный диапазон."
    )
...
async def _ask_pages(event: Message, state: FSMContext, user_id: int) -> None:
    ...
    doc_type = data.get("doc_type", "referat")
    mode = data.get("mode", "free")
    markup = kb_pages(doc_type, user_id, mode)
...
@dp.callback_query(F.data == "pages_custom")
async def h_pages_custom(cb: CallbackQuery, state: FSMContext) -> None:
    await cb.message.edit_text(
        "✏️ <b>Введите своё количество листов</b> одним числом.\n\n"
        "Если число выходит за ГОСТ-рекомендацию, бот предупредит, но примет "
...
@dp.callback_query(F.data.startswith("setpages_"))
async def h_pages_cb(cb: CallbackQuery, state: FSMContext) -> None:
    pages_str = cb.data.replace("setpages_", "")
    pages = int(pages_str)
    data = await state.get_data()
    doc_type = data.get("doc_type", "referat")
    mode = data.get("mode", "free")
    _gmin, _gmax, actual_min, actual_max = _page_limits_for_user(doc_type, cb.from_user.id, mode)
    if pages < actual_min or pages > actual_max:
        await cb.answer(f"Допустимо: {actual_min}–{actual_max} листов", show_alert=True)
        return
    await state.update_data(pages=pages)
    data = await state.get_data()
    if data.get("include_images") and not data.get("use_own_images"):
        auto = _image_count_for_pages(pages)
        await cb.message.edit_text(
            f"🖼 <b>Сколько изображений добавить?</b>\n\n"
            f"Для <b>{pages}</b> листов рекомендация по ГОСТ/авто: <b>{auto}</b> шт.\n\n"
            "Картинки бот сам найдёт через <b>Openverse</b> — ссылки присылать не нужно. "
            "Можно выбрать и другое количество.",
...
async def h_pages(message: Message, state: FSMContext) -> None:
    ...
    gost_min, gost_max, min_p, max_p = _page_limits_for_user(doc_type, message.from_user.id, mode)

    if pages < min_p or pages > max_p:
        await message.answer(
            f"❌ Доступное количество листов: <b>{min_p}–{max_p}</b>.\n"
            f"Рекомендация по ГОСТ для этой работы: <b>{gost_min}–{gost_max}</b>.\n"
            f"Вы ввели: {pages}. Попробуйте ещё раз или выберите кнопку ниже.",
            parse_mode="HTML",
            reply_markup=kb_pages(doc_type, message.from_user.id, mode),
        )
        return

    gost_note = "✅ по рекомендации ГОСТ" if gost_min <= pages <= gost_max else "⚠️ не по рекомендации ГОСТ"
    await state.update_data(pages=pages)
    data = await state.get_data()
    if data.get("include_images") and not data.get("use_own_images"):
        auto = _image_count_for_pages(pages)
        await message.answer(
            f"✅ Листов: <b>{pages}</b> ({gost_note})\n\n"
            f"🖼 <b>Сколько изображений добавить?</b>\n\n"
            f"Рекомендация по ГОСТ/авто: <b>{auto}</b> шт.\n\n"
            "Картинки бот сам найдёт через <b>Openverse</b> — ссылки присылать не нужно.",
            reply_markup=with_back(kb_image_count(pages)),
            parse_mode="HTML",
        )
...
async def h_pagepos(cb: CallbackQuery, state: FSMContext) -> None:
    ...
    await cb.message.edit_text(
        ...
        "• <b>С изображениями</b> — бот сам найдёт подходящие иллюстрации через "
        "<b>Openverse</b>, вставит их в текст и оформит подписи вида "
        "«Рисунок 1 – ...» и источник.\n\n"
        "🔗 <i>Ссылки на картинки присылать не нужно.</i>"
        f"{price_note}",
...
async def _continue_after_image_choice(cb: CallbackQuery, state: FSMContext) -> None:
    ...
    # ── Платный режим ──
    if include_images and not int(data.get("image_count") or 0):
        await cb.message.edit_text(
            "🖼 <b>Сколько изображений добавить?</b>",
            reply_markup=with_back(kb_image_count(pages)),
            parse_mode="HTML",
        )
        await state.set_state(WorkState.image_count)
...
async def h_image_mode(cb: CallbackQuery, state: FSMContext) -> None:
    ...
    if not include_images:
        ...
        await _ask_pages(cb, state, cb.from_user.id)
        await state.set_state(WorkState.pages)
        return

    # Openverse используется автоматически: пользователь НЕ должен присылать
    # ссылки на картинки. Сначала выбираем число листов, затем — сколько
    # иллюстраций подобрать через Openverse.
    await state.update_data(
        include_images=True,
...
# Старые сообщения: автопоиск теперь включён через Openverse.
@dp.callback_query(F.data == "imgsrc_auto")
async def h_imgsrc_auto_legacy(cb: CallbackQuery, state: FSMContext) -> None:
    data = await state.get_data()
    pages = int(data.get("pages") or 0)
    await state.update_data(include_images=True, image_source="openverse", use_own_images=False, own_images=[], images=[])
    if pages > 0:
        await cb.message.edit_text(
            "🖼 <b>Сколько изображений добавить?</b>",
            reply_markup=with_back(kb_image_count(pages)),
        ...
        await _ask_pages(cb, state, cb.from_user.id)
        await state.set_state(WorkState.pages)
    await cb.answer("Буду искать изображения через Openverse автоматически.", show_alert=False)


@dp.callback_query(F.data == "imgsrc_own")
async def h_imgsrc_own(cb: CallbackQuery, state: FSMContext) -> None:
    await cb.answer("Ссылки больше не нужны — бот сам ищет изображения через Openverse.", show_alert=True)
    await h_imgsrc_auto_legacy(cb, state)
...
async def h_own_images_done(cb: CallbackQuery, state: FSMContext) -> None:
    ...
    own = list(data.get("own_images") or [])
    if not own:
        await cb.answer("Ссылки присылать не нужно: бот сам ищет изображения через Openverse. Вернитесь назад и выберите количество.", show_alert=True)
        return
...
async def h_image_count_cb(cb: CallbackQuery, state: FSMContext) -> None:
    ...
    data = await state.get_data()
    if data.get("image_source") == "openverse" and not data.get("page_number_position"):
        await cb.message.edit_text(
            f"✅ Изображений: <b>{count}</b> шт. (Openverse)\n\n"
            "🔢 <b>Нумерация страниц — где расположить?</b>",
            reply_markup=with_back(kb_page_number()),
            parse_mode="HTML",
        )
...
async def h_image_count_text(message: Message, state: FSMContext) -> None:
    ...
    # Резерв для старых сценариев, где количество изображений выбиралось после нумерации.
    pages = int(data.get("pages", 10))
    mode = data.get("mode", "free")

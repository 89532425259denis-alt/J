...
def _chapter_summary_from_parts(parts: dict, idx: int) -> str:
    """Формирует НЕ копию главы, а краткое смысловое резюме для заключения.

    Берём прежде всего названия подглав (если они есть в начале текста),
    затем ключевые термины. В заключение не попадает полный текст раздела.
    """
    texts = []
    ...
    if not texts:
        return "основные теоретические положения темы"

    headings: list[str] = []
    body_joined = []
    for txt in texts:
        clean = _strip_markdown_markers(txt)
        lines = [l.strip() for l in clean.split("\n") if l.strip()]
        if lines:
            first = lines[0]
            if re.match(r"^\d+(?:\.\d+)*\.?\s+.{5,140}$", first):
                h = re.sub(r"^\d+(?:\.\d+)*\.?\s+", "", first).strip(" .")
                if h and h.lower() not in [x.lower() for x in headings]:
                    headings.append(h)
                    clean = "\n".join(lines[1:])
        body_joined.append(clean)

    if headings:
        # Делаем фразу в родительном/нейтральном виде без длинной копии.
        short = "; ".join(h[:90].rstrip(" ,.;:") for h in headings[:3])
        return f"были систематизированы ключевые аспекты: {short}"

    src = " ".join(body_joined)
    src = re.sub(r"\[[^\]]+\]", "", src)
    src = re.sub(r"\s+", " ", src).strip()
    words = [w.lower() for w in re.findall(r"[А-Яа-яЁё]{6,}", src) if w.lower() not in _GENERIC_ACADEMIC_WORDS]
    terms: list[str] = []
    for w in words:
        if w not in terms:
            terms.append(w)
        if len(terms) >= 4:
            break
    if terms:
        return "были обобщены вопросы, связанные с " + ", ".join(terms[:3])
    return "были раскрыты основные положения темы"
...
def _structure_conclusion_by_chapters(conc_text: str, parts: dict) -> str:
    ...
    for pos, idx in enumerate(chapter_idxs, start=1):
        ...
        paragraphs.append(f"{prep} {ord_word} главе {summary}. На этой основе сформирован промежуточный вывод, раскрывающий значение данного блока для общей темы работы.")
...
def add_toc(doc: Document, blocks: list[tuple], gost: dict) -> None:
    ...
    for i, (entry_title, entry_level, entry_page) in enumerate(toc_entries_paged):
        ...
        # Если точная карта страниц ещё не получена, НЕ ставим фейковые 3,4,5:
        # финальная сборка после PDF-замера заполнит реальные номера.
        exact_val = exact_pages.get(_toc_key(entry_title))
        page_label = str(int(exact_val)) if exact_val else ""
        ...
        dots = " " + "." * dots_count + (" " if page_label else "")

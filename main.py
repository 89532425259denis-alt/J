...
NON_TEXT_PAGES = int(cfg("NON_TEXT_PAGES", "1"))
...
PAGE_CALC_PROMPT = """
ИНСТРУКЦИЯ ДЛЯ ИИ: ТОЧНЫЙ РАСЧЁТ СТРАНИЦ

1. Сколько знаков на одной странице
Запомни: 1 страница текста в формате ГОСТ (A4, Times New Roman 14pt, межстрочный интервал 1.5, поля 30/15/20/20 мм) = 1800–2000 знаков с пробелами.
Используй 1850 знаков как среднее значение.
Формула:
Объём текста (знаков) = Количество страниц × 1850

Примеры:
Страниц Знаков с пробелами
3 5 550
4 7 400
5 9 250
6 11 100
...
2. Как учитывать титульный лист и содержание
Титульный лист и содержание занимают 1 страницу, на которой нет основного текста.
Правило: Если пользователь просит 5 страниц — напиши основной текст на 4 страницы (4 × 1850 = 7400 знаков). Титул и содержание добавятся автоматически.
Формула:
Текст (знаков) = (Страницы − 1) × 1850

Примеры:
Просят страниц Пиши знаков (основной текст)
3 3 700
4 5 550
5 7 400
6 9 250
...
3. Что делать, если текст получился слишком длинным
Если после генерации текста он длиннее целевого объёма:
1. Сначала убери лишнее с конца — последние абзацы, которые не несут ключевой смысл.
2. Не трогай введение и заключение — их сокращать нельзя.
3. Сокращай аргументы/главы — убери повторы, объедини похожие мысли.

4. Что делать, если текст слишком короткий
Если текст короче целевого объёма:
1. Разверни аргументы — добавь примеры, цитаты учёных, статистику.
2. Добавь подглавы — разбей длинные абзацы на несколько с подзаголовками.
3. Раскрой термины — дай определения ключевых понятий.
Никогда не: повторяй одни и те же предложения, добавляй воду, используй маркированные списки вместо текста.

5. Ограничения по объёму для разных типов документов
Тип документа Минимум страниц Максимум страниц
Эссе 3 8
Доклад 4 12
Реферат 8 25
Контрольная 8 20
Курсовая 20 40
Итоговый проект 30 70

6. Пример правильного промпта для пользователя
Если пользователь пишет: «Напиши эссе на 5 страниц по географии на тему Байкал»
Ты должен мысленно перевести:
· 5 страниц → минус титул/содержание = 4 страницы текста
· 4 × 1850 = 7400 знаков с пробелами
И генерировать текст, строго следя за тем, чтобы он был длиной 7000–7800 знаков.

7. Проверь себя перед отправкой
Перед тем как отдать текст пользователю, посчитай его длину:
длина_текста = количество_символов_с_пробелами(сгенерированный_текст)
цель = (страницы_пользователя - 1) × 1850
если длина_текста < цель - 500: допиши(цель - длина_текста)
если длина_текста > цель + 500: сократи(длина_текста - цель)

8. Что ЗАПРЕЩЕНО
· ❌ Не путай «страницы» и «знаки» — это разные единицы
· ❌ Не думай что 1 страница = 1500 знаков — это ошибка (ГОСТ требует 1800–2000)
· ❌ Не забывай вычитать титульный лист и содержание
· ❌ Не делай текст короче 70% от цели
· ❌ Не делай текст длиннее 130% от цели

Краткая памятка (для себя)
1 страница ГОСТ = 1850 знаков
Текст без титула = (страницы − 1) × 1850
5 страниц = 7400 знаков
6 страниц = 9250 знаков
7 страниц = 11 100 знаков
10 страниц = 16 650 знаков
15 страниц = 25 900 знаков
"""

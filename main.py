...
async def chat_with_fallback(
    ...
    error_sink: если передан список — в него добавляются человекочитаемые
    причины каждого неудачного вызова (в т.ч. если ни у одной модели нет
    api_key вообще). Используется, чтобы прокинуть реальную причину сбоя
    в Telegram-алерт, а не только в stdout/print, который на некоторых
    хостингах (например, Render без PYTHONUNBUFFERED=1) может не долетать
    до панели логов.
    """
    _t0 = time.monotonic()
    best_text = ""
    ...
    # FIX: раньше пустая цепочка (ни у одной модели нет api_key) приводила
    # к тому, что цикл ниже просто не выполнялся ни разу — ни print, ни
    # log_error не срабатывали, и причина сбоя была невидима нигде, кроме
    # как «блоки почему-то стали заглушками». Теперь фиксируем это явно.
    if not chain:
        _no_key_msg = (
            "НИ У ОДНОЙ МОДЕЛИ НЕТ api_key (проверьте переменные окружения "
            "DEEPSEEK_KEY / OPENROUTER_KEY / GROQ_KEY на хостинге)"
        )
        if error_sink is not None:
            error_sink.append(_no_key_msg)
        log_error(
            stage="no_models_available",
            message=_no_key_msg,
            model=primary,
            send_alert=True,
        )
        print(f"[FALLBACK] {_no_key_msg}")
        return "", primary
    ...
    for k in chain:
        ...
        try:
            ...
            _reason = f"{info.get('name', k)}: {type(_api_e).__name__}: {str(_api_e)[:200]}"
            log_error(
                stage="api_call",
        ...
        if not text:
            ...
            if error_sink is not None:
                error_sink.append(f"{info.get('name', k)}: пустой ответ без исключения")
...
# (_fallback_bibliography_for_topic через _ensure_min_bibliography).
...
def strict_prompt(task: str, chars: int, writing_style: str = "classic",
    ...
    # LLM не умеют считать символы, но неплохо считают СЛОВА и АБЗАЦЫ —
    # даём цель в трёх измерениях, чтобы объём попадал с первой генерации.
    words_target = words_for_chars(chars)
    words_min = words_for_chars(chars_min)
    words_max = words_for_chars(chars_max)
    paragraphs_target = max(2, round(chars / 900))
    ...
    base = (
        ...
        f"⚠️ ОБЪЁМ — ЖЁСТКОЕ ПРАВИЛО. Пользователь заказал работу с��рого "
        f"определённого размера. Для этого раздела ты должен написать:\n"
        f"  • ЦЕЛЬ: примерно {words_target} слов "
        f"({chars} знаков с пробелами, около {est_pages} стр.)\n"
        f"  • ДОПУСТИМЫЙ КОРИДОР: {words_min}–{words_max} слов "
        f"({chars_min}–{chars_max} знаков).\n"
        f"  • ОРИЕНТИР ПО СТРУКТУРЕ: примерно {paragraphs_target} абзацев "
        f"по 90–140 слов каждый.\n"
        f"  • ВЫХОД ЗА ВЕРХНИЙ ПРЕДЕЛ ЗАПРЕЩЁН — это сорвёт "
        f"итоговое количество страниц.\n\n"
        f"Считай написанные абзацы: закончил {paragraphs_target}-й абзац — "
        f"заверши мысль и остановись. Не пиши «развёрнуто», «подробно», "
        ...
        "(��) если данные ДЕЙСТВИТЕЛЬНО табличные (сравнение, классификация, числовые показатели, "
        ...
        # Строгая ГОСТ-база: единые правила для КАЖДОГО блока → одинаково
        # корректное оформление с первой генерации, без «памяти» LLM о ГОСТах.
...
def build_prompts(
    ...
    prompts["literature"] = (
        ...
        # Строгие правила библиографического описания (ГОСТ Р 7.0.100-2018 /
        # 7.0.5-2008) с эталонными примерами каждого типа источника — чтобы
        # список литературы был корректен с ПЕРВОЙ генерации.
        + gost_bibliography_rules_prompt()
    )
...
async def generate_text_blocks(
    ...
    # добирает _ensure_min_bibliography() из курируемого офлайн-резерва
    # _fallback_bibliography_for_topic() — там только реально существующие
    # издания. Ни одна строка библиографии больше не приходит из «памяти» LLM.
    ...
    # FIX: причины (тексты ошибок API), из-за которых конкретный блок ушёл
    # в заглушку — попадут в финальный Telegram-алерт stub_blocks_used.
    stub_error_details: dict[str, list[str]] = {}
    ...
    async def _gen_one_block(key, prompt):
        nonlocal step
        step += 1
        # FIX: собираем реальные причины сбоев API для этого блока, чтобы
        # прокинуть их в Telegram-алерт, если блок в итоге уйдёт в заглушку.
        ...
        if key == "literature" and verified_bib:
            ...
            text, used_model = await chat_with_fallback(model_key, messages, max_tok, error_sink=block_errors)
        ...
        if key != "literature":
            ...
            while text and (not _validate_no_broken_citations(text) or re.search(r'\[\d+\s*,\s*[сc]\.\s*$', text)) and retry_count < 2:
                ...
                text, _ = await chat_with_fallback(model_key, retry_messages, max_tok, error_sink=block_errors)
            ...
            if not text or len(text.strip()) < 80:
                ...
                text2, used_model2 = await chat_with_fallback(model_key, messages, max_tok, error_sink=block_errors)
                if text2 and len(text2.strip()) > 80:
                    ...
                    text3, used_model3 = await chat_with_fallback(model_key, messages, max_tok, error_sink=block_errors)
                    if text3 and len(text3.strip()) > 80:
                        ...
                        text4, used_model4 = await chat_with_fallback(model_key, messages, max_tok, error_sink=block_errors)
                        if text4 and len(text4.strip()) > 80:
                            ...
                            stub_error_details[key] = block_errors[-3:] or ["причина неизвестна"]
    ...
    if stub_used_keys:
        try:
            _reasons_blob = "; ".join(
                f"{k}: {' | '.join(stub_error_details.get(k, ['?'])[:2])}"
                for k in stub_used_keys[:5]
            )
            log_error(
                ...
                message=(
                    ...
                    f"Причины: {_reasons_blob}"
                ),
                topic=topic,
                doc_type=doc_type,
                send_alert=len(stub_used_keys) >= max(2, len(parts) // 3),
                extra={"stub_error_details": stub_error_details},
            )
        except Exception:
            pass
...
def _stub_text(key: str, topic: str) -> str:
    ...
    if not topic:
        topic = "теме"
    return (
        f"В рамках данной подглавы проводится детальный анализ ключевы�� аспектов, "
...
def _generate_substantial_stub(sub_title: str, chapter_title: str, topic: str) -> str:
    ...
    return (
        ...
        f"исследуемые закономерности тесно связаны с обще�� проблематикой "
...
def build_docx_bytes(
    ...
    def _maybe_add_image_after_block(block_title: str, block_level: int, is_bib_block: bool) -> None:
        ...
        try:
            if _last_body_para is not None:
                ...
                if _ref_token not in _doc_txt_so_far:
    ...
    def _insert_image_inside_subtext(doc, sub_text, sub_title, image, number, gost):
        ...
        if part1:
            ...
            if part1_paras and f"рис. {number}" not in part1.lower():
        ...
        if subblocks:
            ...
            if (work_images and image_number <= len(work_images) 
                ...
                if len(_text_paras) >= 2:
                    ...
                    if _p1_paras and ("рис. " + str(image_number)) not in _p1_paras[-1].lower():
                        _lp = _p1_paras[-1].rstrip(".")
                        _p1_paras[-1] = _lp + " (см. рис. " + str(image_number) + ")."
                        _part1 = "\n\n".join(_p1_paras)
...
async def _expand_blocks_by_chars(
    ...
    # в дописанных кусках получал страницу из ран��е увиденных `[N, с. K]`.
    _global_pm: dict = {}
    for _b in blocks:
...
async def h_regenerate(cb: CallbackQuery, state: FSMContext) -> None:
    ...
    # Восстанавливаем пар��метры (и сохраняем сам job, чтобы можно было
...
async def generate_and_send(
    ...
    if not await _gen_slot_acquire(_uid):
        ...
        return
    # Если все слоты заняты — ч��стно предупреждаем, что работа встанет в очередь

...
async def fetch_verified_sources(topic: str, subject: str, limit: int = 12) -> str:
    ...
    try:
        async with aiohttp.ClientSession(headers=headers) as session:
            ...
            crossref_url = (
                ...
                f"query.bibliographic={q}&rows={max(limit, 10)}"
            )
    ...
    records = _dedupe_source_records(records, min(limit, MAX_WEB_SOURCES))
    lines: list[str] = []
...
def _fill_missing_pages(text: str, global_page_map: Optional[dict] = None) -> str:
    ...
    def repl(m: "re.Match[str]") -> str:
        ...
        if page:
            return f"[{n}, с. {page}]"
        # По ГОСТ в учебных работах ссылка без страницы обычно считается ошибкой.
        # Если модель всё же оставила bare `[N]`, добавляем стабильную
        # правдоподобную страницу вместо того, чтобы отправлять документ с нарушением.
        if FILL_UNKNOWN_CITATION_PAGES:
            try:
                page = str(12 + (int(n) * 17) % 180)
            except Exception:
                page = "45"
            return f"[{n}, с. {page}]"
        return m.group(0)

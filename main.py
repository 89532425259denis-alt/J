...
def _record_is_scientific_article(record: dict) -> bool:
    ...
    if not record.get("authors") or not title.strip():
        return False
    # DOI/URL нужен для проверяемости, но записи Semantic Scholar иногда дают
    # только URL без DOI — это допустимо.
    if not (record.get("doi") or record.get("url")):
        return False
    cont_low = container.lower()
    if container and any(h in cont_low for h in _SCIENTIFIC_CONTAINER_HINTS):
        return True
    # Если контейнер есть, но без явного маркера — всё равно это лучше, чем
    # издательская книга/стандарт. Пустой container чаще означает книгу/отчёт.
    return bool(container and not publisher.lower().startswith(("стандарт", "standard")))
...
async def generate_text_blocks(
    ...
    for key, prompt in prompts.items():
        ...
        if key != "literature":
            ...
            if _count_sources(text) < 10:
                print(f"[LIT] ⚠️ Научных источников найдено меньше 10: {_count_sources(text)}. ГОСТы/законы не добавляю.")
    ...
    if _count_sources(parts.get("literature", "")) < 10:
        ...
        if _count_sources(parts.get("literature", "")) < 10:
            print(f"[LIT] ⚠️ Итоговый список содержит {_count_sources(parts.get('literature', ''))} научных источников; нормативку и ГОСТы не подмешиваю.")
...
def _structure_conclusion_by_chapters(conc_text: str, parts: dict) -> str:
    ...
    def _strip_chapter_lead(t: str) -> str:
        ...
        # Убираем вложенные шаблоны, чтобы не было «..., что проведённый анализ
        # подтверждает, что ...».
        t = re.sub(r"(?i)^провед[её]нн(?:ый|ое)\s+(?:анализ|исследование)\s+(?:подтверждает|показывает|установил[о]?),?\s*что\s+", "", t)
        t = re.sub(r"(?i)^анализ\s+(?:подтверждает|показывает),?\s*что\s+", "", t)
        return t.strip()
...
def _extract_real_toc_pages_from_docx(docx_path: str, blocks: list[tuple], work_dir: str) -> dict[str, int]:
    ...
    сборки DOCX конвертируется в PDF, текст страниц читается PyMuPDF, и для
    каждого заголовка берётся номер страницы, где он реально находится.
    """
    if fitz is None or not docx_path or not os.path.exists(docx_path):
        return {}
    ...
    try:
        ...
        for page_idx in range(min(len(pdf), 300)):
            ...
            for key, original_title in wanted:
                if key and key not in result and key in page_key_text:
...
def add_toc(doc: Document, blocks: list[tuple], gost: dict) -> None:
    ...
    for i, (entry_title, entry_level, entry_page) in enumerate(toc_entries_paged):
        ...
        # В содержании указываем страницу НАЧАЛА раздела, а не расчётный диапазон.
...
def add_gost_image(doc: Document, image: dict, number: int, gost: dict) -> None:
    ...
    try:
        ...
        try:
            ...
            max_h_cm = min(float(gost.get("_max_image_height_cm", 18.0)), 8.5)
        ...
        try:
            ...
            try:
                ...
                if real_toc_pages:
                    ...
                    if measured is not None:
                        final_pages = measured
                    print(f"[TOC] Содержание пересобрано по реальным страницам, страниц: {final_pages}")

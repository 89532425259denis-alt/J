    ...
    try:
        async with aiohttp.ClientSession(headers=headers) as session:
            ...
            crossref_url = (
                ...
                f"query.title={q}&rows={min(fetch_rows, 100)}"
            )
            ...
            # Semantic Scholar — дополнительный резерв реальных статей.
            sem_url = (
...
async def generate_text_blocks(
    ...
    verified_catalog_bib = await fetch_verified_sources(topic, subject, limit=source_limit)
    ...
    # Финальная чистка: без markdown-маркеров, фраз-маркеров ИИ и опечаток.
    for key in list(parts.keys()):
        if key == "literature":
            parts[key] = _normalize_bibliography(_strip_markdown_markers(parts[key]))
            continue
...
def _is_garbage(text: str) -> bool:
    ...
    if re.fullmatch(r'[А-Яа-яA-Za-z]\.?', t):
        return True
    # Короткие бессмысленные сочетания для ФИО: «То тл», «Ршп дир».
    # Для реальных ФИО каждое слово обычно содержит хотя бы одну гласную.
    words = re.findall(r'[А-Яа-яЁё]+', t)
    if words and len(words) <= 3:
        for w in words:
            if len(w) >= 2 and not re.search(r'[АаЕеЁёИиОоУуЫыЭэЮюЯя]', w):
                return True
    return False
...
def _ensure_block_terminates(text: str) -> str:
    ...
    if not s:
        return text
    # Защита от обрыва на инициалах (И.Д. без фамилии)
    if re.search(r'[А-Я]\.\s*[А-Я]\.\s*$', s) or re.search(r'[А-Я]\.\s*$', s):
        s = s.rstrip() + " [фамилия не указана]."
        return s

...
def build_prompts(
    ...
    prompts["literature"] = (
        ...
        f"монографии, рецензируемые статьи, законы, ГОСТы, диссертации."
    )
...
async def generate_text_blocks(
    ...
    for key, prompt in prompts.items():
        ...
        if key != "literature":
            ...
            llm_bib = _normalize_bibliography(_clean_ai_artifacts(text or ""))
            if not llm_bib or _count_sources(llm_bib) < 3:
                llm_bib = ""
            text = _combine_bibliographies(
                verified_bib,
                llm_bib,
                limit=source_limit,
            )
            if _count_sources(text) < 10:
                # Аварийный резерв: только реальные методологические/ГОСТ-
                # источники, без фиктивных «Ивановых/Петровых».
                text = _combine_bibliographies(
                    text,
                    _stub_text("literature", topic),
    ...
    # Последняя страховка: в готовом документе должно быть минимум 10 реальных
    # позиций. Сначала добираем найденными каталогами/URL, затем — реальным
    # аварийным методологическим списком, но никогда не фиктивными авторами.
    if _count_sources(parts.get("literature", "")) < 10:
        parts["literature"] = _normalize_bibliography(_combine_bibliographies(
...
def _stub_text(key: str, topic: str) -> str:
    ...
    Для списка литературы используются только реальные методологические и
    нормативные источники (минимум 10), без фиктивных авторов.
    """
    ...
    stubs = {
        ...
        "literature": (
            ...
            "Отчет о научно-исследовательской работе. Структура и правила оформления. — М.: Стандартинформ, 2018.\n"
            "2. ГОСТ Р 7.0.5-2008. Система стандартов по информации, библиотечному и издательскому делу. "
            "Библиографическая ссылка. Общие требования и правила составления. — М.: Стандартинформ, 2008.\n"
            "3. ГОСТ Р 7.0.100-2018. Система стандартов по информации, библиотечному и издательскому делу. "
            "Библиографическая запись. Библиографическое описание. Общие требования и правила составления. — М.: Стандартинформ, 2018.\n"
            "4. ГОСТ Р 7.0.12-2011. Система стандартов по информации, библиотечному и издательскому делу. "
            "Библиографическая запись. Сокращение слов и словосочетаний на русском языке. — М.: Стандартинформ, 2012.\n"
            "5. Федеральный закон от 29.12.2012 № 273-ФЗ «Об образовании в Российской Федерации». — URL: https://www.consultant.ru/document/cons_doc_LAW_140174/.\n"
            "6. Creswell J.W., Creswell J.D. Research Design: Qualitative, Quantitative, and Mixed Methods Approaches. — 5th ed. — Thousand Oaks: SAGE Publications, 2018.\n"
            "7. Booth W.C., Colomb G.G., Williams J.M., Bizup J., Fitzgerald W.T. The Craft of Research. — 4th ed. — Chicago: University of Chicago Press, 2016.\n"
            "8. Eco U. How to Write a Thesis. — Cambridge, MA: MIT Press, 2015.\n"
            "9. Новиков А.М., Новиков Д.А. Методология научного исследования. — М.: Либроком, 2010. — 280 с.\n"
            "10. Кузин Ф.А. Кандидатская диссертация: методика написания, правила оформления и порядок защиты. — М.: Ось-89, 2011. — 224 с."
        ),
    }
    if key in stubs:
        return stubs[key]
...
def _structure_conclusion_by_chapters(conc_text: str, parts: dict) -> str:
    ...
    connectors = [
        "В первой части работы установлено, что",
        "Дальнейшее рассмотрение показало, что",
        "Третий блок исследования позволил уточнить, что",
        "Наряду с этим выявлено, что",
        "Практический аспект темы выражается в том, что",
        "Дополнительно обосновано, что",
    ]
...
def add_toc(doc: Document, blocks: list[tuple], gost: dict) -> None:
    ...
    for i, (entry_title, entry_level, entry_page) in enumerate(toc_entries_paged):
        ...
        next_page = toc_entries_paged[i + 1][2] if i + 1 < len(toc_entries_paged) else None
...
def add_gost_image(doc: Document, image: dict, number: int, gost: dict) -> None:
    ...
    Поэтому изображение, подпись и строка «Источник: …» оформляются как
    неразрывная группа. По умолчанию группа начинается с новой страницы: так
    предыдущий текст не «съедает» место и не выталкивает ссылку на следующий
    лист.
    """
    ...
    try:
        ...
        # Важно: рисунок + подпись + источник должны уйти на один лист.
        # Начинаем группу с новой страницы (можно отключить через GOST/ENV,
        # если когда-нибудь понадобится компактная вёрстка).
        if bool(gost.get("image_page_break_before", True)):
        ...
        try:
            ...
            # Оставляем запас под подпись и длинную строку источника/URL.
            max_h_cm = min(float(gost.get("_max_image_height_cm", 18.0)), 14.5)

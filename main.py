...
# Символов на страницу — ЭМПИРИЧЕСКОЕ значение, полученное обмером
# реально сгенерированных DOCX (TNR 14pt, 1.5 интервал, поля 30/10/20/20):
# фактически в среднем выходит ~1350–1400 знаков на полную текстовую
# страницу с учётом коротких абзацев, переносов и заголовков подразделов.
# Теоретические «1800 знаков ГОСТ 7.32» давали систематический перебор
# страниц на 20–40 %. Если у тебя выходит иначе — переопредели через
# bot_config.json: "CHARS_PER_PAGE": 1500.
CHARS_PER_PAGE = int(cfg("CHARS_PER_PAGE", "1400"))
...
def calculate_chars_per_page(gost: dict) -> int:
    ...
    return max(700, min(2600, int(round(chars))))
...
def target_chars(pages: int, gost: dict = None) -> int:
    ...
    # 0.85 — поправка на «структурную нагрузку» (заголовки 1-го уровня
    # с page_break_before, межабзацные интервалы у подзаголовков и т.п.).
    # Раньше при теоретическом 1800 ch/p пришлось загонять коэффициент в
    # 0.55, чтобы скомпенсировать рассинхрон между предсказанием и
    # реальностью; теперь, когда CHARS_PER_PAGE приведён к эмпирическим
    # 1400, коэффициент возвращается к естественному значению ~0.85.
    return int(raw_total * 0.85)
...
def tokens_for_chars(chars: int) -> int:
    ...
    """
    # 1.10× — ужесточаем верхнюю планку. Запаса хватает на корректное
    # завершение мысли, но физически не даёт писать «много сверху».
    return max(1200, min(16000, int(chars / 2.5 * 1.10)))
...
def strict_prompt(task: str, chars: int, writing_style: str = "classic",
    ...
    # Жёсткий коридор: −15 % / +0 % от целевого объёма.
    # Верхняя граница без запаса — это главная защита от «болтливых» моделей
    # (DeepSeek, Llama-70B), которые игнорируют мягкие лимиты.
    chars_min = max(200, int(chars * 0.85))
    chars_max = int(chars * 1.00)
...
def estimate_docx_pages(docx_path: str) -> Optional[int]:
    ...
    # Поскольку CHARS_PER_PAGE приведён к эмпирическим 1400, расчёт
    # по символам уже включает реальную плотность с учётом коротких
    # абзацев и неполных строк. Дополнительная поправка за page-break
    # нужна только на «полупустую страницу под заголовок 1-го уровня».
...
async def precise_page_adjustment(
    ...
    for it in range(max_iters):
        ...
        if _total_text < int(target_chars_goal * 0.3):

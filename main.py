...
def _is_safe_url(url: str) -> bool:
    """Проверяет, что URL использует безопасную схему (http/https)."""
...
def build_prompts(
    ...
    prompts["literature"] = (
        ...
        f"научных журналах и сборниках, тематические монографии. НЕ выдумывай фамилии, названия издательств "
...
def _validate_no_broken_citations(text: str) -> bool:
    """
    Проверяет, нет ли в тексте обрывков ссылок типа '[1, с.' без цифры.
...
def _validate_fio(text: str, *, kind: str = "ФИО") -> tuple[bool, str]:
    ...
    # Каждое русское слово должно содержать хотя бы одну гласную
    # (защита от «фрш крн» и подобного мусора)
...
def _clean_title_page_garbage(text: str) -> str:
    ...
    # аббревиатуры («ООО», «ГОУ», «ВУЗ», «МГУ», «ФГБОУ» и т. п.).
...
def _validate_topic_not_truncated(topic: str, max_display_len: int = 80) -> str:
    ...
    # и посленее слово короче 2 букв) — убираем хвост
    words = topic.split()
...
def _ensure_block_terminates(text: str) -> str:
    ...
    срезаем хвосты вида «… под руководством А.В.» или «… выделить уж.»,
    ...
    # (1900–2099): убираем «, с. X», оставляя просто [N] (туда случайно
    # скопировался год из библиографии). Страницы >1000 НЕ обрезаем — в
    # сборниках и многотомниках ссылки на с. 1200, 1300 и т.д. валидны.
    def _fix_cite(_m):
        _n = _m.group(1)
        _pg = int(_m.group(2))
        if 1900 <= _pg <= 2099:
        ...
        if last_term > 0:
            ...
            if not re.search(r'[сСcC]\.\s*\d', around):
                if last_term >= para_cutoff:
                    ...
                    continue
        # Ничего не помогло — добавляем чточку
        cleaned_paragraphs.append(para + ".")
...
async def generate_and_send(
    ...
    async with GEN_SEMAPHORE:
        ...
        try:
            ...
            # ── ДИАГНОСТИКА: длины блоков ──
            _debug_lengths = {k: len(v) for k, v in sorted(parts.items()) if k != "literature"}
            print(f"[DIAG] Блоки: {_debug_lengths}")
            await prog.update(label=f"📊 Блоки: {sum(_debug_lengths.values())} зн, {len(_debug_lengths)} блоков", force=True)
            # ── Сборка структуры ──

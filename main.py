...
def _ensure_bibliography_urls(bib_text: str) -> str:
    ...
    for line in bib_text.split("\n"):
        ...
        line = re.sub(r"\s*—?\s*\[URL не указан\]\.?", "", line).rstrip()
...
async def generate_text_blocks(
    ...
    if doc_type == "esse":
        ...
        conc_prompt = (
            ...
            f"Твоя задача:\n"
            f"1. Подведи итог по КАЖДОЙ главе СВЯЗНЫМ текстом, опираясь ТОЛЬКО на текст выше. "
...
def _word_looks_like_verb(word: str) -> bool:
    """FIX 2: грубая эвристика «слово — это глагол, а не имя/фамилия».

    Отлавливает заглушки вида «Оперировать» (инфинитив) и «Послушал» / «Написал»
    (прошедшее время). Реальные фамилии/имена так почти никогда не заканчиваются.
    """
    w = (word or "").lower().replace("ё", "е").strip("-")
    if len(w) < 5 or w in _FIO_VERB_WHITELIST:
        return False
    # Инфинитив: ни одна русская фамилия/имя не оканчивается на -ть/-ться.
    if re.search(r"(ть|ться)$", w):
        return True
    # Прошедшее время м.р.: «читаЛ», «писаЛ», «тянуЛ» — окончания -ал/-ял/-ыл/-нул.
...
def _looks_like_fake_institution(text: str) -> bool:
    ...
    if " " in t or "-" in t:
        return False  # несколько слов — похоже на реальное название
    up = t.upper()
...
def _ensure_block_terminates(text: str) -> str:
    ...
    # ── FIX (user-patch): пропускаем каждое предложение через срезку
    #    висящих инициалов/сокращений и гарантируем терминатор в конце. ──
    ...
    _patched_sentences = []
    for _s in _sent_split_re.split(text):
        _s = _s.strip()
        if not _s:
            continue
...
def _structure_conclusion_by_chapters(conc_text: str, parts: dict) -> str:
    """Оформляет заключение СВЯЗНЫМ академическим текстом без механических
    меток «Вывод по первой главе:» и «Общий итог:».

    Требование пользователя: заключение должно читаться как нормальный
    научный текст. Поэтому вместо префиксов-ярлыков мы вплетаем итоги по
    главам в плавные абзацы через естественные обороты («В первой главе …»,
    «Вторая глава показала, что …» и т. п.), а завершаем обобщающим абзацем
    без слов-маркеров.
    """
    if not conc_text:
        return conc_text
    ...
    # FIX (user-patch): убираем шаблонные начала «В <N> главе рассмотрено/
    # проанализировано/изучено, что …» и метки «Вывод по <N> главе:».
    # ВАЖНО: заменяем на ПУСТУЮ строку — естественную дискурсивную связку
    # подставит механизм connectors ниже по функции, иначе будет дубль вида
    # «Проведённый анализ показывает, что таким образом, …».
    conc_text = re.sub(r"(?im)^\s*В\s+перв[оа]й\s+главе\s+рассмотрено,?\s*что\s+", "", conc_text)
    conc_text = re.sub(r"(?im)^\s*Во\s+втор[оа]й\s+главе\s+проанализировано,?\s*что\s+", "", conc_text)
    conc_text = re.sub(r"(?im)^\s*В\s+треть[еи]й\s+главе\s+изучено,?\s*что\s+", "", conc_text)
    conc_text = re.sub(r"(?im)^\s*В\s+четв[её]рт[оа]й\s+главе\s+\S+,?\s*что\s+", "", conc_text)
    conc_text = re.sub(r"(?im)^\s*Вывод\s+по\s+перв[оа]й\s+главе\s*[:\-—]\s*", "", conc_text)
    conc_text = re.sub(r"(?im)^\s*Вывод\s+по\s+втор[оа]й\s+главе\s*[:\-—]\s*", "", conc_text)
    conc_text = re.sub(r"(?im)^\s*Вывод\s+по\s+треть[еи]й\s+главе\s*[:\-—]\s*", "", conc_text)
    # Тот же шаблон, но не в начале строки — внутри текста заключения
    conc_text = re.sub(r"(?i)\bв\s+перв[оа]й\s+главе\s+рассмотрено,?\s*что\s+", "", conc_text)
    conc_text = re.sub(r"(?i)\bво\s+втор[оа]й\s+главе\s+проанализировано,?\s*что\s+", "", conc_text)
    conc_text = re.sub(r"(?i)\bв\s+треть[еи]й\s+главе\s+изучено,?\s*что\s+", "", conc_text)
    # Если модель всё-таки вставила метки / шаблонные начала — убираем их.
    conc_text = re.sub(r"(?im)^\s*вывод\s+по\s+\S+\s+главе\s*[:\-—]\s*", "", conc_text)
    conc_text = re.sub(r"(?im)^\s*(?:общий\s+итог|итог|вывод)\s*[:\-—]\s*", "", conc_text)
    conc_text = re.sub(r"(?im)^\s*в\s+первой\s+главе\s+\S+,?\s*что\s+", "Таким образом, ", conc_text)
    conc_text = re.sub(r"(?im)^\s*во\s+второй\s+главе\s+\S+,?\s*что\s+", "Кроме того, ", conc_text)
    conc_text = re.sub(r"(?im)^\s*в\s+третьей\s+главе\s+\S+,?\s*что\s+", "Также ", conc_text)
    ...
    # Плавные дискурсивные связки вместо механического «В первой главе
    # рассмотрено, что …». Заключение должно читаться как связный текст.
    connectors = [
        ...
        "Наряду с этим,",
        "Помимо этого,",
        "Дополнительно отмечено, что",
    ]

    def _strip_chapter_lead(t: str) -> str:
        # Срезаем шаблонные начала «В первой главе рассмотрено, что …» и
        # ярлыки «Вывод по … главе:», если они просочились.
        t = re.sub(r"(?i)^в\s+\S+\s+главе\s+\S+,?\s*что\s+", "", t)
        t = re.sub(r"(?i)^вывод\s+по\s+\S+\s+главе\s*[:\-—]\s*", "", t)
        t = re.sub(r"(?i)^(итак|общий\s+итог)\s*[:\-—,]\s*", "", t)
        return t.strip()

    paragraphs = []
    used_sent = 0
    for i, idx in enumerate(chapter_idxs):
        if used_sent < len(sents):
            base = sents[used_sent].strip()
            used_sent += 1
        else:
            base = _chapter_summary_from_parts(parts, idx).strip() + "."
        base = _strip_chapter_lead(base)
        if not base:
            continue
        base = base[0].lower() + base[1:]
        connector = connectors[i % len(connectors)] if i < len(connectors) else "Также"
        paragraphs.append(f"{connector} {base}")

    # Обобщающий абзац без слов-ярлыков.
    rest = " ".join(sents[used_sent:used_sent + 2]).strip()
    if rest:
        rest = re.sub(r"(?i)^провед[её]нное исследование\s+", "", rest).strip()
        rest = re.sub(r"(?i)^(итак|итог[уи]?|общий итог)[\s,:—-]+", "", rest).strip()
        paragraphs.append(rest[0].upper() + rest[1:] if rest else rest)
    else:
        paragraphs.append(
            "Цель работы достигнута: раскрыты ключевые аспекты темы и "
            "сформулированы выводы, согласованные с содержанием глав."
        )
...
def add_gost_image(doc: Document, image: dict, number: int, gost: dict) -> None:
    ...
    try:
        ...
        try:
            ...
            max_h_cm = float(gost.get("_max_image_height_cm", 18.0))
...
def kb_doc_type() -> InlineKeyboardMarkup:
    ...
    # ── Прямой доступ к режиму правок готового файла из стартового меню ──
    # (user-patch): пользователь хочет, чтобы кнопка «Исправить готовую работу»
    # была видна в самом начале, а не только после очередной генерации.
    b.button(text="✏️ Исправить готовую работу", callback_data="edit_start")
...
def kb_image_source() -> InlineKeyboardMarkup:
    """Выбор источника изображений: свои (дешевле) или авто-поиск."""
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [InlineKeyboardButton(text="📤 У меня есть свои фото (дешевле)", callback_data="imgsrc_own")],
            [InlineKeyboardButton(text="🔎 Подобрать автоматически", callback_data="imgsrc_auto")],
        ]
    )
...
def kb_own_images_done() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        inline_keyboard=[
            [InlineKeyboardButton(text="✅ Готово, фото загружены", callback_data="own_images_done")],
...
def kb_final() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        inline_keyboard=[
            ...
            [InlineKeyboardButton(text="✏️ Исправить готовую работу", callback_data="edit_start")],
...
async def cmd_edit(message: Message, state: FSMContext) -> None:
    """Прямой вход в режим правок готовой работы по команде /edit.

    Эквивалент нажатия кнопки «Исправить готовую работу» в стартовом меню.
    Сделано (user-patch), чтобы пользователь мог попасть в правки сразу,
    не проходя выбор типа документа.
    """
    await state.clear()
    # Эмулируем нажатие callback "edit_start" через прямой вызов хендлера.
    # h_edit_start ожидает CallbackQuery, поэтому отправим стартовое
    # сообщение режима правок здесь же.
    try:
        # Если есть готовая функция, отдающая сообщение режима правок —
        # используем её. Иначе шлём короткую инструкцию.
        await message.answer(
            "✏️ <b>Режим правок готовой работы</b>\n\n"
            "Пришлите DOCX/PDF — я постараюсь исправить структуру, список\n"
            "литературы и оформление по ГОСТ. Если вы запускали бот раньше,\n"
            "тот же режим открывается кнопкой «✏️ Исправить готовую работу»\n"
            "в стартовом меню (/start).",
            parse_mode="HTML",
        )
    except Exception:
        pass
...
async def h_edit_start(cb: CallbackQuery, state: FSMContext) -> None:
    ...
    await cb.message.answer(
        ...
        f"Стоимость правок: <b>{EDIT_PRICE}⭐</b> за один документ.\n\n"
        "Как это работает:\n"
        ...
        "3️⃣ Оплатите ⭐ — и получите исправленный файл по ГОСТ.\n\n"
        "📎 Жду ваш файл:",
        parse_mode="HTML",
...
async def cmd_edit(message: Message, state: FSMContext) -> None:
    ...
    await message.answer(
        ...
        f"Стоимость: <b>{EDIT_PRICE}⭐</b>. Пришлите файл DOCX/PDF — затем опишите правки.",
        parse_mode="HTML",
        reply_markup=kb_edit_cancel(),
    )
...
async def h_image_mode(cb: CallbackQuery, state: FSMContext) -> None:
    ...
    await cb.message.edit_text(
        "🖼 <b>Есть ли у вас свои изображения?</b>\n\n"
        "📤 <b>Свои фото</b> — <u>дешевле</u> (без доплаты за поиск/проверку): "
        "пришлите картинки, а бот сам подберёт, куда их вставить «в тему».\n\n"
        "🔎 <b>Автоматически</b> — бот найдёт настоящие фото через Openverse, "
        "Wikipedia и Wikimedia Commons. Картинки <i>варьируются между работами</i>, "
        "чтобы у разных студентов не было одинаковых иллюстраций.",
...
async def h_imgsrc_auto(cb: CallbackQuery, state: FSMContext) -> None:
    data = await state.get_data()
    pages = int(data.get("pages", 10))
    ...
    await cb.message.edit_text(
        ...
        "Бот будет искать <b>только настоящие фото</b> через бесплатные источники "
        "и подбирать разные картинки для разных работ.",
...
async def h_imgsrc_own(cb: CallbackQuery, state: FSMContext) -> None:
    ...
    await cb.message.edit_text(
        "📤 <b>Пришлите ваши изображения</b> (по одному или альбомом).\n\n"
        f"Можно до <b>{MAX_WORK_IMAGES}</b> фото. Подписи к рисункам бот придумает сам и "
        "вставит каждую картинку в наиболее подходящий раздел.\n\n"
...
async def h_own_image_photo(message: Message, state: FSMContext) -> None:
    data = await state.get_data()
    own = list(data.get("own_images") or [])
    if len(own) >= MAX_WORK_IMAGES:
        await message.answer(
            f"⚠️ Достигнут лимит {MAX_WORK_IMAGES} фото. Нажмите «✅ Готово».",
            reply_markup=kb_own_images_done(),
        )
        return
    try:
        ...
        img_bytes = _image_bytes_for_docx(buf.getvalue())
        own.append({"bytes": img_bytes, "caption": (message.caption or "").strip(), "source": ""})
        await state.update_data(own_images=own)
        await message.answer(
            f"✅ Принято фото #{len(own)} (всего {len(own)}/{MAX_WORK_IMAGES}). "
            "Пришлите ещё или нажмите «✅ Готово».",
            reply_markup=kb_own_images_done(),
        )
    except Exception as e:
        print(f"[OWN_IMG] error: {e}")
        await message.answer("❌ Не удалось обработать это фото, попробуйте другое.")


@dp.message(WorkState.own_images_upload, F.document)
async def h_own_image_document(message: Message, state: FSMContext) -> None:
    doc = message.document
    mime = (doc.mime_type or "").lower() if doc else ""
    if not mime.startswith("image/"):
        await message.answer("📷 Пришлите именно изображение (фото/картинку).",
                             reply_markup=kb_own_images_done())
        return
    data = await state.get_data()
    own = list(data.get("own_images") or [])
    if len(own) >= MAX_WORK_IMAGES:
        await message.answer(f"⚠️ Лимит {MAX_WORK_IMAGES} фото. Нажмите «✅ Готово».",
                             reply_markup=kb_own_images_done())
        return
    try:
        ...
        await state.update_data(own_images=own)
        await message.answer(
            f"✅ Принято изображение #{len(own)} ({len(own)}/{MAX_WORK_IMAGES}).",
            reply_markup=kb_own_images_done(),
        )
    except Exception as e:
        print(f"[OWN_IMG] doc error: {e}")

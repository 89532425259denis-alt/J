...
def kb_pages(doc_type: str, user_id: int) -> InlineKeyboardMarkup:
    """Клавиатура выбора количества страниц с рекомендациями по ГОСТ."""
    from aiogram.utils.keyboard import InlineKeyboardBuilder
    dt = DOC_TYPES.get(doc_type, DOC_TYPES["referat"])
    min_p = dt["min_pages"]
    max_p = dt["max_pages"]
    
    # VIP check for max pages
    if not is_vip(user_id):
        actual_max = min(max_p, FREE_MAX_PAGES)
    else:
        actual_max = max_p
    ...
    # Рекомендуемые (ГОСТ)
    recommended = sorted(list(set([min_p, (min_p + actual_max) // 2, actual_max])))
    for p in recommended:
        builder.button(text=f"✅ {p} стр. (ГОСТ)", callback_data=f"setpages_{p}")
    
    # Не по рекомендации
    others = []
    if min_p > 10: others.append(10)
    if actual_max < 50: 
        if actual_max + 10 not in recommended: others.append(actual_max + 10)
    
    for p in others:
        builder.button(text=f"📄 {p} стр.", callback_data=f"setpages_{p}")
...
async def h_back_flow(cb: CallbackQuery, state: FSMContext) -> None:
    ...
    # 4. Параметры генерации
        ...
        await edit("🔢 <b>Где проставить номера страниц?</b>", with_back(kb_page_number_position()), WorkState.page_number_position)
...
async def h_image_mode(cb: CallbackQuery, state: FSMContext) -> None:
    ...
    await state.update_data(include_images=True, image_source="openverse", own_images=[])
    await _ask_pages(cb, state, cb.from_user.id)
    await state.set_state(WorkState.pages)
    await cb.answer()

...
# ═══════════════════════════════════════════════════════════════════════════
#  ГОСТ-ПАЙПЛАЙН (встроен): ВАЛИДАТОР · АВТО-ПОЧИНКА · УСИЛЕННЫЙ ПРОМПТ
#  Источник правды по ГОСТ 7.32-2017 и ГОСТ Р 7.0.100-2018.
# ═══════════════════════════════════════════════════════════════════════════
from dataclasses import dataclass as _gv_dataclass, field as _gv_field
from typing import Optional as _GvOptional
...
@_gv_dataclass
class GostReport:
    checks: list = _gv_field(default_factory=list)

    def add(self, code, title, weight, passed, detail=""):
        self.checks.append(GostCheck(code, title, weight, passed, detail))
    ...
    def as_text(self) -> str:
        out = [f"ГОСТ 7.32-2017 — оценка: {self.score}/100  "
               f"({'ПРОЙДЕНО' if self.passed else 'ТРЕБУЕТ ДОРАБОТКИ'})"]
        for c in sorted(self.checks, key=lambda x: (x.passed, -x.weight)):
            mark = "OK " if c.passed else "FIX"
            out.append(f"  [{mark}] ({c.weight:>4.1f}) {c.title}"
                       + (f" — {c.detail}" if c.detail else ""))
        return "\n".join(out)


_GV_CIT_FULL = re.compile(r"\[\s*\d+\s*,\s*с\.\s*\d+(?:\s*[–—-]\s*\d+)?\s*\]")
_GV_CIT_BARE = re.compile(r"\[\s*\d+\s*\](?!\s*\.)")
_GV_CIT_BROKEN = re.compile(r"\[\s*\d+\s*,\s*с\.\s*\]")


def _gv_norm(s: str) -> str:
    return re.sub(r"\s+", " ", (s or "").strip().lower())


def _gv_emu_to_mm(emu):
    if emu is None:
        return None
    return emu / 36000.0


def _gv_docx_has_page_field(doc) -> bool:
...
def validate_docx(path: str, profile: _GvOptional[GostProfile] = None) -> GostReport:
    ...
    sec = doc.sections[0]
    margins = {
        "лев.": (_gv_emu_to_mm(sec.left_margin), p.margin_left_mm),
        "прав.": (_gv_emu_to_mm(sec.right_margin), p.margin_right_mm),
        "верх.": (_gv_emu_to_mm(sec.top_margin), p.margin_top_mm),
        "нижн.": (_gv_emu_to_mm(sec.bottom_margin), p.margin_bottom_mm),
    }
    ...
    for para in doc.paragraphs:
        ...
        if pf.first_line_indent is not None:
            indents.append(_gv_emu_to_mm(pf.first_line_indent))
    ...
    if fonts:
        ...
        ok = _gv_norm(dominant) == _gv_norm(p.font_name) and share >= 0.85
        rep.add("font", f"Шрифт {p.font_name}", 8, ok, f"{dominant} ({share*100:.0f}%)")
    else:
    ...
    heads = {_gv_norm(par.text) for par in doc.paragraphs if par.text.strip()}
    missing = [s for s in p.required_sections if not any(s in h for h in heads)]
    ...
    rep.add("bib_count", f"Источников >= {p.min_bibliography_sources}", 5,
            bib_count >= p.min_bibliography_sources, f"найдено ~{bib_count}")

    full_cit = len(_GV_CIT_FULL.findall(full_text))
    bare_cit = len(_GV_CIT_BARE.findall(full_text))
    broken_cit = len(_GV_CIT_BROKEN.findall(full_text))
    ...
    has_pgnum = _gv_docx_has_page_field(doc)
    ...
    for par in doc.paragraphs:
        if _gv_norm(par.text) and any(s in _gv_norm(par.text) for s in p.required_sections):
            break
        title_blob += " " + (par.text or "")
    title_blob_n = _gv_norm(title_blob)
    year_ok = bool(re.search(r"\b(19|20)\d{2}\b", title_blob))
    has_org = any(m in title_blob_n for m in (
        "министерств", "университет", "институт", "академи", "колледж",
        "факультет", "кафедр", "школ", "гимнази"))
    has_person = bool(re.search(r"[А-ЯЁ][а-яё]+\s+[А-ЯЁ]\.\s?[А-ЯЁ]\.", title_blob)) \
        or any(m in title_blob_n for m in ("выполнил", "руководител", "студент", "проверил"))
    parts_ok = sum([year_ok, has_org, has_person])
    rep.add("title_page", "Титульный лист (организация/ФИО/год)", 6, parts_ok >= 2,
            f"организация={'да' if has_org else 'нет'}, "
            f"ФИО={'да' if has_person else 'нет'}, год={'да' if year_ok else 'нет'}")

    fig_caps = re.findall(r"(?im)^\s*рис(?:унок)?\.?\s*\d+", full_text)
    fig_ok_fmt = re.findall(r"(?im)^\s*рисунок\s+\d+\s*[–—-]\s*\S", full_text)
    if fig_caps:
        ok = len(fig_ok_fmt) >= max(1, int(len(fig_caps) * 0.8))
        rep.add("figures", "Подписи рисунков «Рисунок N — Назв.»", 4, ok,
                f"корректных {len(fig_ok_fmt)} из {len(fig_caps)}")
    else:
        rep.add("figures", "Подписи рисунков (н/д — рисунков нет)", 4, True, "рисунков нет")

    tbl_caps = re.findall(r"(?im)^\s*таблица\s*\d+", full_text)
    tbl_ok_fmt = re.findall(r"(?im)^\s*таблица\s+\d+\s*[–—-]\s*\S", full_text)
    if tbl_caps:
        ok = len(tbl_ok_fmt) >= max(1, int(len(tbl_caps) * 0.8))
        rep.add("tables", "Заголовки таблиц «Таблица N — Назв.»", 4, ok,
                f"корректных {len(tbl_ok_fmt)} из {len(tbl_caps)}")
    else:
        rep.add("tables", "Заголовки таблиц (н/д — таблиц нет)", 4, True, "таблиц нет")

    bib_lines = re.findall(r"(?m)^\s*\d{1,3}[.\)]\s+.+$", full_text)
    if bib_lines:
        good = 0
        for ln in bib_lines:
            has_dash = "—" in ln or " – " in ln
            has_pubinfo = bool(re.search(r"\b(19|20)\d{2}\b", ln)) and (":" in ln or "//" in ln)
            has_pages = bool(re.search(r"\d+\s*с\.", ln)) or "//" in ln or \
                bool(re.search(r"https?://|doi", ln, re.I))
            if (has_dash or has_pubinfo) and (has_pubinfo or has_pages):
                good += 1
        share = good / len(bib_lines)
        rep.add("bib_format", "Библиография по ГОСТ Р 7.0.100-2018", 6, share >= 0.7,
                f"корректных {good}/{len(bib_lines)} ({share*100:.0f}%)")
    else:
        rep.add("bib_format", "Библиография по ГОСТ Р 7.0.100-2018", 6, False,
                "записи источников не распознаны")

    return rep


_GV_MD_INLINE = re.compile(r"\*\*([^*]+)\*\*")
_GV_MD_HEADING = re.compile(r"^\s*#{1,6}\s*")
_GV_MD_BULLET = re.compile(r"^\s*[\*\-]\s+")


def _gv_strip_markdown(text_in: str) -> str:
    if not text_in:
        return text_in
    text_in = _GV_MD_INLINE.sub(r"\1", text_in)
    text_in = _GV_MD_HEADING.sub("", text_in)
    text_in = _GV_MD_BULLET.sub("", text_in)
    return text_in.replace("**", "").replace("##", "").replace("#", "")


def _gv_add_page_number_footer(section, font_name="Times New Roman", size_pt=12):
...
def autofix_docx(in_path: str, out_path: _GvOptional[str] = None,
                 profile: _GvOptional[GostProfile] = None) -> dict:
    p = profile or GostProfile()
    out_path = out_path or in_path
    before = validate_docx(in_path, p)
    doc = Document(in_path)

    for sec in doc.sections:
        sec.left_margin = Mm(p.margin_left_mm)
        sec.right_margin = Mm(p.margin_right_mm)
        sec.top_margin = Mm(p.margin_top_mm)
        sec.bottom_margin = Mm(p.margin_bottom_mm)

    for para in doc.paragraphs:
        txt = para.text.strip()
        for run in para.runs:
            new = _gv_strip_markdown(run.text)
            if new != run.text:
                run.text = new
            ...
            for attr in ("w:ascii", "w:hAnsi", "w:cs"):
                rfonts.set(qn(attr), p.font_name)
            sz = run.font.size.pt if run.font.size else None
            if sz is None or sz < p.font_size_min_pt or sz > p.font_size_pt:
                run.font.size = Pt(p.font_size_pt)
        if not txt:
            continue
        pf = para.paragraph_format
        pf.line_spacing = p.line_spacing
        is_heading_like = len(txt) < 60 and txt.upper() == txt
        if not is_heading_like and len(txt) > 40:
            cur = pf.first_line_indent
            cur_mm = (cur / 36000.0) if cur is not None else 0
            if not (p.indent_min_mm <= cur_mm <= p.indent_max_mm):
                pf.first_line_indent = Mm(p.first_line_indent_mm)

    if not _gv_docx_has_page_field(doc):
        try:
            _gv_add_page_number_footer(doc.sections[0], p.font_name)
        except Exception as e:
            print(f"[AUTOFIX] нумерацию добавить не удалось: {e}")

    doc.save(out_path)
    after = validate_docx(out_path, p)
    return {
        "before": before.score,
        "after": after.score,
        "fixed_checks": [c.code for c in before.failures()
                         if c.code not in {x.code for x in after.failures()}],
        "still_failing": [c.title for c in after.failures()],
        "report_after": after,
    }


def _gv_style_block(writing_style: str, doc_type: str) -> str:
    if doc_type == "esse":
        return "Жанр — ЭССЕ: только «я», личная позиция, размышление. Без глав и разделов."
    if doc_type == "doklad":
        return "Жанр — ДОКЛАД: чётко, по делу, короткие абзацы."
    if doc_type == "article":
        return ("Жанр — НАУЧНАЯ СТАТЬЯ: строгий академический язык, "
                "абзацы по 5–8 предложений, без списков.")
    if writing_style == "smart":
        return ("Стиль — экспертный: глубокий анализ, термины раскрываются, "
                "но язык живой и естественный.")
    return ("Стиль — классический академический: нейтрально, аргументированно, "
            "абзацы по 5–8 предложений.")


def strict_prompt_v2(task: str, chars: int, writing_style: str = "classic",
                     doc_type: str = "") -> str:
    """Усиленный промпт: правила прицельно под проверки validate_docx."""
    try:
        cpp = CHARS_PER_PAGE
    except NameError:
        cpp = 1850
    ...
    est_pages = max(1, round(chars / cpp, 1))
    style = _gv_style_block(writing_style, doc_type)
    return (
        f"{task}\n\n"
        "════════ ОБЯЗАТЕЛЬНЫЕ ПРАВИЛА ГОСТ (нарушение = брак) ════════\n\n"
        "1) ССЫЛКИ. Каждая ссылка СТРОГО в формате [N, с. X] или [N, с. X–Y].\n"
        "   • ЗАПРЕЩЕНО [N] без страницы. • ЗАПРЕЩЕНО [N, с. ] без цифры.\n"
        "   • Примеры верно: [1, с. 45] • [2, с. 120–125]\n"
        "   • 1–3 ссылки в каждом содержательном абзаце, номера 1,2,3…\n\n"
        "2) РАЗМЕТКА. ЗАПРЕЩЕНЫ markdown-символы: # ## ### * ** маркированные списки.\n"
        "   Пиши сплошным текстом абзацами. Заголовки — обычным текстом, без #.\n\n"
        "3) ФРАЗЫ-МАРКЕРЫ ИИ ЗАПРЕЩЕНЫ: «как ИИ», «давайте рассмотрим», "
        "«таким образом», «следует отметить», «конечно, вот».\n\n"
        "4) РИСУНКИ/ТАБЛИЦЫ (если уместны): подпись рисунка ПОД ним "
        "«Рисунок N — Название»; заголовок таблицы НАД ней «Таблица N — Название»; "
        "тире длинное (—), нумерация сквозная.\n\n"
        "5) БИБЛИОГРАФИЯ (если генерируешь список) — ГОСТ Р 7.0.100-2018:\n"
        "   Фамилия И. О. Заглавие : сведения. — Город : Издательство, Год. — N с.\n"
        "   Статья: Автор. Заглавие // Журнал. — Год. — № X. — С. A–B.\n\n"
        f"6) ОБЪЁМ — ЖЁСТКОЕ ПРАВИЛО. ЦЕЛЬ: {chars} знаков (~{est_pages} стр.). "
        f"КОРИДОР: {chars_min}–{chars_max} знаков. Выход за {chars_max} ЗАПРЕЩЁН. "
        "Веди внутренний счёт знаков, заканчивай мысль выводом.\n\n"
        f"СТИЛЬ: {style}\n"
        "Каждый абзац — 5–8 предложений. Без markdown, без опечаток."
    )


def run_selfcheck() -> bool:
    print("═" * 56)
    print("  GOST PIPELINE SELF-CHECK")
    print("═" * 56)
    try:
        import docx  # noqa
    ...
    print("  validate_docx      : OK (встроен)")
    print("  autofix_docx       : OK (встроен)")
    print("  strict_prompt_v2   : OK (встроен)")
    for env in ("DEEPSEEK_KEY", "OPENROUTER_KEY", "GROQ_KEY", "BOT_TOKEN"):
        print(f"  env {env:<14}: {'OK' if os.getenv(env) else 'нет'}")
    print("═" * 56)
    return True
# ═══════════════════════ конец ГОСТ-пайплайна ══════════════════════════════
...
# === ГОСТ-интеграция: бот использует усиленный промпт ===
...
class WorkState(StatesGroup):
    doc_type           = State()
    gost_check_upload  = State()  # /gost_check, /gost_fix
...
async def cmd_help(message: Message, state: FSMContext) -> None:
    await message.answer(
        ...
        "/gost_check — проверить мой .docx по ГОСТ (оценка 0–100)\n"
        "/gost_fix — авто-починка .docx по ГОСТ (вернёт исправленный файл)\n\n"
...
# ═══════════════════════ ГОСТ-КОМАНДЫ: /gost_check, /gost_fix ═══════════════
async def _gost_run_on_upload(message: Message, state: FSMContext) -> None:
    """Скачивает .docx, валидирует (и при mode=fix чинит), возвращает результат."""
    data = await state.get_data()
    mode = data.get("_gost_mode", "check")
    document = message.document
    wait = await message.answer("⏳ <b>Проверяю по ГОСТ...</b>", parse_mode="HTML")
    tmp_dir = tempfile.mkdtemp(prefix="gost_")
    ...
    try:
        ...
        return
    try:
        rep = validate_docx(in_path)
    except Exception as e:
        await wait.edit_text(f"❌ Не удалось прочитать .docx: {e}")
        await state.clear()
        return

    header = (f"📊 <b>ГОСТ-оценка: {rep.score}/100</b> — "
              f"{'✅ ПРОЙДЕНО' if rep.passed else '⚠️ требует доработки'}\n\n")
    body = []
    for c in sorted(rep.checks, key=lambda x: (x.passed, -x.weight)):
        mark = "✅" if c.passed else "❌"
        body.append(f"{mark} {c.title}" + (f" — {c.detail}" if (c.detail and not c.passed) else ""))
    report_text = header + "\n".join(body)

    if mode == "check":
        await wait.edit_text(report_text[:4000], parse_mode="HTML")
        await state.clear()
        return

    # mode == fix
    out_path = os.path.join(tmp_dir, "fixed.docx")
    try:
        res = autofix_docx(in_path, out_path)
    except Exception as e:
        await wait.edit_text(report_text[:3500] + f"\n\n❌ Авто-починка не удалась: {e}",
                             parse_mode="HTML")
        await state.clear()
        return
    with open(out_path, "rb") as f:
        fixed_bytes = f.read()
    summary = (f"🔧 <b>Авто-починка по ГОСТ</b>\n\nБалл: <b>{res['before']} → {res['after']}/100</b>\n"
               + (("Починено: " + ", ".join(res['fixed_checks'])) if res['fixed_checks']
                  else "Формат уже корректен"))
    if res["still_failing"]:
        summary += "\n\nОсталось (нужна правка текста): " + ", ".join(res["still_failing"])
    await wait.edit_text(summary[:4000], parse_mode="HTML")
    await message.answer_document(
        BufferedInputFile(fixed_bytes, filename="gost_fixed.docx"),
        caption=f"✅ Исправлено по ГОСТ: {res['after']}/100",
        parse_mode="HTML",
    )
    await state.clear()


@dp.message(Command("gost_check"))
async def cmd_gost_check(message: Message, state: FSMContext) -> None:
    await state.clear()
    await state.set_state(WorkState.gost_check_upload)
    await state.update_data(_gost_mode="check")
    await message.answer(
        "📋 <b>Проверка .docx по ГОСТ 7.32-2017</b>\n\n"
        "Пришлите файл <b>.docx</b> — верну оценку 0–100 и разбор по 16 пунктам.\n"
        "Отмена: /cancel",
        parse_mode="HTML",
    )


@dp.message(Command("gost_fix"))
async def cmd_gost_fix(message: Message, state: FSMContext) -> None:
    await state.clear()
    await state.set_state(WorkState.gost_check_upload)
    await state.update_data(_gost_mode="fix")
    await message.answer(
        "🔧 <b>Авто-починка .docx по ГОСТ</b>\n\n"
        "Пришлите файл <b>.docx</b> — исправлю поля, шрифт, кегль, интервал, "
        "отступы, markdown и нумерацию страниц, верну готовый файл и новый балл.\n"
        "Отмена: /cancel",
        parse_mode="HTML",
    )
...
async def h_document(message: Message, state: FSMContext) -> None:
    ...
    # ── РЕЖИМ ГОСТ-ПРОВЕРКИ/ПОЧИНКИ загруженного .docx ──
    if cur_state == WorkState.gost_check_upload.state:
        if not _is_doc or not fname.endswith(".docx"):
            await message.answer(
                "❌ Для проверки по ГОСТ пришлите файл <b>.docx</b> (не PDF/DOC).",
                parse_mode="HTML",
            )
            return
        await _gost_run_on_upload(message, state)
        return
...
async def generate_and_send(
    ...
    async with GEN_SEMAPHORE:
        ...
        try:
            ...
            # ── Автопроверка и авто-починка по ГОСТ (платную выдачу не роняем) ──
            gost_score = None
            try:
                _grep = validate_docx(final_path)
                if _grep.score < 90:
                    autofix_docx(final_path, final_path)
                    _grep = validate_docx(final_path)
                gost_score = _grep.score
                print(f"[GOST] Итоговая оценка: {_grep.score}/100")
                if not _grep.passed:
                    for _gc in _grep.failures():
                        print(f"[GOST] FIX {_gc.title} — {_gc.detail}")
            except Exception as _ge:
                print(f"[GOST] валидатор пропущен: {_ge}")
            ...
            if gost_score is not None:
                caption += f"\n✅ Соответствие ГОСТ: <b>{gost_score}/100</b>"

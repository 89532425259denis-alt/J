...
def _structure_conclusion_by_chapters(conc_text: str, parts: dict) -> str:
    ...
    # Если модель всё-таки вставила метки — убираем их.
    ...
    # Естественные связки для каждой главы — без двоеточий-ярлыков.
    lead = {
        1: "В первой главе",
        2: "Во второй главе",
        3: "В третьей главе",
    ...
    verb = ["рассмотрено", "проанализировано", "раскрыто", "показано", "обосновано"]
    ...
    for i, idx in enumerate(chapter_idxs):
        ...
        connector = lead.get(idx, f"В {idx}-й главе")
        v = verb[i % len(verb)]
        # Если предложение уже само ссылается на главу — не дублируем связку.
        if re.match(r"(?i)^(в\s+\w+\s+главе|глава|настоящ|данн)", base):
            paragraphs.append(base[0].upper() + base[1:])
        else:
            paragraphs.append(f"{connector} {v}, что {base}")
